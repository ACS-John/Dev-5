...
00060   read mat mo$
00070   dat$=mo$(val(date$(4:5)))&" "&date$(7:8)&",19"&date$(1:2)
00080   io1$(1)="10,65,N 1,UT,N"
...
00320   fil$(f1+1)=rtrm$(fil$(f1+1))
00330   if pp>0 or fil$(f1+1)="" then goto L360
00340   f1=f1+1
00350   goto L260
00360 L360: f2=f2+1: rl=l=a=b=ino=pg=j3=0: mat a=(0)
00370   if ev$="" then goto L430
00380 L380: linput #2: a$ eof L1390
00390   a$=uprc$(a$)
00400   if rtrm$(a$(11:13))><ex$ then goto L380
00410   fil$(1)=rtrm$(a$(1:8))&"."&ev$
00420   f2=1: f1=2
00430 L430: if rtrm$(fil$(f2))="" then goto L1390
00440   open #1: "Name="&fil$(f2),display,input ioerr L360
00450   goto L660
00460 L460: p1=pos(filename$,".",1)
00470   if p1=0 then p1=min(8,len(filename$)) else p1=p1-1
00480   pr newpage
00490   pr f "1,1,C 40,R,N": " FIELD DESCRIPTIONS: "&filename$(1:p1)&"/"&volid$
00500   pr f "1,43,C 8,R,N": " NAME"
00510   pr f "1,53,C 8,R,N": " FORMAT"
00520   pr f "1,63,C 5,R,N": " FROM"
00530   pr f "1,70,C 5,R,N": "  TO"
00540   sln=1
00550 L550: if pp=0 or j3=0 then goto L660
00560   if sln=0 then goto L460
00570   if sln<23 then sln=sln+1: goto L610
00580   pr f "24,5,C 60,R,N": "  SCREEN FULL  PRESS ENTER TO CONTINUE:"
00590   input fields "24,50,C 1,RE,N": pause$
00600   goto L460
00610 L610: pr f str$(sln)&",1,C 40,UT,N": a$(j3,1)
00620   pr f str$(sln)&",43,C 8,UT,N": a$(j3,2)(1:10)
00630   pr f str$(sln)&",53,C 8,UT,N": a$(j3,3)(1:8)
00640   pr f str$(sln)&",63,PIC(ZZZZZ),UT,N": a(j3,5)
00650   pr f str$(sln)&",70,PIC(ZZZZZ),UT,N": a(j3,6)
00660 L660: linput #1: ln$ eof L1200
00670   if ln$(7:10)="LET " then goto LETLN
00680   if ln$(7:10)="DATA" then goto DATALN
00690   p1=pos(ln$,"Replace",6)
00700   if p1=0 then goto L740
00710   p1=p1+8
00720   p2=pos(ln$,",",p1)
00730   prg$=ln$(p1:p2-1)
00740 L740: goto L550
00750 LETLN: p2=len(rtrm$(ln$))-1
00760   p1=pos(ln$,"H1$",1)
00770   if p1>0 then h1$=ln$(p1+5:p2) : goto L550
00780   p1=pos(ln$,"FILETYPE$",1)
00790   if p1>0 then filetype$=ln$(p1+11:p2): goto L550
00800   p1=pos(ln$,"FILENAME$",1)
00810   if p1>0 then filename$=ln$(p1+11:p2): goto L550
00820   p1=pos(ln$,"VOLID$",1)
00830   if p1>0 then volid$=ln$(p1+8:p2): goto L550
00840   p1=pos(ln$,"RM$",1)
00850   if p1=0 then goto L880
00860   rm=val(ln$(p1+4:p1+4))
00870   rm$(rm)=ln$(p1+8:p2)
00880 L880: goto L550
00890 DATALN: j3=j3+1
00900   p1=11
00910   p2=pos(srep$(ln$,'^','~'),'~',p1+1)
00920   p3=pos(srep$(ln$,'^','~'),'~',p2+1)
00930   p4=len(rtrm$(ln$))
...
00990   p1=pos(a$(j3,3)," ",1)+1
01000   p2=pos(a$(j3,3),".",1)+1
01010   p3=len(rtrm$(a$(j3,3)))
01020   p4=pos(a$(j3,3),"*",1)
01030   if p4=0 then m1=1 else m1=val(a$(j3,3)(1:p4-1))
01040   l=int(val(a$(j3,3)(p1:p3))) ! FIELD STORAGE LENGTH
01050   if p2>1 then dp=val(a$(j3,3)(p2:p3)) else dp=0 ! DECIMAL POSITIONS
01060   if uprc$(a$(j3,3)(1:p1-2))="PD" then al=l*2-1 else al=l !   ACTUAL FIELD LENGTH
01070   l=l*m1 ! TOTAL STORAGE LENGTH
01080   b=a+l
01090   a=a+1
01100   ino=ino+1
01110   a(j3,1)=ino
...
01160   a(j3,6)=b
01170   a=b
01180   rl=rl+int(val(a$(j3,3)(p1:p3)))*m1
01190 L1190: goto L550
01200 L1200: pgo=ceil(j3/24)
01210   if pp=1 then goto L1370
01220   gosub HDR
01230   for j=1 to j3
01240     p1=pos(a$(j,3)," ",1)
01250     p2=len(a$(j,3))
01260     l=val(a$(j,3)(p1:p2))
...
01400 L1400: pr f "24,1,C 7,UT,N": "Done..."
01410   stop 
