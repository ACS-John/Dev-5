def fn_setup
	if ~setup then
		...
		library 'S:\Core\Library': fnmsgbox,fnEditFile,fnSystemName$,fnlog
...
def fn_FileSaveAs(save_what$; fsa_automatedSaveFileName$*256,suppressErrorLog,disableCopyToLocal)
	...
	if clientServer and ~disableCopyToLocal then
		...
		fnmakesurepathexists('C:\ProgramData\ACS\Temp\'&env$('client')&'\Automated Saves\'&save_filename$&save_ext$)
		fnRename(serverTempSaveFile$,'C:\ProgramData\ACS\Temp\'&env$('client')&'\Automated Saves\'&save_filename$&save_ext$)
	else
