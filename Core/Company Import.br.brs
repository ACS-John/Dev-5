...
library 'S:\Core\Library': fntop,fnxit, fnerror,fnTos,fnLbl,fnTxt,fnCmdSet,fnAcs
library 'S:\Core\Library': fncreg_read,fncreg_write,fnmsgbox
library 'S:\Core\Library': fnputcno,fncursys$,fncheckfileversion
library 'S:\Core\Library': fngethandle,fnStatus,fnStatusPause
library 'S:\Core\Library': fnindex_sys
library 'S:\Core\Library': fnFree,fnCopy
! pr
library 'S:\Core\Library': fnpr_conversion_add_missing,fnpr_conversion_department
! ub
library 'S:\Core\Library': fnLastBillingDate
library 'S:\Core\Library': fnub_cnv_build_transactions
library 'S:\Core\Library': fnub_cnv_note
...
if ~exists(env$('at')&company_file$) then 
  ...
  if ~import_only then let fn_is_converison_detection
  ! pause
  fn_copy_files_in
  ! exe 'dir [Q]\ubmstr\ubmaster.h700' : pause
  if ~import_only then let fn_rename_xxcoinfo_to_company
...
def fn_deleteExistingFiles
  ...
  fnFree('[Q]\[CurSys]mstr\*.h[cno]')
	if cursys$='UB' then
		...
		fnFree('[Q]\[CurSys]mstr\ubData\*.h[cno]')
	end if
...
def fn_copy_files_in
  ...
  if cursys$='UB' then let fn_ub_copy_extras
