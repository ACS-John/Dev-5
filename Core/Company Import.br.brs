...
00024   library 'S:\Core\Library': fnputcno,fncursys$,fncheckfileversion,fngethandle,fnstatus,fnstatus_pause,fnchain,fnchk,fnindex_sys
...
20080     let fnCopy(company_import_path$&'*'&company_import_extension$,env$('Q')&'\'&cursys$&'mstr\*.h'&str$(destination_company_number))
...
30060       let fnCopy(company_import_path$&'ubdata\*'&company_import_extension$,env$('Q')&'\UBmstr\ubdata\*.h'&str$(destination_company_number))
...
30100       let fnCopy(company_import_path$&'..\acsub\ratemst'&company_import_extension$,env$('Q')&'\UBmstr\ubdata\RateMst.h'&str$(destination_company_number))
30110       let fnCopy(company_import_path$&'..\acsub\RateIdx1'&company_import_extension$,env$('Q')&'\UBmstr\ubdata\RateIdx1.h'&str$(destination_company_number))
30120       let fnCopy(company_import_path$&'..\acsub\RateIdx2'&company_import_extension$,env$('Q')&'\UBmstr\ubdata\RateIdx2.h'&str$(destination_company_number))
30130     else 
...
32080 !  pause
32100       if exists(import_install_path$&'S:\acsUB\ratemst'&company_import_extension$) then 
32120         let fnCopy(import_install_path$&'S:\acsUB\ratemst'&company_import_extension$,env$('Q')&'\'&cursys$&'mstr\ubdata\*.h'&str$(destination_company_number))
32140       end if 
32160     end if 
32180 ! /r
...
32360       let fnCopy(env$('Q')&'\UBmstr\UBMaster.h'&str$(destination_company_number),env$('Q')&'\UBmstr\Customer.h'&str$(destination_company_number))
...
41020     let fnstatus('Translating Rate abbreviations from '&from$&' to '&to$)
41040     let from$=uprc$(from$)
41060     let to$=uprc$(to$)
...
50002     let fnstatus('UBMaster Conversion for French Settlement processing...')
...
52002 !         let fnstatus('UBMaster Conversion for Merriam Woods processing...')
52020 !         open #h_old:=fngethandle: "Name="&file_source$,internal,input
52040 !     F_U2CMW_UBMASTER_OLD: form pos 1,c 10,4*c 30,c 12,7*pd 2,11*pd 4.2,4*pd 4,15*pd 5,pd 4.2,pd 4,12*pd 4.2,2*pd 3,c 7,2*c 12,pd 3,10*pd 5.2,78*pd 5,13*pd 4.2,13*n 6,156*pd 4.2,13*n 6,13*pd 4.2,c 1,c 9,c 2,c 17
52060 !         open #h_new:=fngethandle: "Name="&file_destination$&',RecL=2067,Replace',internal,outin,relative
52120 !         dim rw4(22,13),gb(10),adr(2),f$(3)*12,g(12),a(7),b(11),c(4),d(15),e$(4)*30,extra(23),extra$(11)*30
52140 !     F_U2CMW_CUSTOMER_NEW: form pos 1,c 10,4*c 30,c 12,7*pd 2,11*pd 4.2,4*pd 4,15*pd 5,pd 4.2,pd 4,12*pd 4.2,2*pd 3,c 7,2*c 12,pd 3,10*pd 5.2,78*pd 5,13*pd 4.2,13*n 6,156*pd 4.2,13*n 6,13*pd 4.2,c 1,c 9,c 2,c 17,n 2,n 7,2*n 6,n 9,pd 5.2,n 3,3*n 9,3*n 2,3*n 3,n 1,3*n 9,3*pd 5.2,c 30,7*c 12,3*c 30
52160 !         do
52180 !           read #h_old,using F_U2CMW_UBMASTER_OLD: z$,mat e$,f$(1),mat a,mat b,mat c,mat d,bal,f,mat g,mat adr,alp$,f$(2),f$(3),bra,mat gb,mat rw4,df$,dr$,dc$,da$ eof U2CMW_EO_UBMASTER
52200 !     !          !     if trim$(z$)(1:6)='100875' then pause
52202 !     !                let extra(17)=c(3) ! in new system   extra(17) = final billing code
52204 !     !                let c(3)=0 ! in new system   c(3) service 3 deposit date
52206 !     !          !
52207 !     !                let extra(1)=val(z$(1:2))
52208 !     !                let extra(2)=val(f$(2)) ! in new system    extra(2) is the sequence number
52210 !     !                let f$(2)='' ! in new system    f$(2) = service 3 meter number
52212 !     !                let extra$(2)=f$(1) !   extra$(2)  is the phone number
52214 !     !                let f$(1)='' ! f$(1)  is the service 1 meter number
52216 !     !          !
52220 !           write #h_new,using F_U2CMW_CUSTOMER_NEW: z$,mat e$,f$(1),mat a,mat b,mat c,mat d,bal,f,mat g,mat adr,alp$,f$(2),f$(3),bra,mat gb,mat rw4,df$,dr$,dc$,da$,mat extra,mat extra$
52240 !         loop
52260 !     U2CMW_EO_UBMASTER: !
52280 !         close #h_old:
52300 !         close #h_new:
52320 !         let fnub_index_customer
52322     let fnstatus('UBMaster Conversion for Merriam Woods complete.')
52340   fnend 
