...
! /r
  ...
  if enableLine then ! r:
    ...
    if enableText then let fnpa_txt("50mm Square Box",51,95)
    !
    ...
    !
    if enableText then let fnpa_txt("30mm Horizontal Line",105,105)
    ...
    !
    if enableText then let fnpa_txt("30mm Vertical Line",205,105)
  ...
  if enableRulerV then let fn_pa_rulerv
  if enableRulerH then let fn_pa_rulerh
