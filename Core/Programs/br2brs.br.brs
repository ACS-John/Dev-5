...
00100   if dir$(len(dir$):len(dir$))="\" then dir$=dir$(1:len(dir$)-1)
...
00250     tmpa$=uprc$(filename$(j)(len(filename$(j))-3:len(filename$(j)))) !:
...
00260     tmpa$=uprc$(filename$(j)(len(filename$(j))-2:len(filename$(j)))) !:
          if tmpa$=uprc$(".br") then goto VALID_FILE_TEST_PASS
