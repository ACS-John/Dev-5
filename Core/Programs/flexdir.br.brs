...
00160   let fngetdir(dur$,mat brfn$," /s ",filter$)
00170   for j=1 to udim(brfn$)
00180     if brfn$(j)="" then mat brfn$(j) : goto OUT_THE_LOOP else !:
            let fnremove2(dur$,brfn$(j))
00190     let fngetpp(brfn$(j),path$(1,j),prog$(1,j),ext$(1,j))
00200   next j
00210 OUT_THE_LOOP: ! 
00220   let fntos(sn$="flexdir") !:
        let mylen=20 : let mypos=mylen+2
00230   let fnlbl(1,1,"Filter:",mylen,1)
00240   let fntxt(1,mypos,18) !:
        let resp$(1)=filter$
...
00320     let fnflexadd1(mat item$)
00330   next j
00340   let fncmdset(102)
