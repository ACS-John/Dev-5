...
def library fn1099print(vn$*8,nam$*30,mat empAddr$,ss$*11,mat box)
	...
	if ~ten99initialized then ! r: initialize output destination (if necessary)
		...
		if env$('CurSys')='PR' then
			open #hCompany:=fngethandle: "Name=[Q]\PRmstr\company.h[cno],Shr", internal,input,relative
			...
			open #hCompany:=fngethandle: "Name=[Q]\CLmstr\Company.h[cno],Shr",internal,input,relative
			read #hCompany,using ' Form POS 1,3*C 40,C 12': mat a$,fed$
			close #hCompany:
		end if
		ten99initialized=1
		...
		if ten99Export$='True' then
			...
			output_filename$=srep$(output_filename$,'[taxyear]',taxYear$)
			output_filename$=srep$(output_filename$,'[TAXYEAR]',taxYear$)
			open #hExport:=fngethandle: "Name="&br_filename$(output_filename$)&",REPLACE",display,output ioerr ASK_INFO
		else
