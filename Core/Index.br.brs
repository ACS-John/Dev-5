...
def fn_index(data_file$*256,index_statement_or_file$*512; indexParameters$*256,___,isIndexStatement,fail)
	...
	if exists(data_file$) then
		if isIndexStatement then
			fnStatus(srep$(index_statement_or_file$,env$('Q'),''))
			...
			! fnStatus('indexing: '&os_filename$(index_statement_or_file$))
		...
		!!! does not work	! /r
			...
			! srep$(env$('curSysData'),env$('Q'),'[Q]')
			dim statusText$*512
			statusText$=indexStatement$
			statusText$(1:6)=srep$(statusText$(1:6),'index ','')
			statusText$=srep$(statusText$,'Replace DupKeys Shr','')
			statusText$=srep$(statusText$,   ' '   ,'    ')
			! statusText$=srep$(statusText$,   srep$(env$('curSysData'),env$('Q'),'[Q]')&'\'   ,env$('curSystem')&' ')
			statusText$=srep$(statusText$,   srep$(env$('curSysData'),env$('Q'),'[Q]')&'\'   ,' ')
			statusText$=srep$(statusText$,'.h'&env$('cno'),' (company '&env$('cno')&') ')
			fnStatus('inglés: '&statusText$)       ! pr 'indexStatement$='&indexStatement$&' sreped='&fnsrepenv$(indexStatement$,'[Q]') : pause
...
fnend
def library fnIndexSys(; only_cno,system_id$*128)
	fn_setup
	fnIndexSys=fn_indexSys( only_cno,system_id$)
fnend
def fn_indexSys(; only_cno,system_id$*128)
...
fnend
	def fn_indexSysDoOne(cno,system_id$*128)
		...
		! /r
			...
			fn_index('[Q]\UBmstr\UBAdrBil.h'&str$(cno), '[Q]\UBmstr\NoteIdx1.h'&str$(cno),'1 10')
		...
		! /r
			...
			! r: rpwork trash - finally removed 2/16/2024, not used since 2019
