...
def fn_acsSystemInitialize(; isScreenIOtest)
  if ~isScreenIOtest or env$('acsVersion')='' then
    ...
    !  end if
    if env$('temp')(2:2)=':' then
      execute 'CD '&env$('temp')(1:2)
      execute 'CD '&env$('temp')(3:len(env$('temp')))
    ...
    if env$('ACSDeveloper')<>'' then 
      ...
      fn_writeProc(''   ,"exec 'sy ""C:\ACS\Util\Lexi\ConvStoO longPathAssist.cmd"" '&env$('source')&''")
      ! fn_writeProc(''   ,"if pos(env$('source'),' ')<=0 then exec 'sy ""C:\ACS\Util\Lexi\ConvStoO v2.cmd"" '&env$('source')&''")
      ! fn_writeProc(''   ,"if pos(env$('source'),' ')>0  then exec 'sy ""C:\ACS\Util\Lexi\ConvStoO v2.cmd"" ""'&env$('source')&'""'")
      fn_writeProc(''   ,'execute ''load "''&program$&''"''')
    end if
