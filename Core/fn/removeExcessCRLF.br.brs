...
def fn_replaceLFwithSpace$*256(csvFile$*256; ___,line$*1024,outFile$*300, _
	...
	open #hIn:=fnGetHandle:  'name='&csvFile$&',RecL=500,EoL=None',external,input
	open #hOut:=fnGetHandle:  'name='&outFile$&',RecL=2048,EoL=None,Replace',external,output
	do
...
def fn_chunkOpen(filename$*80,&vrec,&chunk,&rpos,&file_size,&trec,&head)
	...
	open #hsuper:=fngethandle: "name="&trim$(filename$)&",recl="&str$(chunk)&",shr",external,input,relative ioerr L4744 ! outin
	fn_chunkOpen=hsuper
	goto L4746
	L4744: !
		fn_chunkOpen=-err
