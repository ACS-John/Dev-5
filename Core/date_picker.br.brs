...
00700 ! 
00800     format$=lwrc$(format$)
00900     date$("*mdcy")
01000     if val(_date$)=0 then 
01100       if format$='' then 
01200         _date$=date$: format$="mdcy"
01300       else 
01400         date$("*"&format$)
01500         _date$=date$
01600         _date$=date$(days(val(_date$),format$),'mdcy')
01700         date$("*mdcy")
01800       end if 
01900     else 
02000       if (lwrc$(_date$(1:1))='m' or lwrc$(_date$(1:1))='d') and len(format$)=8 then 
02100         _date$=lpad$(_date$,8,"0")
02200       end if 
02300       if format$='' then format$="mdcy"
02400       _date$=date$(days(val(_date$),format$),'mdcy')
02500     end if 
02600 ! 
...
03600     month=val(_date$(1:2)) : year=val(_date$(5:8))
03700     fngridform(mat headers$,mat widths,mat forms$,days_in_week)
03800     gridspec$="2,1,grid "&str$(rows_on_grid+1)&"/27"
...
04800 ! 
04900       _date$= lpad$(str$(month),2,'0')&day$&str$(year)
05000     loop until fkey=93 or fkey=99 or (fkey=201 and trim$(day$)<>'')
05100     if fkey=93 or fkey=99 then _date$=save_date$
05200     close #window: 
05300     if not format$='mdcy' and not (fkey=93 or fkey=99) then 
05400       _date$=date$(days(val(_date$),'mdcy'),format$)
05500     end if 
05600     fkey(-1) ! reset fkey, so the calling program doesn't start reacting unexpectedly
...
10900       widths(index_)=3
11000       forms$(index_)="CC 2,/#000066:#FFFFFF"
11100     next index_
11200     headers$(1) ="Sun"
11300     headers$(2) ="Mon"
11400     headers$(3) ="Tue"
