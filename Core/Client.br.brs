...
def fn_client_support_setup(client_id,mat css_system_id$,mat css_system_support_end_date,mat css_on_support; css_days_grace)
	...
	if css_setup<>client_id then ! r:
		...
		open #h_support:=fngethandle: "Name=S:\Core\Data\acsllc\support.h420,Version=2,KFName=S:\Core\Data\acsllc\support-idx.h420,version=0,Shr",internal,input,keyed 
...
def fn_client_is_converting
	...
	! 	cic_return=1
	else if env$('client')='Kathys Bookkeeping' and days(date$)<=days('11/15/2019','mm/dd/ccyy')  then
		cic_return=1
	end if 
