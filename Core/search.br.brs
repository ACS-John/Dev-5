...
def library fnsearch(unused$,file_num,&heading$,&form$,numeric_format$,&selection$,key_length)
	...
	Screen2Ask: !
		...
		for j=1 to 20
			...
			if prtall=1 then goto L420
			if len(rtrm$(na1$))=0 then goto L420
			if fs_info$(1)(1:len(rtrm$(na1$)))>na1$(1:len(rtrm$(na1$))) then goto L490
			L420: !
			cde=cde+1
			...
			bk1=bk1+1
			if kps(file_num)=1 then fs_bk$(bk1)=fs_info$(1) else fs_bk$(bk1)=fs_info$(2) ! when backup a screen, use the numeric key if no alpha key being used for the search (eg G/L) (assuming numeric key starts in position 1)
		L480: !
		next j
	L490: !
		if cde=0 then goto L620
		mat fs_sw$(cde)
		for j=1 to 20 
