...
00280     path_source$=resp$(1)(1:pos(resp$(1),"\",-1)-1) !:
          ...
          path_dest$=resp$(3)(1:pos(resp$(3),"\",-1)-1) !:
          cno_dest=val(resp$(4)) !:
          sys_source$=fncursys$ !:
          sys_dest$=fncursys$
00290 ! If EXISTS(PATH_DEST$&"\"&SYS_DEST$&"mstr\Company.h"&STR$(CNO_DEST)) Then !:
          ! Mat ML$(2) !:
          ! mL$(1)="Please pick a Company Number that is not being used." !:
          ! mL$(2)="Company Number "&STR$(CNO_DEST)&" is currently being used." !:
          ! fnMSGBOX(MAT ML$,RESP$,"Error",0) !:
          ! Goto ASK_PATHS
