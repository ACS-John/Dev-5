def library fnFixWordingOnGrid(ev$*50,outputfile$*50)
	...
	ln$=srep$(ln$,chr$(9),'')
	if uprc$(ln$(7:10))<>"DATA" then goto ReadEv
	DATALN: !
	j3=1
	p1=11
	...
	if uprc$(a$(j3,3)(1:1))="X" then goto ReadEv ! skip any formats of "x"
...
MoveItToText: ! r:
	dim miitLine$*87
	open #hMiitIn :=fnGetHandle: "Name="&env$('Temp')&"\Temp."&session$&",KFName="&env$('Temp')&"\TempIdx."&session$&",RecL=87,KPs=1,KLn=30,use",internal,outIn,keyed 
	open #hMiitOut:=fnGetHandle: "Name="&outputfile$&",RecL=87,Replace",display,output 
