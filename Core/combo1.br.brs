...
00180 ! wIN_WIDTH=0 obsoluete
00190     filnum=val(file_number_or_name$) conv OPEN20
...
00300     for j=1 to udim(mat starting_pos) !:
            frm$=frm$&"Pos "&str$(starting_pos(j)) !:
            frm$=frm$&",C "&str$(field_len(j))
00310       if j<>udim(mat starting_pos) then frm$=frm$&","
00320     next j
00330     return 
...
00400       win_width+=field_len(j)+1
00410 ! If J<>1 AND J<>UDIM(LABEL$) Then wIN_WIDTH+=1
00420     next j
00430     return 
...
00650     win_height=udim(choice$)+1
00660     win_width=32 ! reset to min. size to fit buttons !:
          for l=1 to win_height-1 !:
            win_width=max(win_width,len(choice$(l))) !:
          next l
00670     for j=1 to udim(fio1$) !:
            fio1$(j)=str$(j+1)&",1,C "&str$(win_width)&",N" !:
          next j
...
00870     if star_pos=0 then filter_pass=1
00880     return 
