...
def fn_specialFolderPath$*256(folderName$*64; ___,line$*256,lineCount,return$*256)
	exe 'sy -M reg query "HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Explorer\Shell Folders" /v "'&folderName$&'" >tmp'&session$&'.txt'
	open #hTmp:=fngethandle: 'name=tmp'&session$&'.txt',d,input
	do
