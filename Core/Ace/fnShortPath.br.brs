...
def library fnshortpath$*256(longpath$*256)
	...
	if trim$(longpath$)='' then 
		...
		open #tmp:=fngethandle: 'Name='&csTempPath$&'\sp_'&session$&'.txt',display,input 
...
def fn_make_shortpath_cmd(msc_path$*256)
	if ~exists(msc_path$&'\ShortPath.cmd') or do_debug then 
		open #msc_tmp:=fngethandle: 'name='&msc_path$&'\ShortPath.cmd,RecL=256,Replace',display,output 
		pr #msc_tmp: "echo off"
