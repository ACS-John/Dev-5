...
00320     let fntos(sn$="labellib1")
00330     let fnlbl(1,1,"Label Format:",15,1)
00340     let opt$(1)="Avery 8160 (30/Page Ink Jet)" !:
          ...
          let fncomboa("labellib1."&wsid$,1,17,mat opt$,"Choose either dot-matrix or laser type labels",32) !:
          let resp$(1)=opt$(1)
00350     let fncmdset(2)
00360     let fnacs(sn$,win,mat resp$,ckey)
...
00420     let fntos(sn$="labellib2")
00430     let fnfra(1,2,10,28,"Select First Label to Print","If you only have a partial sheet of labels, you can select the starting place of the first label") !:
          let myline=1: let mypos=1
00440     for j=1 to 30
00450       let fnbutton(myline,mypos,"  "&cnvrt$("pic(zz)",j)&"  ",j+20,"Starts with label "&str$(j),0,6,1)
00460       if j/3=int(j/3) then let myline+=1
00470       let mypos+=10
00480       if mypos>30 then let mypos=1
00490     next j
00500     let fncmdset(1)
00510     let fnacs(sn$,win,mat resp$,ckey)
...
00572     let fnreg_read('top_marg',top_marg$) : let top_marg=val(top_marg$) conv ignore
00573     let fnreg_read('label_pos1',label_pos1$) : let label_pos1=val(label_pos1$) conv ignore
00574     let fnreg_read('label_pos2',label_pos2$) : let label_pos2=val(label_pos2$) conv ignore
00575     let fnreg_read('label_pos3',label_pos3$) : let label_pos3=val(label_pos3$) conv ignore
...
00630     let fnlbl(3,1,"Center Column Position:",24,1)
00640     let fntxt(3,26,3,3,1,'20',0,"Increase or decrease the position to move the center label right or left") !:
          let resp$(3)=str$(label_pos2)
00650     let fnlbl(4,1,"Right Column Position:",24,1)
00660     let fntxt(4,26,3,3,1,'20',0,"Increase or decrease the position to move the right label right or left") !:
          let resp$(4)=str$(label_pos3)
00670     let fncmdset(2)
00680     let fnacs(sn$,win,mat resp$,ckey)
...
00694       let fnreg_write('top_marg',str$(top_marg))
00696       let fnreg_write('label_pos1',str$(label_pos1))
00698       let fnreg_write('label_pos2',str$(label_pos2))
00700       let fnreg_write('label_pos3',str$(label_pos3))
00702     end if 
00708     return  ! /r
00720 LABEL_DONE: ! r:
...
01230 !     If UPRC$(LINESTYLE$(Z))="BAR" AND wabel$(X,Y,Z)<>"" Then !:
                ...
                !     Let FNBARCODE(wabel$(X,Y,Z),LABEL_POS(Y))
...
01640 L1: ! 
01650     let fnopenprn
01660     let fnwait(win,cap$,"Printing: Please wait...",1)
