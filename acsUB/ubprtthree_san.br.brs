...
00040   library 'S:\Core\Library': fnAcs,fnLbl,fnTxt,fnwait,fncmbrt2,fncombof,fnChk,fnerror,fnOpt,fnTos,fncmbact,fnget_services,fnLastBillingDate,fnxit,fnCmdSet,fntop,fnformnumb$,fnpause,fnopenprn,fncloseprn
...
00230   z=26 !:
        ...
        next j
00240   linelength=62
00250   open #1: "Name="&env$('Q')&"\UBmstr\Customer.h"&env$('cno')&",KFName="&env$('Q')&"\UBmstr\ubIndex.h"&env$('cno')&",Shr",internal,input,keyed  ! open in Account order
00260   open #2: "Name="&env$('Q')&"\UBmstr\Customer.h"&env$('cno')&",KFName="&env$('Q')&"\UBmstr\ubIndx5.h"&env$('cno')&",Shr",internal,input,keyed  ! open in route-sequence #
00270   open #81: "Name="&env$('Q')&"\UBmstr\BudMstr.h"&env$('cno')&",KFName="&env$('Q')&"\UBmstr\BudIdx1.h"&env$('cno')&",Shr",internal,outIn,keyed 
...
00340   fnTos(sn$="UBPrtBl1-1") !:
        pf=26 : ll=24 !:
        respc=0
00350 ! fnLbl(1,1,"Service From:",LL,1)
00360 ! fnTxt(1,PF,8,8,1,"1",0,TT$) !:
        ! rESP$(RESPC+=1)=CNVRT$("pic(zzzzzz)",D2)
00370 ! fnLbl(2,1,"Service To:",LL,1)
00380 ! fnTxt(2,PF,8,8,1,"1") !:
...
00740   if d2=0 and d2x>0 then d2=d2x ! set date to screen if no date in record
00750   if d3=0 and d3x>0 then d3=d3x ! set date to screen if no date in record
...
01320   write #9,using L1310: "FILE customer.H"&env$('cno')&","&env$('Q')&"\UBmstr,,"&env$('Temp')&"\Addr."&session$&",,,,,A,N"
...
01660   if pb<>0 then pb$="   PRIOR BALANCE" else pb$=""
01670   pr #255,using L1680: pb$,pb
01680 L1680: form pos 3,c 17,nz 10.2,skip 1
01690   if g(1)=0 then t$="" else t$=newservice$(1)
...
01730   if g(2)=0 then t$="" else t$=newservice$(2)
01740   if bal<=0 then !:
          pr #255,using L1780: t$,0,0,0,g(2),"Balance",bal : goto L1790 ! sangamon
01750   if cno=1 and penalty>0 then penalty=bal*.20 ! company #1 20 %
01760   if cno=2 and penalty>0 then penalty=bal*.10 ! company #2 @ 10 %
...
01790 L1790: if g(3)=0 then t$="" else t$=newservice$(3)
01800   if bal<=0 then payafter=bal else payafter=bal+penalty
01810   pr #255,using L1780: t$,0,0,0,g(3),"Pay after: "& cnvrt$("pic(zz/zz/zz)",d4),payafter ! sangamon
01820   if g(9)=0 then t$="" else t$=newservice$(9)
01830   pr #255,using L1710: t$,0,0,0,g(9)
01840   if g(4)=0 then t$="" else t$=newservice$(4)
01850   pr #255,using L1710: t$,0,0,0,g(4)
01860   if g(6)=0 then t$="" else t$=newservice$(6)
...
01900   if g(8)=0 and g(7)=0 then t$="" else t$=newservice$(8)
