...
def library fnub_cnv_note
	...
	if exists("[Q]\UBmstr\Note1.h[cno]") then
		...
		open #h_note1=fnH: "Name=[Q]\UBmstr\Note1.h[cno],KFName=[Q]\UBmstr\NoteIdx1.h[cno],Shr",internal,input,keyed 
		...
		open #h_note2=fnH: "Name=[Q]\UBmstr\Note2.h[cno],Shr",internal,outIn,relative
		if kln(h_note2)<73 then
			close #h_note2:
			fnCopy("[Q]\UBmstr\Note2.h[cno]","[Q]\UBmstr\Note2.h[cno]", 73)
			open #h_note2=fnH: "Name=[Q]\UBmstr\Note2.h[cno],Shr,Use,RecL=73",internal,outIn,relative 
		end if
