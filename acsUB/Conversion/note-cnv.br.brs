...
def library fnub_cnv_note
	...
	if exists("[Q]\UBmstr\Note1.h[cno]") then
		...
		open #h_note1=fnH: "Name=[Q]\UBmstr\Note1.h[cno],KFName=[Q]\UBmstr\NoteIdx1.h[cno],Shr",i,i,k
		...
		open #h_note2=fnH: "Name=[Q]\UBmstr\Note2.h[cno],Shr",i,outi,r
		if kln(h_note2)<73 then
			close #h_note2:
			fnCopy("[Q]\UBmstr\Note2.h[cno]","[Q]\UBmstr\Note2.h[cno]", 73)
			open #h_note2=fnH: "Name=[Q]\UBmstr\Note2.h[cno],Shr,Use,RecL=73",i,outi,r
		end if
		...
		do
			...
			if adr then
				do
					...
					open #hNote33=fnH: "Name=[Q]\UBmstr\notes.h[cno]\"&trim$(z$)&".txt,RecL=128,use",d,o  ! XXX changed from ,Replace to ,Use on 5/4/2018
					pr #hNote33: rm$
					close #hNote33:
					delete #h_note2:
				loop until adr<=0
