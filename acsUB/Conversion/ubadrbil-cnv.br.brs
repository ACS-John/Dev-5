...
00060     let fnstatus('Converting Alternate Billing Address file (S:\acsUB\conversion\ubadrbil-cnv)')
...
00132       let fnCopy(env$('Q')&"\UBmstr\UBAdrBil.h"&str$(cno),env$('temp')&"\temp."&session$)
00134     end if 
...
00280 END1: close #1: 
00290     close #2: 
00300     let fnCopy(env$('temp')&"\tmp_x_"&session$,env$('Q')&"\UBmstr\UBAdrBil.h"&str$(cno))
...
00370     close #2: 
00380     let fnindex_it(env$('Q')&"\UBmstr\UBAdrBil.h"&str$(cno),env$('Q')&"\UBmstr\adrIndex.h"&str$(cno),"1 10") ! execute "Index "&env$('Q')&"\UBmstr\UBAdrBil.h"&str$(cno)&' '&env$('Q')&"\UBmstr\adrIndex.h"&str$(cno)&" 1,10,Replace,DupKeys -n"
00390     return 
00410 XIT: fnend 
