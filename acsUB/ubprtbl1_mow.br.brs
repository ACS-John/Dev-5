...
00040   library 'S:\Core\Library': fnAcs,fnLbl,fnTxt,fncmbrt2,fncombof,fnChk,fnerror,fnTos,fncmbact,fncno,fnLastBillingDate,fnxit,fnCmdSet,fntop,fnformnumb$,fnpa_txt,fnpa_finis,fnpa_open,fnpa_newpage
...
00830   open #3: "Name="&env$('Q')&"\UBmstr\UBAdrBil.H"&env$('cno')&",KFName="&env$('Q')&"\UBmstr\adrIndex.H"&env$('cno')&",Shr",internal,input,keyed 
...
00875   if st1$=z$ then st1=0 else goto L550
00877 READALTADR: ! 
...
00955     txt$="Last Account entered was "&z$
00957     fnLbl(3,1,txt$,44,1)
00959   else 
00961     txt$=""
00963     fnLbl(3,1,txt$,44,1)
00965   end if 
00967   fncmbact(1,17) ! 
00969   resp$(1)=a$
00971   fnCmdSet(3): fnAcs(sn$,0,mat resp$,ck)
...
00987 !     open #5: "Name="&env$('Q')&"\UBmstr\Cass1.h"&env$('cno')&",KFName="&env$('Q')&"\UBmstr\Cass1Idx.h"&env$('cno')&",Shr",internal,input,keyed ioerr L1370
00989 !     open #6: "Name="&env$('Temp')&"\Temp."&session$&",Replace,RecL=19",internal,output
00991 ! !   s5=1
00993 !     if prtbkno=0 then routekey$="" else routekey$=cnvrt$("N 2",prtbkno)&"       " ! key off first record in route (route # no longer part of customer #)
