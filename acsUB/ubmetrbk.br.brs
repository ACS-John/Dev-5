! Replace S:\acsUB\ubMetrBk
! pr Meter Reading Routes

autoLibrary
on error goto Ertn

dim z$*10,e$(4)*30,x$*10,f$(1)*12,f$(3)*12,cap$*128,dat$*20,service$(1)*2,bulksort$*12
...
MENU1: ! r:
	...
	fncmbrt2(lc,mypos)
	resp$(1)=fn_reg_try$(sn$&'.route',"1")
	fnLbl(lc+=1,1,"Service Type:",mylen,1)
	fncomboa("ubrate3",lc,mypos,mat option$)
	resp$(2)=fn_reg_try$(sn$&'.service type',option$(1))
	lc+=1
	fnChk(lc+=1,mylen+2,"Print Footnotes:",1)
	resp$(3)=fn_reg_try$(sn$&'.print footnotes',"False")
	fnChk(lc+=1,mylen+2,"Double Space:",1)
	resp$(4)=fn_reg_try$(sn$&'.double space',"False")
	fnChk(lc+=1,mylen+2,"Blank Used Column:",1)
	resp$(5)=fn_reg_try$(sn$&'.blank used column',"False")
	fnChk(lc+=1,mylen+2,"Show Disconnects:",1)
	resp$(6)=fn_reg_try$(sn$&'.show disconnects',"False")
	fnChk(lc+=1,mylen+2,"Print Prior Usage:",1)
	resp$(7)=fn_reg_try$(sn$&'.print prior usage',"False")
	fnChk(lc+=1,mylen+2,"Skip Meter Number:",1)
	resp$(8)=fn_reg_try$(sn$&'.skip meter number',"False")
	lc+=1
	...
	resp$(9)=fn_reg_try$(sn$&'.# of blank lines',str$(extralines))
	lc+=1
	...
	resp$(10)=fn_reg_try$(sn$&'.route number sequence',"True")
	...
	if resp$(3)(1:1)="T" then remark=1 ! yes want remarks printed
	if resp$(4)(1:1)="T" then double=1 ! yes want to double space
	if resp$(5)(1:1)="T" then usedcolumn=1 ! yes want a gallons used column
	if resp$(6)(1:1)="T" then skipdisconnects=1 ! yes want show disconnects
	if resp$(7)(1:1)="T" then printprior=1 ! pr prior months usage
	if resp$(8)(1:1)="T" then skipmeter=1 ! don't pr meter number
	extralines=val(resp$(9))
	if resp$(10)="True" then seq=1
	if resp$(11)="True" then seq=2
	fnreg_write(sn$&'.route',resp$(1))
	fnreg_write(sn$&'.service type',resp$(2))
	fnreg_write(sn$&'.print footnotes',resp$(3))
	fnreg_write(sn$&'.double space',resp$(4))
	fnreg_write(sn$&'.blank used column',resp$(5))
	fnreg_write(sn$&'.show disconnects',resp$(6))
	fnreg_write(sn$&'.print prior usage',resp$(7))
	fnreg_write(sn$&'.skip meter number',resp$(8))
	fnreg_write(sn$&'.# of blank lines',resp$(9))
	fnreg_write(sn$&'.route number sequence',resp$(10))
	fnreg_write(sn$&'.account sequence',resp$(11))
! /r
if seq=2 then 
...
LOOP_TOP: ! r: main loop
	read #h_customer,using 'Form POS 1,C 10,4*C 30,C 12,POS 217,15*PD 5,POS 373,C 12,POS 361,C 12,POS 1741,N 2,POS 1821,N 1,pos 143,7*pd 2,pos 1806,3*n 2,pos 1821,n 1,pos 1942,c 12': z$,mat e$,f$(1),mat d,f$(3),f$(2),extra(1),fbc,mat a,extra(11),extra(12),extra(13),extra(17),bulksort$ eof DONE
	...
	for j=1 to udim(x_service$)
		...
		if printprior=1 then ul$="  {\ul          }"
		if printprior=1 and resp$(2)="[All]" and option$(j+1)(1:2)="WA" then 
			prior=d(3)
			x_service$(x+=1)=cnvrt$("pic(zzzzzzzzz)",d(1))&ul$&cnvrt$("pic(zzzzzzzzz)",prior)
		end if 
		if printprior=0 and resp$(2)="[All]" and option$(j+1)(1:2)="WA" then 
			x_service$(x+=1)=cnvrt$("pic(zzzzzzzzz)",d(1))&ul$
		end if 
		if printprior=1 and resp$(2)(1:2)="WA" then 
			prior=d(3)
			x_service$(x+=1)=cnvrt$("pic(zzzzzzzzz)",d(1))&ul$&cnvrt$("pic(zzzzzzzzz)",prior)
			mat x_service$(1)
			goto L940
		end if 
		if printprior=1 and resp$(2)="[All]" and option$(j+1)(1:2)="EL" then 
			prior=d(7)
			x_service$(x+=1)=cnvrt$("pic(zzzzzzzzz)",d(5))&ul$&cnvrt$("pic(zzzzzzzzz)",prior)
		end if 
		if printprior=0 and resp$(2)="[All]" and option$(j+1)(1:2)="EL" then 
			prior=d(7)
			x_service$(x+=1)=cnvrt$("pic(zzzzzzzzz)",d(5))&ul$
		end if 
		if printprior=1 and resp$(2)(1:2)="EL" then 
			prior=d(7)
			x_service$(x+=1)=cnvrt$("pic(zzzzzzzzz)",d(5))&ul$&cnvrt$("pic(zzzzzzzzz)",prior)
			mat x_service$(1)
			goto L940
		end if 
		if printprior=1 and resp$(2)="[All]" and option$(j+1)(1:2)="GA" then 
			prior=d(11)
			x_service$(x+=1)=cnvrt$("pic(zzzzzzzzz)",d(9))&ul$&cnvrt$("pic(zzzzzzzzz)",prior)
		end if 
		if printprior=0 and resp$(2)="[All]" and option$(j+1)(1:2)="GA" then 
			prior = d(11)
			x_service$(x+=1)=cnvrt$("pic(zzzzzzzzz)",d(9))&ul$
		end if 
		if printprior=1 and resp$(2)(1:2)="GA" and option$(j+1)(1:2)="GA" then 
			prior=d(11)
			x_service$(x+=1)=cnvrt$("pic(zzzzzzzzz)",d(9))&ul$&cnvrt$("pic(zzzzzzzzz)",prior)
			mat x_service$(1)
			goto L940
		end if 
		if printprior=0 and resp$(2)(1:2)="GA" and option$(j+1)(1:2)="GA" then 
			prior=d(11)
...
def fn_reg_try$*256(field_name$*128,default_value$*256)
	dim rt_return$*256
	fnreg_read(field_name$,rt_return$)
	if rt_return$='' then rt_return$=default_value$
...
def fn_campbell_meter_book ! Campbell Special Routine (printed once a year, one customer per page)
	dim z$*10,e$(4)*30,x$*10,f$(1)*12,f$(3)*12,message$*40,cap$*40,g(12),d(15)
	cap$="Print Meter Reading Books"
	fnTos(sn$='umbc')
	mylen=22 : mypos=mylen+2 : respc=lc=0
	fnLbl(lc+=1,1,"Route Number:",mylen,1)
	fncmbrt2(lc,mypos)
		resp$(1)=fn_reg_try$(sn$&'.route',"[All]")
	! fnLbl(lc+=1,1,"Service Type:",mylen,1)
	...
	fnCmdSet(3)
	fnAcs2(mat resp$,ckey)
	if ck=5 then goto CAMPBELL_XIT
	if uprc$(resp$(1))=uprc$("[All]") then route=0 else route=val(resp$(1))
	service$="W"
	fnreg_write(sn$&'.route',resp$(1))
	...
	CAMPBELL_LOOP_TOP: ! 
		dim g(12)
		! read #h_customer,using F_CAMPBELL_CUSTOMER: z$,mat e$,f$(1),watcode,elecode,litecode,mat d,f$(3),c4,f$(2), mat g eof CAMPBELL_DONE
		...
		read #h_customer,using 'Form POS 1,C 10,4*C 30,C 12,POS 217,15*PD 5,POS 373,C 12,POS 361,C 12,POS 1741,N 2,POS 1821,N 1,pos 143,7*pd 2,pos 1806,3*n 2,pos 1821,n 1,pos 1942,c 12,pos 300,12*pd 4.2': z$,mat e$,f$(1),mat d,f$(3),f$(2),extra(1),fbc,mat a,extra(11),extra(12),extra(13),extra(17),bulksort$,mat g eof DONE
		watcode=a(1)
