...
25140   open #1: "Name="&env$('Q')&"\UBmstr\Customer.h"&str$(cno)&",KFName="&env$('Q')&"\UBmstr\ubIndex.h"&str$(cno)&",Shr",internal,input,keyed 
25150 F_CUSTOMER: form pos 1,c 10,pos 41,3*c 30,pos 213,pd 4,pos 296,pd 4,pos 385,pd 3,pos 1854,pd 5.2
25160   open #h_adrbil:=3: "Name="&env$('Q')&"\UBmstr\ubAdrBil.h"&str$(cno)&",KFName="&env$('Q')&"\UBmstr\AdrIndex.h"&str$(cno)&",Shr",internal,outin,keyed 
25180 F_ADRBIL: form pos 11,4*c 30
26000   print #h_out,using F_OUT_FIXED: 'Account','Name','Street Address','City','State','Zip Code'
28000 ! r:  main loop
28020   do 
...
28120     if trim$(csz$)="" then let csz$=sta2$ : let sta2$=""
28140 L300: ! 
28160     let csz$=trim$(csz$)
28180     if csz$(len(csz$)-5:len(csz$)-5) =" " then goto L330
28200     if csz$(len(csz$)-5:len(csz$)-5) < "0" or csz$(len(csz$)-5:len(csz$)-5)>"9" then 
28220       let csz$=csz$(1:len(csz$)-5) &" "&csz$((len(csz$)-4):len(csz$))
28240     end if 
28260 L330: ! 
28280     let fncsz(csz$,city$,state$,zip$)
28300     let zip$=trim$(zip$)(1:5)
28320 L350: ! 
28340     print #h_out,using F_OUT_FIXED: z$,nam$,sta$,city$,state$,zip$,chr$(10)
28360   loop 
28380 ! /r
29000 XIT: let fnxit
