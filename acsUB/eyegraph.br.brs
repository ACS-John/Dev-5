00020 ! print bar graph of eye improvement
...
00810     let cd1(j)=val(resp$(j)) conv MSGBOX
...
00970   if resp$(25)="Water" then let codepos=143: let service=1: let opt=1
00980   if resp$(25)=trim$(opt$(2)) then let codepos=147: let service=3: let opt=2
00990   if resp$(25)=trim$(opt$(3)) then let codepos=149 : let service=4 : let opt=3
01000   if resp$(26)="True" then let baseon=1 else let baseon =2 ! 1=usage  2=dollars
01010   for j=1 to 24
01020     let actualdate$(j)=resp$(j)
01030   next j
...
01485   let cnam$="Ken Johnson"
01490   let cnam=(len(trim$(cnam$))/2)+120
01500   print #20: 'Call Print.MyFontsize(14)'
01510   let txt$=cnam$ !:
        print #20: 'Call Print.AddText("'&txt$&'",'&str$(cnam)&','&str$(10)&')'
01520   print #20: 'Call Print.MyFontsize(12)'
01530   let txt$="Eye Chart"
01540   let servicetype=(len(trim$(txt$))/2)+120
01550   print #20: 'Call Print.AddText("'&txt$&'",'&str$(servicetype)&','&str$(16)&')'
01560   if baseon=1 then let txt$=""
01570   if baseon=2 then let txt$=trim$(opt$(opt))
01580   let servicetype=(len(trim$(txt$))/2)+140
01590   print #20: 'Call Print.AddText("'&txt$&'",'&str$(servicetype)&','&str$(20)&')'
01600   print #20: 'Call Print.MyFontsize(9)'
01610   let txt$=cnvrt$("pic(--------)",top) !:
        print #20: 'Call Print.AddText('&txt$&','&str$(1)&','&str$(lyne+=spacing)&')'
01620   print #20: 'Call Print.AddLine('&str$(15)&','&str$(lyne)&',240,0)' ! left,up/down,lenght of top line on chart
01630   for j=1 to 10 ! wording down side
01640     let txt$=cnvrt$("pic(-------#)",top-((.10*j)*top)) !:
          print #20: 'Call Print.AddText('&txt$&','&str$(1)&','&str$(lyne+=spacing)&')'
01650     print #20: 'Call Print.AddLine('&str$(15)&','&str$(lyne)&',240,0)'
01660   next j
01670 ! zero line starts right here
01680   let linezero=(spacing*10)+40
01690   let column=18 ! spacing sideways
01700   print #20: 'Call Print.MyFontBold(1)'
01710   for j=1 to 24
01720     let homedot=140-((dollars(j)/top)*100)
01730     if homedot<0 then let homedot=0
01740     if homedot>140 then let homedot=140
01750     print #20: 'Call Print.AddLine('&str$(column-1)&','&str$(homedot)&','&str$(7)&','&str$(linezero-homedot)&',1)'
01760     for q=1 to 6
01770       print #20: 'Call Print.AddLine('&str$(column-1+q)&','&str$(homedot)&','&str$(7-q)&','&str$(linezero-homedot)&',1)'
01780     next q
01790     print #20: 'Call Print.MyFontsize(6)'
01800     if dollars(j)>0 then let txt$=cnvrt$("pic(--------#)",round(dollars(j),0)): print #20: 'Call Print.AddText('&txt$&','&str$(column-2)&','&str$(homedot-2)&')'
01810     print #20: 'Call Print.MyFontsize(9)'
01820     let column+=10
01830   next j
01840   print #20: 'Call Print.MyFontBold(0)'
...
01880     print #20: 'Call Print.AddText("'&txt$&'",'&str$(indent+1)&','&str$(linezero+8)&')'
01890     let txt$=actualdate$(j)(3:4)
01900     print #20: 'Call Print.AddText("'&txt$&'",'&str$(indent+1)&','&str$(linezero+11)&')'
01910   next j
...
03060     print #20: 'Call Print.MyOrientation("Landscape")'
03080     let lyne=margin ! starting of 1st line
03100     let column1=16
03120     let column2=103
03140     let column3=153
03160   end if 
03180   return  ! /r
