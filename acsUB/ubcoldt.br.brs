...
! -- Cash Receipts Journal
	...
	dim r(20,4),hd1$*255,cap$*128,serviceName$(10)*20,tg(11),resp$(7)*40
	dim ml$(3)*90
 
	fnTop("S:\acsUB\UBColPrn",cap$="Cash Receipts Journal")
...
L430: read #2,using 'Form POS 1,C 10,N 8,N 1,12*PD 4.2,6*PD 5,PD 4.2,N 1': p$,tdate,tcode,tamount,mat tg,wr,wu,er,eu,gr,gu,tbal,pcode eof PRTOTALS
	...
	if tcode<3 or tcode>5 then goto L430 : _
	...
	x=0
	for j=1 to 10
		if trim$(serviceName$(j))="" then goto L600
		alloc(x+=1)=tg(j)
		if ti2=3 then r(x+3,1)-=tg(j) else r(x+3,1)+=tg(j)
		r(x+3,ti2+1)+=tg(j)
L600: next j
	c$=" "
	if tcode=4 then c$="CM" else : _
...
SCREEN1: !
	...
	fnTxt(2,mypos,10,0,1,"3",0,"First day of the period to be printed. (ccyymmdd format)") : _
	resp$(2)=str$(ld1)
	fnLbl(3,1,"Ending Date (blank for all):",mylen,1)
	fnTxt(3,mypos,10,0,1,"3",0,"Last day of the period to be printed. (ccyymmdd format)") : _
	resp$(3)=str$(hd1)
