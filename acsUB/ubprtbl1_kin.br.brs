...
00020 ! print bills for Village of Kincaid  (4 part plain paper)
00030 ! ______________________________________________________________________
00040   library 'S:\Core\Library': fnacs,fnlbl,fntxt,fnwait,fncmbrt2,fncombof,fnchk,fnerror,fnopt,fntos,fncmbact,fncno,fnd1,fnxit,fncmdset,fntop,fnformnumb$,fnpause,fnpa_finis,fnpa_newpage,fnpa_open
...
00242   let mg$(1)='Temp Return Service Requested'
...
00900 L900: let fnpa_finis
00930   goto ENDSCR
00940 ! ______________________________________________________________________
00950 L950: ! 
00960   let pb=bal-g(11)
00970   if bal<=0 then let g(10)=0 ! don't show penalty if balance 0 or less
00980 ! ______________print bill routine______________________________________
00990   gosub VBPRINT
01000 ! _____________end of print routine______________________________________
01010   let bct(2)=bct(2)+1 !:
        ! accumulate totals
01020   goto L550
01030 ! ______________________________________________________________________
01040 SCREEN3: ! 
01050   let sn$ = "UBPrtBl1-2" !:
        let fntos(sn$)
01060   let txt$="Account (blank to stop)" !:
...
01090   let fncmbact(1,17) ! !:
        let resp$(1)=a$
01100   let fncmdset(3): let fnacs(sn$,0,mat resp$,ck)
01110   let a$ = lpad$(trim$(resp$(1)(1:10)),10) !:
...
01190   let s5=1
01200   let routekey$="" ! If PRTBKNO=0 Then Let ROUTEKEY$="" Else !:
...
01440 ENDSCR: ! print totals screen
01450   if sum(bct)=0 then let pct=0 else let pct=bct(2)/sum(bct)*100
01460   let fntos(sn$="Bills-Total") !:
        let mylen=23 : let mypos=mylen+2 !:
        let respc=0
01470   let fnlbl(1,1,"Total Bills Printed:",mylen,1)
01480   let fntxt(1,mypos,8,0,1,"",1) !:
...
01510 ! Let FNLBL(3,1,"Total Bills Not Coded:",MYLEN,1)
01520 ! Let FNTXT(3,MYPOS,8,0,1,"",1) !:
        ! Let RESP$(RESPC+=1)=CNVRT$("N 8",BCT(1))
01530 ! Let FNLBL(4,1,"Percent of Bills Coded:",MYLEN,1)
01540 ! Let FNTXT(4,MYPOS,8,0,1,"",1) !:
        ! Let RESP$(RESPC+=1)=CNVRT$("N 8.2",PCT)
01550   let fncmdset(52) !:
        let fnacs(sn$,0,mat resp$,ck)
01560 XIT: let fnxit
01570 ! ______________________________________________________________________
01580 ERTN: let fnerror(program$,err,line,act$,"xit")
01590   if uprc$(act$)<>"PAUSE" then goto L1620
01600   execute "list -"&str$(line) !:
        pause  !:
        goto L1620
01610   print "PROGRAM PAUSE: Type GO and press [Enter] to continue." : print "" : pause 
...
01770   let checkcounter+=1
01780   if checkcounter=1 then let xmargin=0 : let ymargin=0
01790   if checkcounter=2 then let xmargin=139 : let ymargin=0
01800   if checkcounter=3 then let xmargin=0 : let ymargin=108
01810   if checkcounter=4 then let xmargin=139 : let ymargin=108 !:
          let checkcounter=0
01820 ! ______________________________________________________________________
01830   print #20: 'Call Print.AddLine('&str$(xmargin+5)&','&str$(ymargin+2)&',57,'&str$(lyne*3+3)&',True)'
01840   print #20: "Call Print.MyFontBold(True)"
01850   print #20: 'Call Print.MyFontSize(12)'
01860   print #20: 'Call Print.MyFont("Courier New")'
01870   print #20: 'Call Print.AddText("'&at$(1)&'",'&str$(xmargin+8)&','&str$(lyne*1-1+ymargin)&')'
01880   print #20: 'Call Print.MyFont("Lucida Console")'
01890   print #20: 'Call Print.MyFontSize(10)'
01900   print #20: 'Call Print.MyFontBold(False)'
01910   print #20: 'Call Print.AddText("'&at$(2)&'",'&str$(xmargin+6)&','&str$(lyne*2+1+ymargin-.2)&')'
01920   print #20: 'Call Print.AddText("'&at$(3)&'",'&str$(xmargin+6)&','&str$(lyne*3+1+ymargin)&')'
01930   print #20: 'Call Print.AddText("#'&trim$(z$)&'  '&bulk$&'",'&str$(xmargin+4)&','&str$(lyne*5+ymargin)&')'
01940   print #20: 'Call Print.AddText("'&e$(1)&'",'&str$(xmargin+4)&','&str$(lyne*6+ymargin)&')'
01950   print #20: 'Call Print.AddText("From: '&cnvrt$("PIC(ZZ/ZZ/ZZ)",d2)&'  To: '&cnvrt$("PIC(ZZ/ZZ/ZZ)",d3)&'",'&str$(xmargin+2)&','&str$(lyne*7+1+ymargin)&')'
01960   print #20: 'Call Print.AddText("                       ",'&str$(xmargin+2)&','&str$(lyne*8+ymargin)&')'
01970   print #20: 'Call Print.AddText("Billing Date: '&cnvrt$("PIC(ZZ/ZZ/ZZ)",d1)&'",'&str$(xmargin+2)&','&str$(lyne*11+ymargin)&')'
01980   print #20: 'Call Print.AddLine('&str$(xmargin+1)&','&str$(lyne*12+1+ymargin)&',62,0)'
01990   print #20: 'Call Print.AddText("Reading",'&str$(xmargin+10)&','&str$(lyne*13+ymargin)&')'
02000   print #20: 'Call Print.AddText("Usage",'&str$(xmargin+33)&','&str$(lyne*13+ymargin)&')'
02010   print #20: 'Call Print.AddText("Charge",'&str$(xmargin+50)&','&str$(lyne*13+ymargin)&')'
02020 ! ______________________________________________________________________
02030 PRINTGRID: let meter=14 !:
...
02060 L2060: if g(3)=0 and d(7)=0 then goto L2080 else !:
          print #20: 'Call Print.AddText("LM",'&str$(xmargin+1)&','&str$(lyne*(meter+=1)+ymargin)&')' !:
...
02070 !  Print #20: 'Call Print.AddText("'&FNFORMNUMB$(G(3),2,9)&'",'&STR$(XMARGIN+45)&','&STR$(LYNE*METER+YMARGIN)&')'
02080 L2080: if a4=1 then let gcode$="RSGS" else !:
...
02100 L2100: if g(5)=0 then goto L2110 else !:
          print #20: 'Call Print.AddText("WSur",'&str$(xmargin+1)&','&str$(lyne*(meter+=1)+ymargin)&')' !:
...
02110 L2110: if g(6)=0 then goto L2120 else !:
          print #20: 'Call Print.AddText("SSur",'&str$(xmargin+1)&','&str$(lyne*(meter+=1)+ymargin)&')' !:
...
02120 L2120: if g(7)=0 then goto L2130 else !:
          print #20: 'Call Print.AddText("FUEL ADJ",'&str$(xmargin+1)&','&str$(lyne*(meter+=1)+ymargin)&')' !:
...
02150 L2150: if pb><0 then print #20: 'Call Print.AddLine('&str$(xmargin+46)&','&str$(lyne*(meter+=1)+ymargin)&',15,0)'
02160   if pb><0 then print #20: 'Call Print.AddText("   Subtotal",'&str$(xmargin+1)&','&str$(lyne*(meter+=.25)+ymargin)&')' !:
          print #20: 'Call Print.AddText("'&fnformnumb$(g(1)+g(2)+g(3)+g(4)+g(5)+g(6)+g(7)+g(8)+g(9),2,9)&'",'&str$(xmargin+45)&','&str$(lyne*meter+ymargin)&')'
...
02180 L2180: print #20: 'Call Print.MyFontSize(10)'
02190 ! ______________________________________________________________________
02200   if estimatedate=d1 then print #20: 'Call Print.AddText("Bill estimated!",'&str$(xmargin+1)&','&str$(lyne*20+ymargin)&')'
02210   print #20: 'Call Print.AddLine('&str$(xmargin+1)&','&str$(lyne*22+1+ymargin)&',63,0)'
02220   print #20: 'Call Print.AddText("Pay By '&cnvrt$("PIC(ZZ/ZZ/ZZ)",d4)&':",'&str$(xmargin+1)&','&str$(lyne*23+ymargin)&')'
02230   print #20: 'Call Print.AddText("'&fnformnumb$(bal,2,9)&'",'&str$(xmargin+42)&','&str$(lyne*23+ymargin)&')'
02240   print #20: 'Call Print.AddText("Pay After '&cnvrt$("PIC(ZZ/ZZ/ZZ)",d4)&':",'&str$(xmargin+1)&','&str$(lyne*24+ymargin)&')'
02250   print #20: 'Call Print.AddText("'&fnformnumb$(bal+g(10),2,9)&'",'&str$(xmargin+42)&','&str$(lyne*24+ymargin)&')'
02260   print #20: 'Call Print.AddLine('&str$(xmargin+1)&','&str$(lyne*25+1+ymargin)&',63,0)'
02270   print #20: 'Call Print.AddText("Phone: 217-237-4713",'&str$(xmargin+1)&','&str$(lyne*26+ymargin)&')'
02271   print #20: "Call Print.MyFontBold(True)"
02272   print #20: 'Call Print.AddText("Shut off notices will",'&str$(xmargin+8)&','&str$(lyne*27+ymargin)&')'
02273   print #20: 'Call Print.AddText("no longer be mailed.",'&str$(xmargin+10)&','&str$(lyne*28+ymargin)&')'
02274   print #20: 'Call Print.AddText("Last Tuesday of the month",'&str$(xmargin+8)&','&str$(lyne*29+ymargin)&')'
02275   print #20: 'Call Print.AddText("at 9 AM water will be",'&str$(xmargin+8)&','&str$(lyne*30+ymargin)&')'
02276   print #20: 'Call Print.AddText("disconnected if unpaid.",'&str$(xmargin+8)&','&str$(lyne*31+ymargin)&')'
02277   print #20: 'Call Print.MyFontBold(False)'
02280 ! ______________________________________________________________________
02290   let special=28
02300 ! ______________________________________________________________________
02310   print #20: 'Call Print.MyFontSize(7)'
02320   print #20: 'Call Print.AddLine('&str$(xmargin+97)&','&str$(ymargin+0)&',29,'&str$(lyne*5+2)&',TRUE)'
02330   print #20: 'Call Print.AddLine('&str$(xmargin+90)&','&str$(ymargin+0)&',7,0)'
02340   print #20: 'Call Print.AddLine('&str$(xmargin+90)&','&str$(ymargin+2.8)&',7,0)'
02350   print #20: 'Call Print.AddLine('&str$(xmargin+90)&','&str$(ymargin+5.6)&',7,0)'
02360   print #20: 'Call Print.AddLine('&str$(xmargin+90)&','&str$(ymargin+8.4)&',7,0)'
02370   print #20: 'Call Print.AddLine('&str$(xmargin+90)&','&str$(ymargin+11.2)&',7,0)'
02380   print #20: 'Call Print.AddLine('&str$(xmargin+90)&','&str$(ymargin+14)&',7,0)'
02390   print #20: 'Call Print.AddLine('&str$(xmargin+90)&','&str$(ymargin+17)&',7,0)'
02400   print #20: 'Call Print.AddText("   Pre-Sorted",'&str$(xmargin+100)&','&str$(lyne*1-1+ymargin)&')'
02410   print #20: 'Call Print.AddText("First Class Mail",'&str$(xmargin+100)&','&str$(lyne*2-1+ymargin)&')'
02420   print #20: 'Call Print.AddText("  U.S. Postage  ",'&str$(xmargin+100)&','&str$(lyne*3-1+ymargin)&')'
02430   print #20: 'Call Print.AddText("      Paid",'&str$(xmargin+100)&','&str$(lyne*4-1+ymargin)&')'
02440   print #20: 'Call Print.AddText("  Permit No 13",'&str$(xmargin+100)&','&str$(lyne*5-1+ymargin)&')'
02450   print #20: 'Call Print.MyFontSize(9)'
02460 ! Print #20: 'Call Print.AddText("Address Service Requested",'&STR$(XMARGIN+68)&','&STR$(LYNE*7+YMARGIN-6)&')'
02470   print #20: 'Call Print.AddText("Please return this",'&str$(xmargin+68)&','&str$(lyne*7+ymargin)&')'
02480   print #20: 'Call Print.AddText("side with payment to:",'&str$(xmargin+68)&','&str$(lyne*8+ymargin)&')'
02490   print #20: 'Call Print.AddText("'&cnam$&'",'&str$(xmargin+68)&','&str$(lyne*9+ymargin)&')'
02500   print #20: 'Call Print.MyFontSize(10)'
02510   print #20: 'Call Print.AddText("Pay By '&cnvrt$("PIC(ZZ/ZZ/ZZ)",d4)&':",'&str$(xmargin+68)&','&str$(lyne*11+ymargin)&')'
02520   print #20: 'Call Print.AddText("'&fnformnumb$(bal,2,9)&'",'&str$(xmargin+106)&','&str$(lyne*11+ymargin)&')'
02530   print #20: 'Call Print.AddText("After  '&cnvrt$("PIC(ZZ/ZZ/ZZ)",d4)&':",'&str$(xmargin+68)&','&str$(lyne*12+ymargin)&')'
02540   print #20: 'Call Print.AddText("'&fnformnumb$(bal+g(10),2,9)&'",'&str$(xmargin+106)&','&str$(lyne*12+ymargin)&')'
02550   print #20: 'Call Print.MyFontSize(9)'
02560   let addy=14
02570   print #20: 'Call Print.AddText("'&mg$(1)&'",'&str$(xmargin+68)&','&str$((addy+=1)*lyne+ymargin)&')'
02580   print #20: 'Call Print.AddText("'&mg$(2)&'",'&str$(xmargin+68)&','&str$((addy+=1)*lyne+ymargin)&')'
02590   print #20: 'Call Print.AddText("'&mg$(3)&'",'&str$(xmargin+68)&','&str$((addy+=1)*lyne+ymargin)&')'
02600   let addy+=1
02610   print #20: 'Call Print.MyFontSize(10)'
02620   if df$="Y" then !:
...
02640   print #20: 'Call Print.AddText("#'&trim$(z$)&' '&bulk$&'",'&str$(xmargin+68)&','&str$(lyne*(addy+=1)+ymargin)&')'
02650   if pe$(1)<>"" then !:
          print #20: 'Call Print.AddText("'&trim$(pe$(1))&'",'&str$(xmargin+68)&','&str$(lyne*(addy+=2)+ymargin)&')'
02660   if pe$(2)<>"" then !:
...
02690   if checkcounter=1 then let checkx=1.375 : let checky=3.6875
02700   if checkcounter=2 then let checkx=6.75 : let checky=3.6875
02710   if checkcounter=3 then let checkx=1.375 : let checky=7.9375
02720   if checkcounter=0 then let checkx=6.75 : let checky=7.9375
02730 ! let bc$=""
02740 ! if trim$(bc$)<>"" then print #20: 'Call Print.DisplayBarCode('&str$(checkx)&','&str$(checky)&',"'&bc$&'")'
