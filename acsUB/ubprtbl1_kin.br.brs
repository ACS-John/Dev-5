...
L1340: form pos 1,c 128
	write #9,using L1340: "File "&env$('Temp')&"\Temp."&wsid$&",,,"&env$('Temp')&"\Addr."&session$&",,,,,A,N"
	write #9,using L1340: "Mask 1,19,C,A"
	close #9:
	execute "Free "&env$('Temp')&"\Addr."&session$&" -n" ioerr L1390
L1390: execute "Sort "&env$('Temp')&"\Control."&session$&" -n"
...
ERTN: fnerror(program$,err,line,act$,"Xit")
	if uprc$(act$)<>"PAUSE" then goto L1620
...
L2860: execute "Index "&env$('Temp')&"\Temp."&wsid$&" "&env$('Temp')&"\TempIdx."&session$&" 1,19,Replace,DupKeys -n" ioerr L2880
