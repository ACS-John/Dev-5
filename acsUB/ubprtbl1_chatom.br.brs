...
00040   library 'S:\Core\Library': fnacs,fnlbl,fntxt,fnwait,fncmbrt2,fncombof,fnchk,fnerror,fnopt,fntos,fncmbact,fncno,fnLastBillingDate,fnxit,fncmdset,fntop,fnformnumb$,fnpause,fnpa_txt,fnpa_finis,fnpa_open,fnpa_newpage
...
00310   fn_bulksort
00320   open #1: "Name="&env$('Q')&"\UBmstr\Customer.h"&env$('cno')&",KFName="&env$('Q')&"\UBmstr\ubIndex.H"&env$('cno')&",Shr",internal,input,keyed  ! open in Account order
00330   open #2: "Name="&env$('Q')&"\UBmstr\Customer.h"&env$('cno')&",KFName="&env$('Q')&"\UBmstr\ubIndx5.H"&env$('cno')&",Shr",internal,input,keyed  ! open in route-sequence #
...
00550   datafile$=env$('Q')&"\UBmstr\Customer.h"&env$('cno')
00560   indexfile$=env$('Q')&"\UBmstr\ubindx5.h"&env$('cno')
...
00830   open #3: "Name="&env$('Q')&"\UBmstr\UBAdrBil.H"&env$('cno')&",KFName="&env$('Q')&"\UBmstr\adrIndex.H"&env$('cno')&",Shr",internal,input,keyed ioerr FAIL_OPEN_3
00835 FAIL_OPEN_3: ! 
...
00880   if st1$=z$ then st1=0 else goto L550
00882 READALTADR: ! 
...
00890   e1=0 : mat pe$=("")
00892   for j=1 to 4
00894     if rtrm$(ba$(j))<>"" then e1=e1+1 : pe$(e1)=ba$(j)
00896   next j
00898   goto L900
00900 ! ______________________________________________________________________
00902 L750: e1=0 : mat pe$=("")
00904   for j=2 to 4
00906     if rtrm$(e$(j))<>"" then e1=e1+1 : pe$(e1)=e$(j)
00908   next j
00910   if trim$(extra1$)<>"" then pe$(4)=pe$(3): pe$(3)=extra1$ ! set third address line to extra1$ (2nd address)
00912   goto L900
...
00922 L850: fnpa_finis
00924   goto ENDSCR
00926 ! ______________________________________________________________________
00928 L900: ! 
00930   pb=bal-g(11)
00932   if bal<=0 then g(10)=0 ! don't show penalty if balance 0 or less
...
00972   fncmbact(1,17) ! 
00974   resp$(1)=a$
00976   fncmdset(3): fnacs(sn$,0,mat resp$,ck)
...
00992   open #5: "Name="&env$('Q')&"\UBmstr\Cass1.h"&env$('cno')&",KFName="&env$('Q')&"\UBmstr\Cass1Idx.h"&env$('cno')&",Shr",internal,input,keyed ioerr L1370
00994   open #6: "Name="&env$('Temp')&"\Temp."&wsid$&",Replace,RecL=19",internal,output 
00996   s5=1
00998   if prtbkno=0 then routekey$="" else routekey$=cnvrt$("N 2",prtbkno)&"       " ! key off first record in route (route # no longer part of customer #)
...
01510 ! rESP$(RESPC+=1)=CNVRT$("N 8",BCT(2))
01520 ! fnLBL(3,1,"Total Bills Not Coded:",MYLEN,1)
01530 ! fnTXT(3,MYPOS,8,0,1,"",1)
01540 ! rESP$(RESPC+=1)=CNVRT$("N 8",BCT(1))
01550 ! fnLBL(4,1,"Percent of Bills Coded:",MYLEN,1)
01560 ! fnTXT(4,MYPOS,8,0,1,"",1)
01570 ! rESP$(RESPC+=1)=CNVRT$("N 8.2",PCT)
01580   fncmdset(52)
...
01690     if checkcounter=2 then xmargin=140 : ymargin=0
01695     if checkcounter=3 then xmargin=0 : ymargin=108
01700     if checkcounter=4 then xmargin=140 : ymargin=108 : checkcounter=0
...
01770 ! fnpa_txt("From: "&cnvrt$("PIC(ZZ/ZZ/ZZ)",d2)&'  To: '&cnvrt$("PIC(ZZ/ZZ/ZZ)",d3),xmargin+2,lyne*7+ymargin)
...
03090     open #1: "Name="&env$('Q')&"\UBmstr\Customer.h"&env$('cno')&",KFName="&env$('Q')&"\UBmstr\ubIndex.h"&env$('cno')&",Shr",internal,input,keyed  ! open in Account order
