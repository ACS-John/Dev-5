...
00100   let fntop(program$,cap$="Type of Services (All Companies)")
00110   let fntos(sn$:="TypeOServices")
00120   let fnlbl(1,1,sn$&'1',20,2,3)
00130   let colhdr$(1)='CNo'
00140   for j=1 to 10
00150     let colhdr$(j+01)='Name '&str$(j)
00160     let colhdr$(j+11)='Code '&str$(j)
00170     let colhdr$(j+21)='Tax  '&str$(j)
00180     let colhdr$(j+31)='Pen  '&str$(j)
00190     let colhdr$(j+41)='Sub  '&str$(j)
00200     let colhdr$(j+51)='Ord  '&str$(j)
00210   next j
00220   mat colmask$(60)
00230   mat colmask$=("")
00240   let fnflexinit1(filename$:='toses',2,1,10,72,mat colhdr$,mat colmask$,1)
00241   dim data_dir$*256
00242   let data_dir$=env$('Q')&"\UBmstr\ubData\"
00243   let fngetdir2(data_dir$,mat service_file$, '/ON','Service.h*')
00244 ! fnpause
00250   for service_file_item=1 to udim(service_file$)
00270     let item$(1)=service_file$(service_file_item)(10:len(service_file$(service_file_item)))
...
00300     let fnflexadd1(mat item$)
00310     close #h_service: 
00330   next service_file_item
00340   let fncmdset(4)
00350   let fnacs(sn$,win,mat resp$,ckey)
00360   let fnxit
