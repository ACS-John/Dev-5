...
L380: form pos 1,c 10,pos 41,c 30,pos 227,pd 5,pos 296,pd 4
	fnopenprn
! sort prep !!!
	open #6: "Name="&env$('temp')&"\Work."&session$&",REPLACE,RecL=50",internal,output
...
L520: form pos 1,c 128
	write #9,using L520: "FILE "&env$('temp')&"\Work."&session$&",,,"&env$('Temp')&"\Addr."&session$&",,,,,A,N"
	write #9,using L520: "MASK 1,10,N,A,11,10,C,A"
	close #9:
	execute "FREE "&env$('Temp')&"\Addr."&session$&" -n" ioerr L570
L570: execute "Sort "&env$('Temp')&"\Control."&session$&" -n"
