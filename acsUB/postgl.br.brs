...
SCREEN2: !
	...
	for j=1 to 10
		...
		fnqgl25(j+2,12,fram1,2,pas)
		resp$(respc+=1)=fnrgl$(gln$(x+=1))
		fnqgl25(j+2,42,fram1,2,pas)
		resp$(respc+=1)=fnrgl$(gln$(x+=1))
		fnqgl25(j+2,72,fram1,2,pas)
		resp$(respc+=1)=fnrgl$(gln$(x+=1))
	next j
...
CREATE_ENTRIES: !
	for j=1 to 10
		...
		read #15,using "form pos 24,3*c 12",key=service$(j)&cnvrt$("pic(zz#)",ratecode): mat gl$ nokey L1300
...
L1300: next j
	x=0
	for j=1 to 10
L1330: x+=1 : if x=4 then x=0 : goto L1370
		if amount(j,x)=0 then goto L1360
...
L1370: next j
	...
	for j=1 to 10
L1410: x+=1: if x>3 then x=0 : goto L1460
		if amount(j,x)=0 then goto L1450
		if amount(j,x)>0 then pr #255,using "form pos 1,c 12,x 7,n 13.2": gln$(j,x),amount(j,x) else pr #255,using "form pos 1,c 12,x 20,n 13.2": gln$(j,x),amount(j,x) pageoflow PAGE_OVER_FLOW
		if amount(j,x)>0 then totaldebits+=amount(j,x) else totalcredits+=amount(j,x)
L1450: goto L1410
L1460: next j
