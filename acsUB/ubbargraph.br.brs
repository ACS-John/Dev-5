...
00040   library 'S:\Core\Library': fntop,fnxit, fnsearch,fnerror,fnget_services, fnTos,fnLbl,fnCmdSet,fnAcs,fnTxt, fnqgl,fnagl$,fnOpt,fnFra,fncomboa,fndat,fnmsgbox,fndate_mmddyy_to_ccyymmdd,fnpa_finis
...
00130   dim n2(24),n3(24,13),cap$*128,resp$(27),txt$*80
...
00730   for j=1 to 24
00740 L740: x=pos(resp$(j),"/",1) !:
          if x>0 then resp$(j)(x:x)="": goto L740
00750     cd1(j)=val(resp$(j)) conv MSGBOX
00760     y=val(resp$(j)(5:6))
00770     if y=1 then month$(j)="Jan"
00780     if y=2 then month$(j)="Feb"
00790     if y=3 then month$(j)="Mar"
00800     if y=4 then month$(j)="Apr"
00810     if y=5 then month$(j)="May"
00820     if y=6 then month$(j)="Jun"
00830     if y=7 then month$(j)="Jul"
00840     if y=8 then month$(j)="Aug"
00850     if y=9 then month$(j)="Sep"
00860     if y=10 then month$(j)="Oct"
00870     if y=11 then month$(j)="Nov"
00880     if y=12 then month$(j)="Dec"
00890   next j
...
01060   if service=1 and baseon=1 then usage=wu ! analyzing water
01070   if service=1 and baseon=2 then usage=tg(1) ! analyzing water dollars
01080   if service=3 and baseon=1 then usage=eu ! analyzing electric
01090   if service=3 and baseon=2 then usage=tg(3) ! analyzing electric dollars
01100   if service=4 and baseon=1 then usage=gu ! analyzing gas
01110   if service=4 and baseon=2 then usage=tg(4) ! analyzing gas dollars
01120   for j=1 to 24
...
01180   for j=1 to 24
01190     dollars(j)=u1(j)
01200   next j
01210 PRINT_CHART: ! 
01220   gosub VBOPENPRINT
01230 ! determine maximum height and depth
01240   for j=1 to 24
01250     if dollars(j)>0 then maximumheight=max(dollars(j),maximumheight) ! largest dollars by month for either year  (dollars is negative figure
01260   next j
01270 ! determine top line and bottom line
01280   if baseon=1 then top$=str$(maximumheight): toplen=len(top$): top=toplen*10
01290   if baseon=2 then top$=str$(round(maximumheight,0)): toplen=len(top$): top=toplen*10
01300   toplen$=str$(val(top$(1:1))+1)
01310   for j=1 to toplen-1
01320     toplen$=toplen$&str$(0)
01330   next j
01340   top=val(toplen$)
01350   x=top*.10
01360 DETERMINE_BOTTOM_LINE: ! 
01370   spacing=10 : lyne=30
01380   cnam=(len(trim$(env$('cnam')))/2)+110
01390   pr #20: 'Call Print.MyFontsize(14)'
01400   txt$=env$('cnam') !:
        pr #20: 'Call Print.AddText("'&txt$&'",'&str$(cnam)&','&str$(10)&')'
01410   pr #20: 'Call Print.MyFontsize(12)'
01420   if baseon=1 then txt$="Usage By Month"
01430   if baseon=2 then txt$="Dollars By Month"
01440   servicetype=(len(trim$(txt$))/2)+120
01450   pr #20: 'Call Print.AddText("'&txt$&'",'&str$(servicetype)&','&str$(16)&')'
01460   if baseon=1 then txt$=trim$(opt$(opt))
01470   if baseon=2 then txt$=trim$(opt$(opt))
...
01520 ! tXT$=TRIM$(MONTH$(J))(1:3) !:
        ! iNDENT=8+(10*J) !:
        ! pr #20: 'Call Print.AddText("'&TXT$&'",'&STR$(INDENT)&','&STR$(25)&')'
01530 ! Next J
01540   txt$=cnvrt$("pic(--------)",top) !:
...
01570     txt$=cnvrt$("pic(-------#)",top-((.10*j)*top)) !:
          pr #20: 'Call Print.AddText('&txt$&','&str$(1)&','&str$(lyne+=spacing)&')'
...
01640   for j=1 to 24
01650     homedot=140-((dollars(j)/top)*100)
01660     if homedot<0 then homedot=0
01670     if homedot>140 then homedot=140
...
01730     if dollars(j)>0 then txt$=cnvrt$("pic(--------#)",round(dollars(j),0)): pr #20: 'Call Print.AddText('&txt$&','&str$(column-2)&','&str$(homedot-2)&')'
