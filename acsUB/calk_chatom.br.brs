...
13200     if x(12)>0 then usage_water=x(12) ! water usage entered
13300     if x(9)><0 then w(1)=x(9) : goto WATER_END ! water charge entered
13400     if b(1)><0 then w(1)=b(1) : goto WATER_END ! standard water charge
...
13900     w(1)=mc1
14000     if usage_water>mu1 then w(1)=w(1)+(usage_water-mu1)*rt(1,3)
14100 WATER_END: g(1)=round(w(1),2)
14200     gosub WATER_USED
14300     return  ! /r
15000 SEWER: ! r: calculate sewer
15020 ! pr x$ : if trim$(x$)='100106.00' then pause
15040     if x(5)><0 then w(2)=x(5) : goto SEWER_COMPLETED
15060     if b(2)><0 then w(2)=b(2) : goto SEWER_COMPLETED
...
15180 !     w(2)=mc1+(usage_water-mu1)*rt(a(2),3)
15200 !   else 
15220 !     w(2)=mc1
15240 !   end if
15260 ! /r
15280 ! r: new chatom sewer calculation - more standard... i hope
15300     usage_sewer=usage_water
15310     w(2)=mc1*max(1,extra(14)) ! units per meter - sewer (default to one)
15320     if usage_sewer<=mu1 then goto L3300 else mu2=mu1
15340     for j=1 to 10
15360       if rt(j,1)>usage_sewer then goto L3300
15380       if usage_sewer<rt(j,2) then w1=usage_sewer-mu2 else w1=rt(j,2)-mu2
15400       w(2)=w(2)+round(w1*rt(j,3),2)
...
16800     if x(6)><0 then w(5)=x(6) : goto SANITATION_END
16900     if b(5)><0 then w(5)=b(5) : goto SANITATION_END
17000     read #ratemst,using FORM_RATEMSTR, key="SN"&lpad$(str$(a(5)),2): mc1,mu1,mat rt nokey SANITATION_END
17100     w(5)=rt(a(5),3)
17200 SANITATION_END: g(5)=w(5)
17300     return  ! /r
17400 FIRE: ! r: CALCULATE FIRE PROTECTION
17500     if x(7)><0 then g(6)=x(7) : goto FIRE_END
17600     if b(6)><0 then g(6)=b(6) : goto FIRE_END
17700 FIRE_END: return  ! /r
17800 MERCHANDISE: ! r:
17900     g(7)=0
18000     return  ! /r
18100 OTHER_CHG: ! r: CALCULATE OTHER CHARGES
18200     if x(8)><0 then g(8)=x(8) : goto OTHER_END
18300     if b(7)><0 then g(8)=b(7) : goto OTHER_END
...
18800     g(9)=round(g(1)*rt(a(6),3),2)
...
19300     g(10)=mc1 ! standard penalty
19400 PENALTY_END: return  ! /r
19500 NET_GROSS: ! r: calculate net & gross bill
19600     for j=1 to 10
19700       if uprc$(penalty$(j))><"Y" then g(11)=g(11)+g(j) ! don't add penalties into net
19800       g(12)=g(12)+g(j)
19900     next j
20000     return  ! /r
