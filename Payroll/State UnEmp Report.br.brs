...
! /r
	...
	open #hDepartment=fnH: 'Name=[Q]\PRmstr\Department.h[cno],Shr, KFName=[Q]\PRmstr\DeptIdx.h[cno],Shr',i,outIn,k
	open #hChecks=fnH: 'Name=[Q]\PRmstr\payrollchecks.h[cno],KFName=[Q]\PRmstr\checkidx.h[cno]',i,outIn,k
	gosub HDR
NextEmployee: !
	read #hEmployee,using 'form pos 1,N 8,3*C 30,C 11': eno,mat em$,ss$ eof DONE
	...
	restore #hChecks,key>=checkkey$: nokey ANALYZE_WAGES
MAIN_LOOP: ! r:
	read #hChecks,using 'form pos 1,N 8,n 3,PD 6,N 7,5*PD 3.2,37*PD 5.2': heno,tdn,prd,ckno,mat tdc,mat tcp eof ANALYZE_WAGES
	...
	read #hDepartment,using Fdepartment,key=deptkey$: sc1 nokey ReadDeptNokey ! no state code (assume this state)
	Fdepartment: form pos 48,n 2
...
! end if
	m1=m1+qtr(31)-dcq
! L1030: !
	if m2=0 then goto NextEmployee
	state_wh=qtr(4)
	gosub SUBTOTALS
	goto NextEmployee
! /r
DONE: ! r:
	...
	L1460: form pos 5,c 15,pos 42,pic(--,---,---.zz),pos 57,pic(--,---,---.zz),pos 70,pic(----,---.zz),skip 1
