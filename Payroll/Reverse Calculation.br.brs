...
dim fullname$(20)*20,abrevname$(20)*8,newcalcode(20)
fnDedNames(mat fullname$,mat abrevname$,mat dedcode,mat newcalcode,mat newdedfed,mat dedfica,mat dedst,mat deduc,mat gl$)
open #hTmp=fnH: 'Name=[Q]\PRmstr\Company.h[cno],Shr',i,i
read #hTmp,using 'form pos 605,C 12,N 1': bankgl$,gli
close #hTmp: : hTmp=0
if gli=1 then
	! gosub PostGL0
	! PostGL0: ! r:
...
open #hTrMstr=6: 'Name=[Q]\CLmstr\TRMSTR.h[cno],KFName=[Q]\CLmstr\TRIDX1.h[cno],Shr',i,outIn,k ioerr L330
FtrMstr: form pos 1,n 2,n 1,c 8,g 6,pd 10.2,c 8,c 35,n 1,n 6,n 1
open #7: 'Name=[Q]\CLmstr\TRMSTR.h[cno],KFName=[Q]\CLmstr\TRIDX2.h[cno],Shr',i,outIn,k
if exists('[Q]\CLmstr\Tralloc-Idx.h[cno]') then
	open #hTrAlloc:=8: 'Name=[Q]\CLmstr\TrAlloc.h[cno],Version=2,KFName=[Q]\CLmstr\TrAlloc-Idx.h[cno],Shr',i,outIn,k
else
	open #hTrAlloc:=8: 'Name=[Q]\CLmstr\TrAlloc.h[cno],Shr',i,outi,r
end if
...
AskEmployeeNo: !
	...
	mat tcp=(0)
	if reverseAll then gosub CreateReverseList

	L730: ! r: main loop
	if reverseAll then read #13,using 'form pos 1,n 8,n 7': eno,w1 eof Finis
...
UPDATE_CHECKBOOK: ! r:
	...
	read #hTrMstr,using FtrMstr,key=k$: bcde,tcde,tr$(1),tr$(2),tr3,tr$(4),tr$(5),pcde,clr,scd nokey L1300
	goto Location0
	L1300: !
	mat ml$(2)
	...
	rewrite #hTrMstr,using FtrMstr,key=k$: bcde,tcde,tr$(1),tr$(2),tr3,tr$(4),tr$(5),pcde,clr,scd
...
PostGL: ! r:
	...
	fnCmdKey('E&xit',5,0,1,'Without Posting')
	ckey=fnAcs(mat resp$)
	if ckey<>5 then
		dat1=val(resp$(1)) ! posting date
		if glb=2 then 
...
PostGL2: ! r:
	...
	read #2,using Ftwo,rec=rec1: teno,mat tgl,dat,mat tcp,nta
	Ftwo: form pos 1,n 8,x 3,n 3,n 6,n 3,pos 42,n 6,pos 358,22*pd 5.2,pos 468,pd 3
	if dat><d1 then goto L2340
	if tgl(2)=0 then goto L2340
	if mastercd=0 then gosub OpenWorkAndDueToFrom
	tgl$=lpad$(str$(tgl(1)),3)&lpad$(str$(tgl(2)),6)&lpad$(str$(tgl(3)),3)
	if tgl(1)=0 or tgl(1)=oldtgl then goto L2130 else gosub OpenWorkAndDueToFrom
...
PostGL3: ! r: Subtotal routine and write GL trans
	...
	subtotal=0
return  ! /r
PrGlDistHeaders: ! r: headers for General Ledger Distribution for Payroll
	p1=p1+1
...
POST_WH_AND_NET: ! r: ASSIGN G/L NUMBERS AND POST TO GL WORK FILE
	for j=1 to 26
		if tx(j)=0 then goto L3030
		if tx(j)<0 then goto L2950
		if j<=4 then goto L2850 else goto L2870
		L2850: !
		pr #255,using L2180: 0,' ',prgl(j,1),prgl(j,2),prgl(j,3),-tx(j) pageoflow PgOf
		goto L2930
		L2870: !
		pr #255,using L2920: 0,' ',gl$(j-4),-tx(j) pageoflow PgOf
		if j<25 then goto L2890 else goto L2910
		L2890: !
		pr #255,using L2920: 0,' ',gl$(j-4),-tx(j) pageoflow PgOf
		goto L2920
		L2910: !
		pr #255,using L2920: 0,' ',gl$(j-11),-tx(j) pageoflow PgOf
		L2920: form pos 1,pic(zzzzzzzz),pos 15,c 30,pos 50,c 12,n 12.2,skip 1
		L2930: !
		totaldr+=tx(j)
		goto L2980
		L2950: !
		pr #255,using L2960: 0,' ',prgl(j,1),prgl(j,2),prgl(j,3),-tx(j) pageoflow PgOf
		...
		if j<=4 then write #3,using L2450: prgl(j,1),prgl(j,2),prgl(j,3),dat,-tx(j),5,0,tr$,td$,prgl$(15)
		if j>4 and j<25 then write #3,using L3010: gl$(j-4),dat,-tx(j),5,0,tr$,td$,prgl$(15)
		L3010: form pos 1,n 3,c 12,pd 6.2,n 2,n 2,c 12,c 52,c 12
		if j>24 then write #3,using L2450: prgl(j-11,1),prgl(j-11,2),prgl(j-11,3),dat,-tx(j),5,0,tr$,td$,prgl$(15)
		L3030: !
	next j
	...
	L3070: !
return  ! /r
OpenWorkAndDueToFrom: ! r: OPEN G/L WORK FILES AND CREATE DUE TO AND DUE FROM ENTRIES
...
CreateReverseList: ! r:
	...
	CreateReverseListRead: !
		read #1,using 'form pos 1,n 8,pos 132,2*PD 4.2',release: eno,em10,em11 eof L4100
		checkkey$=cnvrt$('pic(ZZZZZZZ#)',eno)&'         '
		restore #4,key>=checkkey$: nokey CreateReverseListRead
		L4040: !
		read #4,using 'form pos 1,N 8,n 3,PD 6,N 7,5*PD 3.2,37*PD 5.2': heno,tdn,prd,ckno,mat tdc,mat cp eof CreateReverseListRead
		if heno<>eno then goto CreateReverseListRead
		if prd=d1 then goto L4080
		goto L4040
		L4080: !
		write #13,using 'form pos 1,n 8,n 7': eno,ckno
	goto CreateReverseListRead
	L4100: !
	restore #13:
return  ! /r
include: ertn
