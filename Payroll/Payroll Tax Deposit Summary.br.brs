...
do
	ReadEmployee: !
	dim em$*30
	read #hEmployee,using 'form pos 1,N 8,C 30': eno,em$ eof PrFinalTotals
	...
	do
		dim tdc(10)
		dim cp(32)
		read #hChecks,using 'form pos 1,N 8,n 3,PD 6,N 7,5*PD 3.2,37*PD 5.2': heno,dep,prd,ckno,mat tdc,mat cp eof ReadEmployee
		if eno=heno and prd=>beg_date and prd<=end_date then
			...
			! pr #255: 'employee number: '&str$(heno)&' employee record: '&str$(rec(hEmployee))&' check number: '&str$(ckno)&' check history record number: '&str$(rec(hChecks))
