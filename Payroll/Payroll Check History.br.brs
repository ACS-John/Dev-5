...
hact$=''
fn_checkHistory(hact$,hCheckIdx3,hCheckIdx1,hEmployee)
...
def library fnCheckHistory(hact$*8,hCheckIdx3,hCheckIdx1,hEmployee)
	if ~setup then fn_setup
	fnCheckHistory=fn_checkHistory(hact$,hCheckIdx3,hCheckIdx1,hEmployee)
fnend
def fn_checkHistory(hact$*8,hCheckIdx3,hCheckIdx1,hEmployee; ___,returnN,gridname$*30,oldgridname$*30)
	dim resp$(60)*128
	dim hf(46)
	dim hf$(46)
	ckCancel=5 : ckBack=2 : ckEdit=1 : ckSave=1
	ckAdd=4
	...
	open #hGridName=fnH: 'Name=[Q]\PRmstr\GridNames.h[cno],USE,RecL=30',i,outi,r
	...
	SCREEN2: ! r:
		...
		fnCmdKey('&Back',ckBack,0,0)
		fnCmdKey('&Add',ckAdd,0,0)
		fnCmdKey('&Close',ckCancel,0,1)
		ckey=fnAcs(mat resp$) ! check history building grid
		addcode=0 : holdeno=0
		if ckey=ckBack then goto SCREEN1
		if ckey=ckCancel then goto SCREEN1
	...
	SCREEN3_ADD: !
		...
		resp$(rc+=1)=str$(heno)
		fnLbl(lc+=1,1,'Department:',mylen,1)          	: fnTxt(lc,mypos,3,0,0,'',0) 	: resp$(rc+=1)=str$(tdn)
		fnLbl(lc+=1,1,'Date:',mylen,1)                 	: fnTxt(lc,mypos,10,0,0,'3') 	: resp$(rc+=1)=str$(prd)
		fnLbl(lc+=1,1,'Check #:',mylen,1)             	: fnTxt(lc,mypos,10,0,0,'30')	: resp$(rc+=1)=str$(ckno)
		fnLbl(lc+=1,1,'Regular Hours:',mylen,1)       	: fnTxt(lc,mypos,10,0,0,'32') 	: resp$(rc+=1)=str$(tdc(1))
		fnLbl(lc+=1,1,'Overtime Hours:',mylen,1)      	: fnTxt(lc,mypos,10,0,0,'32') 	: resp$(rc+=1)=str$(tdc(2))
		fnLbl(lc+=1,1,'Sick Hours:',mylen,1)          	: fnTxt(lc,mypos,10,0,0,'32') 	: resp$(rc+=1)=str$(tdc(3))
		fnLbl(lc+=1,1,'Vacation Hours:',mylen,1)      	: fnTxt(lc,mypos,10,0,0,'32') 	: resp$(rc+=1)=str$(tdc(4))
		fnLbl(lc+=1,1,'Holiday Hours:',mylen,1)       	: fnTxt(lc,mypos,10,0,0,'32') 	: resp$(rc+=1)=str$(tdc(5))
		fnLbl(lc+=1,1,'Regular Earnings:',mylen,1)    	: fnTxt(lc,mypos,10,0,0,'10')	: resp$(rc+=1)=str$(tcp(26))
		fnLbl(lc+=1,1,'Overtime Earnings:',mylen,1)   	: fnTxt(lc,mypos,10,0,0,'10')	: resp$(rc+=1)=str$(tcp(27))
		fnLbl(lc+=1,1,'Other Compensation:',mylen,1)  	: fnTxt(lc,mypos,10,0,0,'10')	: resp$(rc+=1)=str$(tcp(28))
		fnLbl(lc+=1,1,'Meals:',mylen,1)                	: fnTxt(lc,mypos,10,0,0,'10')	: resp$(rc+=1)=str$(tcp(29))
		fnLbl(lc+=1,1,'Tips:',mylen,1)                 	: fnTxt(lc,mypos,10,0,0,'10')	: resp$(rc+=1)=str$(tcp(30))
		fnLbl(lc+=1,1,'Total Wage:',mylen,1)          	: fnTxt(lc,mypos,10,0,0,'10')	: resp$(rc+=1)=str$(tcp(31))
		fnLbl(lc+=1,1,'Net Pay:',mylen,1)             	: fnTxt(lc,mypos,10,0,0,'10')	: resp$(rc+=1)=str$(tcp(32))
		fnLbl(lc+=2,1,'Workmans Comp Wages:',mylen,1)	: fnTxt(lc,mypos,10,0,0,'10')	: resp$(rc+=1)=str$(tdc(6))
		fnLbl(lc+=1,1,'SS Wages:',mylen,1)            	: fnTxt(lc,mypos,10,0,0,'10')	: resp$(rc+=1)=str$(tdc(7))
		fnLbl(lc+=1,1,'Medicare Wages:',mylen,1)      	: fnTxt(lc,mypos,10,0,0,'10')	: resp$(rc+=1)=str$(tdc(8))
		fnLbl(lc+=1,1,'Federal U/C Wages:',mylen,1)   	: fnTxt(lc,mypos,10,0,0,'10')	: resp$(rc+=1)=str$(tdc(9))
		fnLbl(lc+=1,1,'State U/C Wages:',mylen,1)     	: fnTxt(lc,mypos,10,0,0,'10')	: resp$(rc+=1)=str$(tdc(10))
		mypos+=37 : lc=2
		fnLbl(lc+=1,38,'Federal Wh:',mylen,1)         	: fnTxt(lc,mypos,10,0,0,'10')	: resp$(rc+=1)=str$(tcp(1))
		fnLbl(lc+=1,38,'SS Withholdings:',mylen,1)    	: fnTxt(lc,mypos,10,0,0,'10')	: resp$(rc+=1)=str$(tcp(2))
		fnLbl(lc+=1,38,'Medicare Wh:',mylen,1)        	: fnTxt(lc,mypos,10,0,0,'10')	: resp$(rc+=1)=str$(tcp(3))
		fnLbl(lc+=1,38,'State Wh:',mylen,1)           	: fnTxt(lc,mypos,10,0,0,'10')	: resp$(rc+=1)=str$(tcp(4))
		for j=1 to 20
		...
		next j
		fnCmdKey('&Save',ckSave,1,0)
		fnCmdKey('&Delete',4,0,0)
		fnCmdKey('&Cancel',ckCancel,0,1)
		ckey=fnAcs(mat resp$) ! correcting check
		if ckey=ckCancel then
			goto SCREEN2
		else if ckey=4 then
			mat mg$(3)
	...
	ScrFilters: ! r:
		...
		printit=0: f1=0
		if ckey=ckCancel then goto CheckFileXit
	...
	AddGrid: ! r:
		...
		ckey=fnAcs(mat resp$) ! add grid name
		if ckey<>5 then
...
def library fnOpenPrReport
	if ~setup then fn_setup
	fnOpenPrReport=fn_openPrReport
fnend
	def fn_openPrReport(; ___,returnN)
		L320: !
		if exists('[Q]\PRmstr\payrollreports.h[cno]') =0 then
			open #returnN=fnH: 'Name=[Q]\PRmstr\payrollreports.h[cno],KFName=[Q]\PRmstr\reportidx.h[cno],RecL=85,kps=1,kln=30,replace',i,outIn,k
			! 1 - 30 Name c 30
		...
		open #returnN=fnH: 'Name=[Q]\PRmstr\payrollreports.h[cno],KFName=[Q]\PRmstr\reportidx.h[cno],Shr',i,outIn,k
		if kln(returnN)<>30 then
			close #returnN:
			fnFree('[Q]\PRmstr\payrollreports.h[cno]')
			fnFree('[Q]\PRmstr\reportidx.h[cno]')
			goto L320
		end if
		fn_openPrReport=returnN
	fnend
