...
ASKJOB: !
	...
	if resp$='Yes' then 
		rewrite #1,using 'form pos 157,N 2',key=jn$: 9 nokey ASKJOB
	end if
goto ASKJOB
...
L540: !
	read #1,using 'form pos 1,C 6,C 40,3*C 30,N 6,2*PD 7.2,N 2,C 30,C 12,C 60': jn$,n$,mat a$,mat b,contact$,ph$,email$ eof EOF1
...
goto L620
L610: !
	read #2,using L590: cn$,k$,mat l,mat ta eof L790
L620: !
	if jn$><cn$(1:6) then goto L790
	if ta(1)=0 then goto L770
	adr=ta(1)
	mat ta=(0)
L660: !
	read #3,using L530,rec=adr: eno$,jno$,mat tr,pd$,nta
	ot4=ot4+1
...
goto L660
L770: !
	write #12,using L590: cn$,k$,mat l,mat ta
goto L610
L790: !
	write #11,using 'form pos 1,C 6,C 40,3*C 30,N 6,2*PD 7.2,N 2,C 30,C 12,C 60': jn$,n$,mat a$,mat b,contact$,ph$,email$
goto L540
