...
! Direct Deposit file creator
	...
	fncreg_read('Direct Deposit Save File Path',path$,fnSpecialFolderPath$('desktop')&'\DirDep.txt') ! The path should contain the drive designation, any folders and a file name. Eg  'A:\DirDep.txt'
...
ASK_INFO: !
	...
	resp$(respc+=1)=path$
	fnLbl(5,1,"Your Bank Account #:",mypos,1)
	...
	fnTxt(6,mypos+3,12,0,0,"number",0,"The middle set of numbers at the bottom of your checks.")
	...
	resp$(respc+=1)="False"
	fnCmdKey("&Next",1,1,0,"Creadt the direct deposit files." )
	fnCmdKey("E&xit",5,0,1,"Returns to menu")
	fnAcs(sn$,0,mat resp$,ckey) ! ask employee #
	if ckey=5 then goto XIT
	ppd=val(resp$(1))
	path$=resp$(2)
	bankaccount$=odi$=resp$(3) ! bank account #
	...
	READ_DD: !
	L830: !
	read #dd,using "Form pos 1,C 10,C 1,N 9,N 2,N 17": key$,dd$,rtn,acc,acn eof END1
	mat tcp=(0): mat ttdc=(0)
	if rtn=0 and acc=0 then goto L830
	if uprc$(dd$)<>"Y" then goto READ_DD      ! Y means Yes Direct Deposit is active for this person
	key$=lpad$(rtrm$(ltrm$(key$)),8)
	read #hEmployee,using 'Form pos 9,3*C 30,Pos 162,N 6,Pos 173',key=key$: mat em$,em17 nokey L830
...
FileHeaderRecord: ! r: File Header Record _____________________________________________
	...
	! if env$('client')="Washington Parrish" then imd$=" 061000146" ! Immediate Destination   (routing # for federal reserve bank they use)
	! if env$('client')="West Rest Haven" then imd$=" 111000038" ! Immediate Destination   (routing # for federal reserve bank they use) ! most likely wrong
	if env$('client')="Billings" then imd$=" 081505964" ! Immediate Destination   (routing # for federal reserve bank they use)
	! if env$('client')="Washington Parrish" then imo$=" 065201611" ! Immediate Origin  (routing # for your bank)
...
EntryDetailRecord: ! r: entry detail
	...
	write #ddout,using 'Form POS 1,G 1,G 2,pic(########),C 1,C 17,PIC(##########),C 15,C 22,G 2,N 1,C 8,c 7,c 2': 6,tc,int(rtn/10),str$(rtn)(len(str$(rtn)):len(str$(rtn))),str$(acn),tcp(32)*100,z$,em$(1)(1:22),"",ari,lpad$(trim$(odi$),8),tn$,crlf$         ! changed dr$ to str(rtn) ; also da$ to str$(acn)  ! entry to place money in employees account
	pr #255: z$&" "&em$(1)&" "&str$(tcp(32)) pageoflow PRINT_NEWPAGE
	td1=td1+(tcp(32)*100)
	tc1+=(tcp(32)*100) ! added this for the batch totals - ??
	! if env$('client')="West Rest Haven" and rtn=1190515 then totalin=totalin+tcp(32)
	eh=eh+int(rtn/10) ! Entry Hash should accumulate Routing numbers    dropping the last digit of the routing number
return ! /r
...
BatchControlRecord: ! r: Company/Batch Control Record
	...
	write #ddout,using 'Form POS 1,G 1,G 2,pic(########),C 1,C 17,PIC(##########),C 15,C 22,G 2,N 1,C 8,c 7,c 2': 6,27,int(bnkrtn/10),str$(bnkrtn)(len(str$(bnkrtn)):len(str$(bnkrtn))),bankaccount$,td1,"","","",ari,lpad$(trim$(odi$),8),tn$,crlf$        ! changed dr$ to str(rtn) ; also da$ to str$(acn)  ! total entry for  debiting customer account
...
goto DONE_X ! /r
DONE_X: ! r:
	if trim$(path$)<>'' and exists(path$) then let fnFree(path$)
	if trim$(path$)<>"" then execute "Copy DDout"&wsid$&".txt "&path$&" -n"
	if trim$(email$)<>"" then
