...
L320: fnTos(sn$='W2-1')
	...
	fnFra(1,1,3,60,'Date Range for Corrected W2s','Normally this would the first and last day of the calendar year',0)
	cf+=1 : fratype=cf
	...
	fnFra(7,1,3,60,'Date Range used on Original W2s','This could be any date rqnge entered by mistake',0)
	cf+=1 : fratype=cf
...
L1240: open #101: 'SROW=2,SCOL=3,EROW=23,ECOL=77,BORDER=DR,CAPTION=<Create Electronic W2 Diskette for I.R.S.',display,outIn
	pr f '3,15,C 51,R,N': '  INSERT DISKETTE FOR ELECTRONIC W2S IN DRIVE A:'
	...
	pr f '20,5,C 60': 'Computer Manufacturer Name:'
...
L1750: pr newpage
	...
	pr f '8,4,C 72': 'Some states require filing W2s on diskette.  Answer the following'
...
RECRCW: ! r:
	...
	L3260: ! ss wh
	w24$='           ': orgw24$='           '
	if w2(4)=orgw2(4) then orgw2(4)=0: goto L3300 ! try to stop accuwage errors when both both medicare wages agree !! had to set the orgw2(4) to zero because I could never change the pr line to allow me to make it a alpha field  ??? still dont know why i cannot change it
...
! Win_Height and Win_Width= seems pretty obvious dont it
...
! response$(1)= code youre looking for 2-5 are reserved for future use
