...
fncreg_read(cap$&'.emplRegisterFile$',emplRegisterFile$, 'D:\ACS\(Client_Files)\Payroll Done Right\YEE ENTERPRISES 2019 TRANSACTION REBUILD - employee.csv')
fnTos
...
for empItem=1 to emplRecordCount
	...
	tcp7 =emplList_IRA(empItem)         				! ,GARN standard deduction 3
...
def fn_readEmplIntoArrays
	...
	do
		...
		emplList_IRA            	(emplRecordCount)	=val(item$(empl_IRA           	))
