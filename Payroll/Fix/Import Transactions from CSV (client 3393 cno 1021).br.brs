...
fncreg_read(cap$&'.emplRegisterFile$',emplRegisterFile$, 'D:\ACS\(Client_Files)\Payroll Done Right\Mayas.txt')
fnTos
...
for empItem=1 to emplRecordCount
	...
	tcp7 =emplList_MEALS(empItem)          			! ,GARN standard deduction 3
...
def fn_readEmplIntoArrays ! it is all local
	...
	do
		...
		emplList_MEALS             	(emplRecordCount)	=val(item$(empl_MEALS            	))
...
def fn_mat_emplList(newArraySize)
	...
	mat emplList_MEALS          	(newArraySize)
