...
28460       let item$(1)=str$(rec(paymstr))
28480       let item$(2)=vn$ 
28500       let item$(3)=nam$ 
28520       let item$(4)=ad1$
28540       let item$(5)=ad2$ 
28560       let item$(6)=csz$ 
28580       let item$(7)=str$(typ)
28600       let item$(8)=ss$ 
28620       let item$(9)=ph$
28640       let item$(10)=contact$ 
28660       let item$(11)=email$
28680       let item$(12)=fax$
28700       let fnflexadd1(mat item$)
28720     loop
28740     EO_FLEX1: ! 
28760     let fncmdkey("&Add",1,0,0,"Add new payee records") 
28780     let fncmdkey("&Edit",2,1,0,"Highlight any record and press Enter or click Edit or press Alt+E to change any existing payee record.") 
...
34020     let vn$=nam$=ad1$=ad2$=csz$=ss$=ph$=contact$=email$=fax$=myact$="" 
34040     let typ=0
34060   goto EDIT_PAYEE  ! /r
36000   EDIT_PAYEE: ! r:
36020     let holdvn$=vn$
36040     let fntos(sn$="payee-2")
36060     let respc=0
36080     let mylen=28 : let mypos=mylen+2
36100     let fnfra(1,1,12,70,"Payee Information"," ")
36120     let fnlbl(1,1,"Payee Number:",mylen,1,0,1)
36140     let fntxt(1,mypos,8,0,1,"",0,"",1)
36160     let resp$(respc+=1)=vn$
36180     let fnlbl(2,1,"Payee Name:",mylen,1,0,1)
36200     let fntxt(2,mypos,30,0,0,"",0,"",1)
36220     let resp$(respc+=1)=nam$
36240     let fnlbl(3,1,"Address:",mylen,1,0,1)
36260     let fntxt(3,mypos,30,0,0,"",0,"",1)
36280     let resp$(respc+=1)=ad1$
36300     let fnlbl(4,1,"Address:",mylen,1,0,1)
36320     let fntxt(4,mypos,30,0,0,"",0,"",1) 
36340     let resp$(respc+=1)=ad2$
36360     let fnlbl(5,1,"City, St. Zip:",mylen,1,0,1)
36380     let fncombof("CityStZip",5,mypos,30,env$('Q')&"\Data\CityStZip.dat",1,30,0,0,env$('Q')&"\Data\CityStZip.idx",0,0, " ",1,0) 
36400     let resp$(respc+=1)=csz$
36420     let fnlbl(6,1,"Type:",mylen,1,0,1)
36440     let fncombof("Payeetype",6,mypos,27,env$('Q')&"\GLmstr\PayeeType.dat",1,2,3,25,"",0,0, "The payee type is a code used to detemine which box should be used on a 1099 misc form.  Only enter a type code if the payee should get a 1099",1) 
36460     let resp$(respc+=1)=str$(typ)
36480     let fnlbl(7,1,"Federal ID or SS No.",mylen,1,0,1)
36500     let fntxt(7,mypos,11,0,0,"",0,"",1) 
36520     let resp$(respc+=1)=ss$
36540     let fnlbl(8,1,"Phone Number:",mylen,1,0,1)
36560     let fntxt(8,mypos,12,0,0,"",0,"",1) 
36580     let resp$(respc+=1)=ph$
36600     let fnlbl(9,1,"Contact Name:",mylen,1,0,1)
36620     let fntxt(9,mypos,30,0,0,"",0,"",1)
36640     let resp$(respc+=1)=contact$
36660     let fnlbl(10,1,"E-mail Address:",mylen,1,0,1)
36680     let fntxt(10,mypos,30,50,0,"",0,"",1)
36700     let resp$(respc+=1)=email$
36720     let fnlbl(11,1,"Fax Number:",mylen,1,0,1)
36740     let fntxt(11,mypos,12,0,0,"",0,"",1)
36760     let resp$(respc+=1)=fax$
36780     let fnlbl(12,1,"My Account Number:",mylen,1,0,1)
36800     let fntxt(12,mypos,20,0,0,"",0,"",1)
36820     let resp$(respc+=1)=myact$
...
37220       let fnflexadd1(mat glitem$)
37240     loop
37260     EO_FLEX3: ! /r
37300     let fnlbl(21,1,"",1,0,0,0) ! add space before buttons
37320     let fnbutton(lc=21,61,"Add",2,"Add a standard general ledger breakdowns",0,4) 
37340     let fnbutton(lc,67,"Edit",7,"Edit or Delete a standard general ledger breakdowns")
37360     let fncmdkey("Save",1,1,0,"Saves and returns to payee selection") 
37380     let fncmdkey("&Transactions",4,0,0,"List all checks for this payee") 
37400     let fncmdkey("&Cancel",5,0,1,"Return to payee selection")
37420     let fnacs(sn$,0,mat resp$,ck)
...
38400       let percent=gldistrec=0: let payeekey$=gldesc$=payeegl$="" 
...
38560       mat ml$(3) 
38580       let ml$(1)="Your percentage breakdowns total "&str$(tac)&"." 
38600       let ml$(2)="The percentage breakdown must add to 100%." 
38620       let ml$(3)="Correct the percentages." 
38640       let fnmsgbox(mat ml$,resp$,cap$,16) 
...
44020     mat ml$(2) 
44040     let ml$(1)="A record for payee number "&vn$&" already exists" 
44060     let ml$(2)="You must select a different payee number." 
44080     let fnmsgbox(mat ml$,resp$,cap$,16) 
...
66060   let fntos(sn$='payee_hist') 
66080   let lc=0 : let mylen=25 : let mypos=mylen+2 : let width=50
66100   let lc+=1
66120   let fnlbl(lc+=1,30,'Payee Transactions',width,center)
66140   let fnlbl(lc+=1,1,"Transaction Starting Date:",mylen,right)
66160   let fntxt(lc,mypos,8,0,left,'CCYYMMDD',0,'Blank for All') 
66180   let resp$(1)=str$(transactionstartingdate)
66200   let fnlbl(lc+=1,1,"Transaction Ending Date:",mylen,right)
66220   let fntxt(lc,mypos,8,0,left,'CCYYMMDD',0,'Blank for All') 
66240   let resp$(2)=str$(transactionendingdate)
66260   let wbc=0
...
66660     let item6$(1)=str$(rec(trans)) : let item6$(2)=trvn$ 
66680     let item6$(3)=str$(dt): let item6$(4)=str$(am) 
66700     let item6$(5)=rn$ : let item6$(6)=de$ 
66720     let fnflexadd1(mat item6$) 
66740     transOnScreenCount+=1
66760     let transactionstotal+=am
66780   loop
66800   EO_FLEX2: ! 
66820   let fnlbl(5,1,'Transactions Total:',mylen,right)
66840   let fntxt(5,mypos,12,0,right,"10",1,'This is the total of only the transactions shown in the Transaction Grid above. ') 
66860   let resp$(3)=str$(transactionstotal)
66880   let fncmdkey('&Refresh',2,0,0,"If you select a date range, you must refresh the screen to see the transactions for that date range.")
66900   let fncmdkey('&Add',3,1,0,"Allows you to add a transaction that you would like to have included in the totals on a 1099 form.")
66920   if transOnScreenCount>0 then
66940     let fncmdkey('&Edit',4,0,0,"Allows you to change or delete a transaction.")
66960   end if
66980   let fncmdkey('&Close',5,0,1)
67000   let fnacs(sn$,0,mat resp$,ck)
...
68180       let transactionstartingdate=val(resp$(1))
68200       let transactionendingdate=val(resp$(2))
...
72020   let fntos(sn$='payee_gl_dist') 
72040   let respc=0 : let mylen=28 : let mypos=mylen+2
72060   let fnlbl(1,25,"Breakdown for "&nam$(1:20),40)
72080   let fnlbl(3,1,"General Ledger Number:",mylen,right)
72100   let fnqgl(3,mypos) 
72120   let resp$(respc+=1)=fnrgl$(payeegl$) ! think maybe here kj
72140   let fnlbl(4,1,'Percent:',mylen,right)
72160   let fntxt(4,mypos,6,0,0,'32',0,"Percent of total check to be charged to this g/l account.  Enter 25% as 25.00!") 
72180   let resp$(respc+=1)=str$(percent)
72200   let fnlbl(5,1,"Description:",mylen,right)
72220   let fntxt(5,mypos,30) 
72240   let resp$(respc+=1)=gldesc$
72260   let fncmdset(7)
72280   let fnacs(sn$,0,mat resp$,ck)
72300   if ck<>5 then 
72320     let payeekey$=vn$
72340     let payeegl$=fnagl$(resp$(1))
72360     let percent=val(resp$(2)) ! percent
...
84020   if edittrans=0 then let dt=am=0: let rn$=de$=""
84040   let fntos(sn$='add_trans')
84060   let respc=0 : let mylen=28 : let mypos=mylen+2
84080   let fnlbl(1,1,"Date:",mylen,right)
84100   let fntxt(1,mypos,6,0,left,'1',0,'')
84120   let resp$(1)=str$(dt)
84140   let fnlbl(2,1,"Amount:",mylen,right)
84160   let fntxt(2,mypos,12,0,0,'10',0,'')
84180   let resp$(2)=str$(am)
84200   let fnlbl(3,1,"Ref #:",mylen,right)
84220   let fntxt(3,mypos,12,0,0,'',0,'')
84240   let resp$(3)=rn$
84260   let fnlbl(4,1,"Description:",mylen,right)
84280   let fntxt(4,mypos,30,0,0,'',0,'')
84300   let resp$(4)=de$
84320   let fncmdkey("Save",1,1,0,"Saves any changes and returns to Payee selection")
84340   let fncmdkey("&Delete",3,0,0,"Highlight any record and press Alt+D or click Delete to remove any existing transaction.")
84360   let fncmdkey("&Cancel",5,0,1,"Return to Payee selection screen.")
84380   let fnacs(sn$,0,mat resp$,ck)
86000   if ck=5 then 
86020     goto XitTransactionAdd
86040   else if ck=3 then 
86060     mat ml$(2) 
86080     let ml$(1)="You have chosen to delete this transaction." 
86100     let ml$(2)="Click OK to delete or Cancel to retain the transaction." 
86120     let fnmsgbox(mat ml$,resp$,cap$,49)
