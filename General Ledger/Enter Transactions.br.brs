...
ScreenOne: ! r:
	...
	fnbutton_or_disabled(gridSelected<>gridTransactions   	,lc,10,'Allocations' 	,ck_gridTrans    	=55,'',13)
	fnbutton_or_disabled(gridSelected<>gridTransactionsNet,lc,25,'Allocs+Net'  	,ck_gridTransNet 	=57,'',13)
	fnbutton_or_disabled(gridSelected<>gridProofTotals    	,lc,40,'Total by Acct'	,ck_gridTotals   	=56,'',13)
	...
	if gridSelected<>gridProofTotals and count then
		fnButton(lc-0,84,"Edit",ck_Edit=63,"Modify selected transaction",1,8,0,0,defaultEdit,0)
		fnButton(lc-0,93,"Delete",ck_deleteOne=67,"Delete selected transaction",1,8)
	end if
	fn_addOneGrid
	...
	if (selx=1 or selx=2 or selx=4) then
		read #hAccount,using "form pos 13,c 40",key=bankgl$,release: bankAcctName$ nokey S1BankGlFail
	else
		read #hAccount,using "form pos 13,c 40",key=bankgl$,release: bankAcctName$ nokey ignore
	end if
	! pr 'just read bankAcctName$='&bankAcctName$ : pause
	...
	if ckey=ck_edit then
		...
		fn_transactionEdit(recordNumber)
		...
		fn_prProofTotals(hAccount,totalDebits,totalCredits,bankgl$,contraEntryDateN,mat kList$,mat kVal,mat chdr_proof_total$,mat glitem3$)
...
def fn_transactionGrid(hMerge,row,col,twenty,ninety,previouslySelected; _
	...
	restore #hMerge:
	do
		if fn_readMerge(0,gl$,tDate,tAmt,tType,postCode,tr$,desc$,vn$,jv1$,jv2$,jv3$,key$)=-4270 then goto TgEoF
		if enableNet then
...
def fn_totalGrid(hAccount,mat kList$,mat kVal,lc,ps)
	! r: setup
	dim cmask3$(6),chdr_proof_total$(6),glitem3$(6)*128
...
def fn_transactionAdd(typeOfEntryN,bankGl$,transDate; ___,returnN,gl$*12,tr4,tr5,tr6,tr7,lrPrior)
	gl$=''
	tr5=tr6=tr7=0
	tr$=td$=vn$=key$=('')
	jv1$=jv2$=jv3$=''
	tr4=transDate
	tr6=typeOfEntryN
	! write #hMerge,using F_merge: '',0,0,0,0,tr$,td$,vn$,jv1$,jv2$,jv3$,bankGl$
	...
	! response=fn_transactionEdit(recordNumber)
	lrPrior=lrec(hMerge)
	fn_scrMain( 0,'Adding '&fn_transType$(typeOfEntryN),bankGl$,transDate)
	response=ckey
...
fnend
def fn_transactionEdit(recordNumber; ___,returnN,gl$*12,tr4,tr5,tr6,tr7,tr$*12,td$*30,vn$*8,jv1$*6,jv2$*5,jv3$*3,key$*12)
		if recordNumber then
			...
			fn_readMerge(recordNumber,gl$,tr4,tr5,tr6,tr7,tr$,td$,vn$,jv1$,jv2$,jv3$,key$)  ! get basic information from record clicked to find the complete transaction
			selx=tr6
			hMtemp=fn_makeMergeTemp(hMerge,recordNumber,transactionAmt,totalalloc,selx)
			fn_scrMain( 1,'Editing '&fn_transType$(tr6),key$,tr4)
			returnN=ckey
...
def fn_makeMergeTemp(hMerge,mergeRecordNumber,&tAmt,&totalalloc,&selx; _
		___,allocAmt,tr$*12,holdtr$*12,td$*30,vn$*8,jv1$*6,jv2$*5,jv3$*3,gl$*12)

			if fn_readMerge(mergeRecordNumber,gl$,tr4,tr5,tr6,tr7,tr$,td$,vn$,jv1$,jv2$,jv3$,key$)=57 then goto PES_XIT
			holdtr$=tr$
			...
			restore #hMerge:
			do
				if fn_readMerge(0,payeegl$,tr4,allocAmt,tr6,tr7,tr$,td$,vn$,jv1$,jv2$,jv3$,key$)=-4270 then goto MtEoM
				if trim$(tr$)=trim$(holdtr$) then
					if tr6=2 or tr6=7 then allocAmt=-allocAmt ! reverse signs on receipts and sales BEFORE DISPLAYING ON CORRECTION SCREEN
					tAmt+=allocAmt
					vn$=vn$

					selx=tr6
					! if tr6=1 then selx=1 ! reg Disbursements
					! if tr6=2 then selx=2 ! reg Receipts
					! if tr6=3 then selx=3 ! reg Adjustments
					! if tr6=4 then selx=4 !     Payroll Check
					! if tr6=7 then selx=7 !     Sales
					! if tr6=8 then selx=8 !     Purchases
...
fnend
def fn_transactionSave(hMerge,transadr,gl$,tr4,tr5,tr6,tr7,tr$,td$*30,vn$*8,jv1$*6,jv2$*5,jv3$*3,key$*12)
	if tr6=2 or tr6=7 then tr5=-tr5 ! reverse signs on receipts and sales
	if transadr>0 then
		rewrite #hMerge,using F_merge,rec=transadr: gl$,tr4,tr5,tr6,tr7,tr$,td$ noRec L3280
	else
		L3280: !
		write #hMerge,using F_merge: gl$,tr4,tr5,tr6,tr7,tr$,td$,vn$,jv1$,jv2$,jv3$,key$
	end if
fnend
...
def fn_scrMain(; editMode,heading$*64,bankGl$,transDate, _
	___,rc,x$*8) ! mostly local stuff still
	if ~sm_setup then
		sm_setup=1
	...
	mylen=18 : mypos=mylen+3 : rc=0

	! dim typeofentry_selected$*30
	! if editMode then
	! 	typeofentry_selected$=fn_transType$(tr6)
	! else
	...
	! fnLbl(1,4,typeofentry_selected$)
	fnLbl(2,1,"Bank Account:",mylen,1)
	fnLbl(2,mypos,bankAcctName$)
	! pr "Type of Entry: "&typeofentry_selected$
	! pr "Bank Account: "&bankAcctName$
	! pause
	...
	if selx=2 or selx=3 or selx=5 then
		...
		if disable_payee=1 then
			...
			fncombof("payee",6,mypos,35,'[Q]\GLmstr\PayMstr.h[cno]',1,8,9,30,'[Q]\GLmstr\PayIdx1.h[cno]',0,0, 'If the payee number is known, the general ledger information can be extracted from that record.',0)
			resp$(respc_payee=4)=vn$
		end if
	end if

	if selx=1 or selx=7 then
		! if ~editMode then
		! 	fnButton(6,63,"Extract",ck_extract,"Extracts general ledger numbers from payee records",1,7)
		! end if
		fnbutton_or_disabled(~editMode,6,63,'Extract',ck_extract,'Extracts general ledger numbers from payee records')
	...
	resp$(respc_AllocAmt=6)='' : if selx=3 then resp$(respc_AllocAmt)=str$(totalalloc)
	fnButton(lc,59,"Add Allocation",ck_breakdownAdd,'Enter the GL Number and Amount then click this button to add a new allocation to the breakdown.\nThis option is not available during Edits, only Adds.')
	...
	mat cmask$=('')
	cmask$(3)='10'
	fnflexinit1('Glalloc',lc+=3,1,6,60,mat chdr$,mat cmask$,1,0,0)
	restore #hMtemp:
	dim allocItem$(4)*30
	mat allocItem$(4)
	do
	...
	loop
	EO_FLEX1: ! /r
	fnButton(lc,85,"&Edit",ck_breakdownEdit,"Correct an allocation.",1,5)
	fnButton(lc,92,"Edit All",ck_breakdownEditAll,"Edit all allocations without returning to this screen.",1,8)
	if editMode then
		fnCmdKey("Save",ck_save1,1,0,"Completed making corrections to this transaction.")
	else
		fnCmdKey("Save",ck_save2,1,0,"You are completed with this transaction and ready to move to the next transaction.")
	end if
	...
	! pr 'scrMain -> ckey=';ckey : pause

	if ckey=ck_save1 and selx=3 and val(resp$(2))<>0 then ckey=ck_save2 ! force the last entry to write   ! KJ 50707
	...
	else if ckey=ck_payeeTogle then
	...
	if selx=3 and ckey<>ck_save1 then allocamt=transactionAmt ! create an allocation amount automatically on adjustments
	if allocamt=0 and ckey=ck_save2 and lrec(hMtemp)=0 then allocamt=transactionAmt ! allows them to press enter if only allocation without haveing to key the amount a second time

	! ! r: GL Number Test
	! if extract then goto SmGlNoTestFinis
	...
	! if selx=3 and (allocamt=0 or ckey=ck_save1) then goto SmGlNoTestFinis ! don't require gl # on adjustments when amount=0 (actually changing from one adjustment to the next)
	! if selx=4 and editMode then goto SmGlNoTestFinis ! never should have an amount on payroll check. (done from another screen and edits all made from allocation screen
	...
	if ckey=ck_payeeAdd then
		...
		goto ScrMainTop
	else if ckey=ck_save1 then
		goto L4460
	end if
	if ckey=ck_save2 and selx<>3 and transactionAmt<>totalalloc then message$="Allocations don't add up!" : goto ScrMainTop
	! if ckey=ck_save2 and selx=3 and totalalloc <>0 then message$= "Allocations don't add up!": Goto ScrMainTop
	if ckey=ck_save2 then
		L4460: !
		restore #hMtemp:
		do
			read #hMtemp,using "form pos 1,c 12,pd 10.2,c 30,pd 5": allocgl$,allocamt,td$,transadr eof L4520
			tr5=allocamt
			fn_transactionSave(hMerge,transadr,gl$,tr4,tr5,tr6,tr7,tr$,td$,vn$,jv1$,jv2$,jv3$,key$)
		loop
		L4520: !
...
ScrPayroll: ! r:
	...
	fnLbl(1,1,'______________________________ '&heading$&' ______________________________')
	...
	lc=8
	! fnLbl(lc,1,"General Ledger:",mylen,1)
	! fnqgl(lc,mypos,0,2)
	! resp$(5)=fnrgl$(gl$)
	fnTxt(lc,mypos,20,0,1,"",1,"Disabled General Ledger selection")
	resp$(5)=''
	! fnLbl(lc,45,"Amount:",mylen,1)
	fnTxt(lc,50,13,0,1,"10",1,"Amount to allocated to this general ledger number.",0 )
	resp$(6)=''
	! fnLbl(lc+=1,20,"________________________________________ Payroll Breakdown ________________________________________")

	! fnLbl(1,38,"Bank Account: "&bankAcctName$,50,1)

	lc-=2 ! move the Payroll Breakdown frame up to cover up the disabled GL text boxes - only leaving them because it's easier than refactoring all the mat resp$ to use enumerations
	lc+=1
	...
	ScrPayrollXit: !
return ! /r
def fn_clearVar(&hMtemp,&transactionAmt,&td$,&tr$,&vn$,&gl$,&totalalloc,&gl_retainFieldsDuringAdd$)
	!  clear entry screen
	close #hMtemp: ioerr ignore
	open #hMtemp=fnH: "Name=[Q]\GLmstr\Allocations[acsUserId].h[cno],Version=1,replace,RecL=59",internal,outIn,relative
	transactionAmt=0
	...
	! extract=0 kj
fnend
WritePayrollTrans: ! r: &tr$,gl$,...; ___,j,allocgl$,td$*30
	! only routine that uses mat pgl (read from company.h[cno])
	tr6=4 ! payroll transaction type  (was converted back to 1 in old system)
	for j=2 to 19
		...
		if prx(j)<>0 then
			write #hMerge,using F_merge: allocgl$,tr4,prx(j),tr6,tr7,tr$,td$,vn$,jv1$,jv2$,jv3$,key$ ! gross wage
		end if
	next j
	transactionAmt=0
	mat prx=(0)
...
def fn_extract(&hMtemp,vn$,transactionAmt; ___, _
	...
	do
		...
		if payeekey$=glkey$ then
			...
			write #hMtemp,using "form pos 1,c 12,pd 10.2,c 30": payeegl$,allocamt,td$: allocgl$='': totalalloc+=allocamt: allocamt=0
		end if
...
def fn_prProofList(hMerge; ___,holdtr$,tr$)
	...
	do
		holdtr$=tr$
		if fn_readMerge(0,gl$,tr4,tr5,tr6,tr7,tr$,td$,vn$,jv1$,jv2$,jv3$,key$)=-4270 then goto PE_FINIS
		if trim$(holdtr$)<>'' and holdtr$<>tr$ then
			pr #255,using "Form pos 10,c 10,n 14.2,skip 1": "Net",netamount
			netamount=0
		end if
		if tr6=3 then prntkey$='' else printkey$=key$
		pr #255,using F_PE_LINE: gl$,tr4,tr5,tr6,tr7,tr$,td$,vn$,printkey$,jv1$,jv2$,jv3$ pageoflow PrProofListPgOf
...
def fn_buildMatK(hMerge,mat kList$,mat kVal,&totalCredits,&totalDebits,&count; ___,tAmt,key$*12,tranType)
	...
	do
		...
		if kList$(j)=key$ then
			if tranType=1 or tranType=4 or tranType=8 then
				! 1 = Disbursements
				! 4 = Payroll Check
				kVal(j,6)+=tAmt ! total debits entered
				totalDebits+=tAmt
				totalCredits-=tAmt
			else if tranType=2 or tranType=7 then
				! 2 = Receipts
				kVal(j,5)+=tAmt ! total credits entered
				totalCredits+=tAmt
				totalDebits-=tAmt
			else if tranType=3 then
				! 3 = Adjustments
				kVal(j,7)+=tAmt ! net adjustments
...
ScrPost: ! r:
	...
	ckey=fnAcs(mat resp$)
	fnfscode(0)
	fn_createContras(hMerge,mat kList$,mat kVal,contraEntryDateN)
...
goto Xit ! /r
def fn_createContras(hMerge,mat kList$,mat kVal,contraEntryDateN; ___,j,tr$*12)
	restore #hMerge:
	tr$="999999999999"
	for j=1 to 30
...
def fn_readMerge(record,&gl$,&tDate,&tAmt,&tType,&postCode,&tr$,&desc$,&vn$,&jv1$,&jv2$,&jv3$,&key$; ___,returnN)
		if record then
			read #hMerge,using F_merge,rec=record: gl$,tDate,tAmt,tType,postCode,tr$,desc$,vn$,jv1$,jv2$,jv3$,key$ ioerr RmErr
		else
			read #hMerge,using F_merge            : gl$,tDate,tAmt,tType,postCode,tr$,desc$,vn$,jv1$,jv2$,jv3$,key$ ioerr RmErr
		end if
