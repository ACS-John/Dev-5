...
! r: setup
	...
	dim bg(6),bm(13),io1$(3),g1(3),aa(2),gld$*30,ln$*132
...
! r: initialize
	...
	read #5,using 'form N 2,C 50,40*C 12': bud,bud$,mat gl$ noRec ScrMaintainRangeFile, eof ScrMaintainRangeFile
	open #12: 'Name=[Q]\GLmstr\BudgetInfo.h[cno],KFName=[Q]\GLmstr\BudIndx.h[cno],Use,RecL=28,KPs=1,KLn=14,Shr',i,outIn,k

	open #2: 'Name=[Q]\GLmstr\Budget'&str$(bud)&'.h[cno],KFName=[Q]\GLmstr\BgIndx'&str$(bud)&'.h[cno],Shr',i,outIn,k ioerr ScrMenu1
...
ScrMenu1: ! r:
	...
	execute 'Dir [Q]\GLmstr\budget*.h[cno] >[Temp]\FlexWork.tmp' ioerr ignore
	j=0
...
! fnCMBBUD(INDEXFILE$)
	...
	ckey=fnAcs(mat resp$) ! ask budget file #
	ad1=0
	hg1$=g1$=resp$(1)(1:12)
	bud=val(resp$(1)(1:2))
	if ckey=1 then
		ad1=1 : goto ScrMaintainRangeFile
	else if ckey=2 then
		goto ScrMaintainRangeFile
	else if ckey=4 then
...
ckey=fnAcs(mat resp$) ! ask general ledger #
if ckey=5 then goto ScrMenu1
g1$=startgl$=fnagl$(resp$(1))
restore #2,key>=g1$&'B': nokey L2370
L2370: if ckey=2 then goto ScrMenu1
bk=0 ! /r
DISPLAY_GRID: ! r:
restore #2:
x=13
...
restore #2:
goto DISPLAY_GRID
L2890: if ckey=8 then goto ScrMenu1
...
bobtom=len(line$)
if cd$='A' and an1$(1)='Y' then pr #255,using L5120: gd$ pageoflow PgOf
if cd$='B' then pr #255,using L5130: line$(1:len(line$)) pageoflow PgOf
if cd$='S' then pr #255,using L5140: uline$(1:len(line$)),line$,sline$(1:len(line$)) pageoflow PgOf
if cd$='T' then pr #255,using L5150: uline$(1:len(line$)),line$,dline$(1:len(line$)) pageoflow PgOf
if cd$='X' then pr #255,using L5120: '' pageoflow PgOf
...
ckey=fnAcs(mat resp$) ! gl breakdown screen
if ckey=5 then goto ScrMenu1
bud=val(resp$(1))
bud$=resp$(2)
k$=lpad$(str$(bud),2)
