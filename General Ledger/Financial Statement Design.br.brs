...
AddEdit: ! r:
	...
	if edit=1 then
		...
		if holdrno$<>rno$ and trim$(holdrno$)<>'' then
			mat ml$(3)
			ml$(1)='You are changing reference number '&holdrno$&' to '
			ml$(2)='reference # '&rno$&'.  Click OK to continue, else'
			ml$(3)='Cancel to prevent changing the reference number.'
...
DeleteRef: ! r: delete a reference #
	mat ml$(3)
	ml$(1)='You are attempting to delete reference number '&rno$&'.'
	ml$(2)='Click OK to continue, '
	ml$(3)='else Cancel to prevent deleting the reference number.'
	fnMsgBox(mat ml$,resp$,'',49)
