...
00110   open #1: "Name="&env$('Q')&"\TMmstr\CLmstr.h"&env$('cno')&",KFName="&env$('Q')&"\TMmstr\CLIndex.h"&env$('cno')&",Shr",internal,outin,keyed ioerr L700
00120   open #2: "Name="&env$('Q')&"\TMmstr\ARTrans.h"&env$('cno')&",Shr",internal,outin,relative ioerr L700
00130   open #h_addr:=fngethandle: "Name="&env$('Temp')&"\Addr."&session$,internal,outin,relative ioerr L700
00140 ! open #h_armotran:=4: "Name="&env$('Q')&"\TMmstr\ARMoTran.h"&env$('cno')&",Shr",internal,output ioerr L700
00150 LOOP_TOP: ! 
00152   r3=r3+1
...
00260   tr2=tr(2)
00270   if tr(5)=3 then tr(3)=tr(3)+tr2
00280   tr(2)=tr(3)
00290   if tr(5)=4 then tr(5)=6
00300   if tr(5)=3 then tr(5)=4
00310   if tr(5)=2 then tr(5)=5
00320   if ltrm$(p$)="-1" then goto L420
00330   read #2,using L370,rec=1,reserve: rec2
00340 L340: rec2=lrec(2)+1
...
00380   if ta(2)=0 then ta(1)=rec2 else rewrite #2,using L370,rec=ta(2),reserve: rec2
00390   ta(2)=rec2
00400   rewrite #2,using L370,rec=1,release: rec2
00410   rewrite #1,using L210,key=p$: am6,mat ta nokey CLSMSTR_NOKEY
00420 L420: if tr(5)=4 then tr(2)=tr2
00430   if tr(5)=4 then tr(3)=tr(3)-tr2
