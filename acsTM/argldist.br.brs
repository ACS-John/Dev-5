...
00130   print newpage
00140   print fields "10,15,c 60": "POSITION PAPER FOR G/L DISTRIBUTION REPORT"
00150   print fields "12,12,c 60": "ENTER THE AS OF DATE: FORMAT = MMDDYY"
00160   print fields "12,53,n 6,N": dat
...
00230   print newpage
00240   print fields "10,15,c 50,h,n": "PRINT A/R GENERAL LEDGER DISTRIBUTION IN PROCESS"
00250   print fields "23,2,C 30,N": "Press F5 to stop"
00260   gosub L590
...
00340 L340: print #255,using L350: mat gh,"TOTAL ",tdb,tcr
...
00410   let tcr=0
00420 L420: return 
00430 L430: let adb=0
00440   let acr=0
00450   if tr3>0 then let adb=tr3 else let acr=-tr3
00460   if adb=0 then goto L500
00470   print #255,using L480: mat gl,p$,iv$,tr1,adb pageoflow L560
00480 L480: form pos 1,pic(zzzz),n 7,pic(zzzz),x 2,c 8,c 14,pic(zz/zz/zz),n 15.2
00490   goto L520
00500 L500: print #255,using L510: mat gl,p$,iv$,tr1,acr pageoflow L560
...
00540   mat gh=gl
00550   goto L270
00560 L560: print #255: newpage
00570   gosub L590
00580   continue 
00590 L590: print #255,using L600: date$,a$
00600 L600: form skip 2,pos 1,c 8,pos pa,c 40,skip 1
00610   print #255,using L620: time$,"G/L DISTRIBUTION FOR ACCOUNTS RECEIVABLE"
00620 L620: form pos 1,c 8,pos 20,c 45,skip 1
00630   print #255,using L640: "AS OF ",tr4
00640 L640: form pos 33,c 6,pic(zz/zz/zz),skip 2
00650   print #255: "  G/L ACCOUNT # CLIENT #    INVOICE #    DATE           DEBITS        CREDITS"
00660   return 
00670 L670: gosub L340
00680   print #255,using L350: 0,0,0,"FINAL TOTAL ",gtdb,gtcr
00690 L690: close #1: ioerr L700
00700 L700: close #2: ioerr L710
00710 L710: if nw=1 then close #255: else print #255: newpage
00720 XIT: let fnxit
00730 L730: if err=61 then print fields "23,3,C 75,N": "THIS PROGRAM IS TRYING TO ACCESS A RECORD THAT IS IN USE!" else goto L750
00740   goto L790
00750 L750: print newpage
00760   if err=4148 then print fields "23,3,C 78,N": "THIS PROGRAM IS TRYING TO ACCESS A FILE THAT IS IN USE AND CANNOT BE SHARED!" else goto L780
00770   goto L790
00780 L780: print fields "23,3,C 75,N": "YOU HAVE A WORKSTATION BASIC ERROR # "&str$(err)&" AT LINE # "&str$(line)&"."
00790 L790: print fields "24,3,C 70,N": "PRESS ENTER TO RETRY; ELSE ENTER  Q  TO QUIT"
00800   input fields "24,60,C 1,N": quitcode$
00810   if rtrm$(uprc$(quitcode$))="Q" then goto L850
00820   print fields "23,3,C 78,N": ""
00830   print fields "24,3,C 78,N": ""
00840   retry 
00850 L850: goto XIT
