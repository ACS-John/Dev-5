...
00440   print fields "11,21,C 14,N": "Client Number:"
00460   print fields "13,22,C 32,B,5": "Cancel (Esc)"
00480   print fields "14,22,C 32,B,6": "Search (F6)"
00500   print fields "15,22,C 32,B,10": "Print all (F10)"
...
00720 PRINT_EM: print newpage
00740   print fields "10,25,c 50,n": "PRINT CLIENT LABELS IN PROCESS"
00760   print fields "12,34,C 11,B,5": "Cancel (F5)"
00780   let j=1
00800 READ_CLIENT: ! 
...
00980 !     ONLY PRINT CERTAIN TYPES OF MAINTENANCE - type 16=MC
01000 !     app(x) = system
01020 !     ma(x) = support
01040 ! end if
...
01140   let st$=uprc$(ltrm$(rtrm$(a$(3)(p1+1:p1+3))))
...
01240 PRINT_LABEL: ! r:
01260   let a$(1)=rtrm$(a$(1))
01280   let l2=len(a$(1))
01300   let l1=l2-3
01320   let c$=a$(1)(l1:l2)
...
01440   let a$(1)=rtrm$(a$(1)(p1+p2:30))&" "&a$(1)(1:p1-1)
01460 L640: ! 
...
01740   end if 
01760   let fnxit
01780 L850: if err=61 then print fields "23,3,C 75,N": "THIS PROGRAM IS TRYING TO ACCESS A RECORD THAT IS IN USE!" else goto L870
01800   goto L910
01820 L870: print newpage
01840   if err=4148 then print fields "23,3,C 78,N": "THIS PROGRAM IS TRYING TO ACCESS A FILE THAT IS IN USE AND CANNOT BE SHARED!" else goto L900
01860   goto L910
01880 L900: print fields "23,3,C 75,N": "YOU HAVE A WORKSTATION BASIC ERROR # "&str$(err)&" AT LINE # "&str$(line)&"."
01900 L910: print fields "24,3,C 70,N": "PRESS ENTER TO RETRY; ELSE ENTER  Q  TO QUIT"
01920   input fields "24,60,C 1,N": quitcode$
01940   if rtrm$(uprc$(quitcode$))="Q" then goto XIT
01960   print fields "23,3,C 78,N": ""
01980   print fields "24,3,C 78,N": ""
02000   retry 
02020 SRCH1: let s1=1 ! NAME SEARCH
02040   open #127: "SROW=1,SCOL=1,EROW=24,ECOL=80",display,outin  ! SAVE SCREEN
02060 L990: print #127: newpage
...
02120   let prtall=0
02140   print fields "7,4,C 55,H,N": "Enter beginning search info. or blank for all:"
02160   print fields "9,32,C 16,R,N": "Press F5 to stop"
02180 L1050: input fields "7,50,C 25,UE,N": nam$
02200   if cmdkey=5 then goto SRCHEND
02220   let nam$=rtrm$(nam$)
02240   let l1=len(nam$)
02260   restore #32,search>=nam$: nokey L1050
02280   close #101: ioerr L1110
02300 L1110: print newpage
02320   print fields "1,10,C 5,R,N": "ACCT#"
02340   print fields "1,17,C 30,R,N": "COMPANY NAME"
02360   let cde=0
02380   for j=1 to 20
...
02480 L1200: let cde=1
02500     print fields str$(j+1)&",10,C 5,U,N": k$
02520     print fields str$(j+1)&",17,C 30,U,N": a1$
02540     if j>1 then goto L1270
02560     let bk=bk+1
02580     if bk>20 then let bk=1
02600     let bk$(bk)=a1$
02620 L1270: next j
02640 L1280: if j>1 then let j=j-1
02660   mat in2$(j)
02680 L1300: print fields "24,08,C 60,R,N": "Enter to continue; F5 to stop or enter ACCOUNT #:"
...
02920   let bk=bk-1
02940 L1430: let selclp=1
02960   goto L1110
02980 L1450: let selclp=0
03000   goto L990
