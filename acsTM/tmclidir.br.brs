...
00210 L210: print newpage
00230   print fields "10,10,c 48,n": "ENTER DATE FOR CLIENT DIRECTORY IN MMDDYY FORMAT"
00240   print fields "10,60,n 6,n": dat
00250 L250: input fields "10,60,n 6,eu,n": dat conv L250
00260   if dat<10100 or dat>123199 then goto L210
00280   form pos 83,n 6
00290   print newpage
00300   print fields "3,15,C 65,N": "You have the option to get the client directory sorted by "
00310   print fields "4,10,c 70,n": "category.  Place a 1 by any category you want printed.  Leave all"
00320   print fields "5,10,c 70,n": "items blank for numeric order."
00330   print fields mat flo$: mat cat$
00340 L340: input fields mat fli$: mat catcode conv L340
00350   if sum(catcode)=0 then let numprint=1
00360   print newpage
00370   print fields "10,25,c 48,n": "CLIENT DIRECTORY IN PROCESS"
00380   print fields "23,2,c 30,n": "Press F5 to stop"
...
00490 L490: print #255,using L500: z$,a$(1),"BUS PHONE:",ph$,mye,pno
00500 L500: form pos 2,c 5,pos 8,c 30,pos 39,c 10,pos 50,c 12,pos 70,pic(zzz),pos 76,pic(zzzzzzzzz),skip 1
00510     print #255,using L520: a$(2),"HOME PHONE:",ph2$
00520 L520: form pos 8,c 30,pos 39,c 11,pos 51,c 30,skip 1
00530     print #255,using L540: a$(3),"CONTACT:",a$(4) pageoflow L580
00540 L540: form pos 8,c 30,pos 39,c 8,pos 48,c 30,skip 1
00550     print #255,using L560: cm$
00560 L560: form pos 8,c 70,skip 2
00570     goto L450
00580 L580: print #255: newpage
00590     gosub L610
00600     goto L450
00610 L610: let p1=p1+1
00620     print #255,using L630: date$,cnam$,"PAGE",p1
00630 L630: form skip 3,pos 1,c 8,pos namtab,c 40,pos 76,c 5,n 4,skip 1
00640     print #255,using L650: time$,"CLIENT DIRECTORY"
00650 L650: form pos 1,c 8,pos 36,c 16,skip 1
00660     if numprint = 1 then let cattab=37 else let cattab=44-len(rtrm$(cat$(j)))/2
00670     if numprint = 1 then print #255,using L680: "NUMERIC ORDER" else print #255,using L680: cat$(j)
00680 L680: form pos cattab,c 30,skip 1
00690     print #255,using L700: dat
00700 L700: form pos 40,pic(zz/zz/zz),skip 3
00710     print #255,using L720: "CLIENT","YEAR","EMPLOYEE"
00720 L720: form pos 2,c 6,pos 70,c 4,pos 77,c 8,skip 1
00730     print #255,using L740: "NUMBER","NAME AND ADDRESS","CLIENT INFORMATION","END","IN CHARGE"
00740 L740: form pos 2,c 6,pos 12,c 16,pos 42,c 19,pos 71,c 3,pos 76,c 9,skip 2
00750     return 
00760 L760: if numprint=1 then goto L800
00770     print #255: newpage
...
00850   goto XIT
00860 L860: if err=61 then print fields "23,3,C 75,N": "THIS PROGRAM IS TRYING TO ACCESS A RECORD THAT IS IN USE!" else goto L880
00870   goto L920
00880 L880: print newpage
00890   if err=4148 then print fields "23,3,C 78,N": "THIS PROGRAM IS TRYING TO ACCESS A FILE THAT IS IN USE AND CANNOT BE SHARED!" else goto L910
00900   goto L920
00910 L910: print fields "23,3,C 75,N": "YOU HAVE A WORKSTATION BASIC ERROR # "&str$(err)&" AT LINE # "&str$(line)&"."
00920 L920: print fields "24,3,C 70,N": "PRESS ENTER TO RETRY OR ENTER  Q  TO QUIT"
00930   input fields "24,60,C 1,N": quitcode$
00940   if rtrm$(uprc$(quitcode$))="Q" then goto XIT
00950   print fields "23,3,C 78,N": ""
00960   print fields "24,3,C 78,N": ""
00970   retry 
00980 XIT: let fnxit
