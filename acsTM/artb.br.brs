...
00185   close #1: 
00190   let at=int(66-len(rtrm$(cnam$))/2)
00200   let fli$(1)="10,56,C 20,U,N"
00210   let fli$(2)="12,56,N 6,U,N"
00220   print newpage
00230 L230: print fields "5,30,C 18,H,N": "AGED TRIAL BALANCE"
00240   print fields "10,5,cr 50,n": "ENTER AS OF DATE  (EXAMPLE = "&dat$&":"
00250   print fields "12,5,Cr 50,N": "ENTER THE AGING DATE IN MMDDYY FORMAT:"
00260   print fields "10,56,c 20,n": dat$
00270   print fields "14,28,C 30,N": "F1 Continue   F5 Stop"
...
00330   print newpage
00340   let fnopenprn
00350   print fields "10,15,c 50,h": "A/R AGED TRIAL BALANCE PROGRAM IN PROCESS"
00360   print fields "23,2,C 30,N": "Press F5 to stop"
...
00450 L450: if am6<0 then let cb1=cb1+(-am6) else let db1=db1+am6
00460   if am6<=0 then goto L400
00470   gosub L1000
00480   mat s=s+e
00490   print #255,using L500: z$,e$(1),am6,mat e pageoflow L520
00500 L500: form pos 1,c 12,c 30,x 10,6*n 12.2,skip 1
00510   goto L400
00520 L520: print #255: newpage
00530   gosub L550
00540   goto L400
00550 L550: let p2=p2+1
00560   print #255,using L570: date$,cnam$,"PAGE ",p2
00570 L570: form skip 1,pos 1,c 8,pos at,c 40,pos 120,c 5,n 4,skip 1
00580   print #255,using L590: time$,"A/R AGED TRIAL BALANCE"
00590 L590: form pos 1,c 8,pos 55,c 22,skip 1
00600   let q$=rtrm$("As of "&dat$)
00610   let tabq=int(66-len(q$)/2)
00620   print #255,using L630: q$
00630 L630: form pos tabq,c 26,skip 0
00640   if v9=9 then goto L690
00650   print #255: 
00660   print #255: tab(71);"-----------------------AGING--------------------------"
00670   print #255,using L680: "CLIENT #","CLIENT NAME"," ","BALANCE","CURRENT",age(1),"-",age(2),age(2)+1,"-",age(3),age(3)+1,"-",age(4),"OVER ",age(4)
...
00690 L690: return 
00700 L700: let c6=s(1)+s(2)+s(3)+s(4)+s(5)
00710   if c6=0 then goto L790
00720   mat c=(0)
00730   for j=1 to 5
00740     if s(j)=0 then goto L760
00750     let c(j)=s(j)/c6*100
00760 L760: next j
00770   print #255: 
00780   print #255,using L500: "","   COMPANY TOTALS",tam6,mat s
00790 L790: print #255: newpage
00800   let v9=9
00810   gosub L550
00820   print #255,using L830: "A/R AGING SUMMARY"
00830 L830: form skip 2,pos 58,c 40,skip 2
00840   print #255: tab(68);"AMOUNT   PERCENT"
00850   print #255: 
00860   print #255,using L870: "CURRENT",s(1),c(1)
00870 L870: form pos 43,c 10,pos 61,n 13.2,n 10.2,skip
00880   print #255,using L870: str$(age(1)+1)&"-"&str$(age(2)),s(2),c(2)
00890   print #255,using L870: str$(age(2)+1)&"-"&str$(age(3)),s(3),c(3)
00900   print #255,using L870: str$(age(3)+1)&"-"&str$(age(4)),s(4),c(4)
00910   print #255,using L870: "OVER "&str$(age(4)),s(5),c(5)
00920   print #255: 
00930   print #255,using L940: "                       TOTAL",db1
00940 L940: form pos 30,c 30,n 14.2,skip 2
00950   print #255,using L940: "LESS CREDIT BALANCE ACCOUNTS",cb1
00960   print #255,using L940: "NET TOTAL ACCOUNTS RECEIVABLE",db1-cb1
...
01080   let ag1=mo(mm)+dd+yy*365+int(yy/4)
01090   if yy-int(yy/4)*4=0 and mm>2 then let ag1=ag1+1
01100   let ag2=ag0-ag1
...
01540   let ag0=mo(mm)+dd+yy*365+int(yy/4)
01550   if yy-int(yy/4)*4=0 and mm>2 then let ag0=ag0+1
01560   return 
01570 L1570: if err=61 then print fields "23,3,C 75,N": "THIS PROGRAM IS TRYING TO ACCESS A RECORD THAT IS IN USE!" else goto L1590
01580   goto L1630
01590 L1590: print newpage
01600   if err=4148 then print fields "23,3,C 78,N": "THIS PROGRAM IS TRYING TO ACCESS A FILE THAT IS IN USE AND CANNOT BE SHARED!" else goto L1620
01610   goto L1630
01620 L1620: print fields "23,3,C 75,N": "YOU HAVE A WORKSTATION BASIC ERROR # "&str$(err)&" AT LINE # "&str$(line)&"."
01630 L1630: print fields "24,3,C 70,N": "PRESS ENTER TO RETRY; ELSE ENTER  Q  TO QUIT"
01640   input fields "24,60,C 1,N": quitcode$
01650   if rtrm$(uprc$(quitcode$))="Q" then goto L1690
01660   print fields "23,3,C 78,N": ""
01670   print fields "24,3,C 78,N": ""
01680   retry 
