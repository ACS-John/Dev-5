...
def library fnprint_invoice(out,align, &actnum$, mat billto$, inv_num$, inv_date, mat desc$, mat amt,pbal,ebilling;pdfline$*255)
	...
	pr #out: ""
	pr #out: ""
	let pdfline$="[pos(+0,+7)][SETSIZE(14)][Bold]"&lpad$("_",65,"_")&"[/BOLD][SETSIZE(8)]"
	...
	if ebilling=0 then pr #out: "" else print #out: "[SETSIZE(8)][/BOLD][LEFT] [FONT LETTER GOTHIC]"
	...
	total_amt=0
	pr #out: ""
	if ebilling=0 then pr #out,using "Form pos 1,C 73,C 12": "\qc {\b Description","Amount}" else pr #out: "[pos(+0,+7)][Bold]Description [pos(+0,+47)]Amount[/BOLD]"
	...
	for j1=1 to udim(mat desc$)
		if ebilling=0 then 
			...
			if amt(j1)<>0 then pr #out: "[LEFT]"&desc$(j1)&"[pos(+0,+45)][RIGHT]"&cnvrt$("pic(##/##/##)",amt(j1)) else pr #out: ""
		end if 
	...
	if ebilling=0 then pr #out,using "Form POS 51,Cr 13,PIC($-,---,---.##)": "{\b Total:}",total_amt else pr #out: "[LEFT][bold][pos(+0,+47)] Total: [/bold][RIGHT][pos(+0,+2)]"&cnvrt$("pic($zzz,zzz,zz#.##)",total_amt)
	...
	if ebilling=0 then pr #out: "*INSERT FILE:S:\acsTM\black line - six inch.rtf.txt" else pr #out: pdfline$
	! if ebilling=1 then  execute "CONFIG OPTION 31" 
	total_amt=0
fnend 
