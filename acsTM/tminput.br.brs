...
00630   pr fields mat fl2$: mat scr2$,thrs,mat scrid$
00640   pr fields "22,2,Cc 60,B,1": "Continue (F1)"
00650   pr fields "24,2,Cc 60,B,5": "Abort and drop all inputed transactions (F5)"
...
00840 L840: pr fields mat fl1$: mat scr1$,mat scrid$
...
00890   let e1$=a1$=" "
00900   pr fields mat io1$: mat inp,b8,sc,des$
00910   pr fields "22,13,C 45,R,N": "F1 Continue; F4 Search; F5 Complete"
...
00940   if ce>0 then let io1$(ce)(ce1:ce2)="U": ce=0
00950   if cmdkey>0 or curfld=2 then goto L1020 else ce=curfld
00960 L960: ce=ce+1: if ce>udim(io1$) then ce=1
00970 L970: let io1$(ce)=rtrm$(uprc$(io1$(ce))) : ce1=pos(io1$(ce),"U",1) : if ce1=0 then goto L960
00980   ce2=ce1+1 : let io1$(ce)(ce1:ce1)="UC" : goto L930
00990 CONV1: if ce>0 then let io1$(ce)(ce1:ce2)="U"
01000   ce=cnt+1
01010 ERR1: pr fields "24,78,C 1": bell : goto L970
...
01070 L1070: pr fields "4,45,c 30,n": "Enter customer name:"
...
01110   pr fields "4,45,c 30,n": " "
01120   pr fields "5,50,c 30,n": " "
01130   pr fields "23,45,C 35,N": " "
...
01220 L1220: ce=2
01230   goto L1320
01240 L1240: pr fields "5,40,c 30,n": a1$
01250   pr fields "6,40,c 25,n": e1$
...
01290   if ce>0 then let io1$(ce)(ce1:ce2)="U": ce=0
...
01320 L1320: let io1$(ce)=rtrm$(uprc$(io1$(ce))) : ce1=pos(io1$(ce),"U",1) : if ce1=0 then goto L1310
01330   ce2=ce1+1 : let io1$(ce)(ce1:ce1)="UC" : goto L1282
01340 CONV2: if ce>0 then let io1$(ce)(ce1:ce2)="U"
01350   ce=cnt+1
01360 ERR2: pr fields "24,78,C 1": bell : goto L1320
...
01970   pr fields "20,15,C 44": "Do you wish to record PHONE EXPENSE (Y/N):"
...
02050   ce=5
02060   pr fields mat io1$: mat inp,b8,sc,des$
02070   goto L1320
...
02120   pr fields "10,10,c 60": "ENTER REF # TO CORRECT; ENTER 0 WHEN COMPLETED"
02130   pr fields "12,12,C 40": "LAST REF # ON FILE IS: "&str$(lrec(2))
...
02650 L2650: if err=61 then pr fields "23,3,C 75,N": "THIS PROGRAM IS TRYING TO ACCESS A RECORD THAT IS IN USE!" else goto L2670
02660   goto L2710
02670 L2670: pr newpage
02680   if err=4148 then pr fields "23,3,C 78,N": "THIS PROGRAM IS TRYING TO ACCESS A FILE THAT IS IN USE AND CANNOT BE SHARED!" else goto L2700
02690   goto L2710
02700 L2700: pr fields "23,3,C 75,N": "YOU HAVE A WORKSTATION BASIC ERROR # "&str$(err)&" AT LINE # "&str$(line)&"."
02710 L2710: pr fields "24,3,C 70,N": "PRESS ENTER TO RETRY; ELSE ENTER  Q  TO QUIT"
...
02750   pr fields "23,3,C 78,N": ""
02760   pr fields "24,3,C 78,N": ""
02770   retry 
02780 XIT: pr newpage : let fnxit
...
02850   let prtall=0
02860   pr fields "7,4,C 55,H,N": "Enter beginning search info. or blank for all:"
02870   pr fields "9,32,C 16,R,N": "Press F5 to stop"
...
02950   pr fields "1,10,C 5,R,N": "ACCT#"
02960   pr fields "1,17,C 30,R,N": "COMPANY NAME"
02970   cde=0
02980   for j=1 to 20
...
03020 L3020: cde=1
03030     pr fields str$(j+1)&",10,C 5,ut,N": k$
03040     pr fields str$(j+1)&",17,C 30,ut,N": a1$
...
03110   mat in2$(j)
03120   pr fields "24,08,C 60,R,N": "Enter to continue; F5 to stop or enter ACCOUNT #:"
...
03330   pr fields io1$(1): k1
03340   pr fields "5,40,c 30,n": a1$
