...
00040   library 'S:\Core\Library': fncno,fnxit,fntop,fncloseprn,fnopenprn
00050   let fntop(program$,cap$="Company Information")
00060   let fncno(cno,cnam$)
00070   dim co$(4)*40,co(6),ag(4),dat$*20,gln(4,3)
00080   dim fl1$(20),sc1$(19)*256,io1$(27),hd1$*50,cap$*128
00090 ! 
...
00170   data PRINT DETAILS ON STATEMENTS                              (1=YES)
...
00400     let io1$(j*3+13)=str$(j+16)&",37,N 6,U,N"
00410     let io1$(j*3+14)=str$(j+16)&",47,N 3,U,N"
00420   next j
00430   let io1$(27)="21,30,C 20,U,N"
00440   let hd1$="TIME MANAGEMENT CHANGE COMPANY INFORMATION"
00450   open #1: "Name="&env$('Q')&"\TMmstr\Company.h"&str$(cno),internal,outin,relative ioerr L480
00460   goto L500
00470   close #1,free: 
00480 L480: open #1: "Name="&env$('Q')&"\TMmstr\Company.h"&str$(cno)&",SIZE=0,RecL=245",internal,outin,relative ioerr L750
...
00520   print newpage
00530   print fields mat fl1$: mat sc1$,hd1$
00540   print fields mat io1$: mat co$,mat co,mat ag,mat gln,dat$
00545   print fields "23,34,c 20": "F1 Continue"
00550 L550: input fields mat io1$: mat co$,mat co,mat ag,mat gln,dat$ conv L670
00560   if ce>0 then let io1$(ce)(ce1:ce2)="U": let ce=0
00570   if rtrm$(co$(1))="" then let ce=1: goto L690
00580   for j=1 to 4
00590     if co(j)<0 or co(j)>1 then let ce=j+4: goto L690
00600     if j=1 then goto L620
00610     if ag(j-1)>ag(j) then let ce=j+10: goto L690
00620 L620: next j
00630   if co(5)<0 or co(5)>99 then let ce=9: goto L690
00640   rewrite #1,using L510,rec=1: mat co$,mat co,mat ag,mat gln,dat$
00650   close #1: 
00660   goto XIT
00670 L670: if ce>0 then let io1$(ce)(ce1:ce2)="U"
00680   let ce=cnt+1
00690 L690: print fields "24,78,C 1": bell
00700   let io1$(ce)=rtrm$(io1$(ce))
00710   let ce1=pos(uprc$(io1$(ce)),"U",1)
00720   let ce2=ce1+1
00730   let io1$(ce)(ce1:ce1)="CR"
00740   goto L550
00750 L750: if err=61 then print fields "23,3,C 75,N": "THIS PROGRAM IS TRYING TO ACCESS A RECORD THAT IS IN USE!" else goto L770
00760   goto L810
00770 L770: print newpage
00780   if err=4148 then print fields "23,3,C 78,N": "THIS PROGRAM IS TRYING TO ACCESS A FILE THAT IS IN USE AND CANNOT BE SHARED!" else goto L800
00790   goto L810
00800 L800: print fields "23,3,C 75,N": "YOU HAVE A WORKSTATION BASIC ERROR # "&str$(err)&" AT LINE # "&str$(line)&"."
00810 L810: print fields "24,3,C 70,N": "PRESS ENTER TO RETRY; ELSE ENTER  Q  TO QUIT"
00820   input fields "24,60,C 1,N": quitcode$
00830   if rtrm$(uprc$(quitcode$))="Q" then goto XIT
00840   print fields "23,3,C 78,N": ""
00850   print fields "24,3,C 78,N": ""
00860   retry 
00870 XIT: let fnxit
