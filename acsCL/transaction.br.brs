10000 ! replace R:\acsCL\transaction
10100 ! check book transaction file editor
10200 ! ______________________________________________________________________
10300   library 'R:\Core\Library': fntop,fnxit, fnopenprn,fncloseprn,fncno,fnerror,fndat,fndate_mmddyy_to_ccyymmdd,fncmdset,fntos,fnlbl,fnacs,fncombof,fnmsgbox,fnfra,fntxt,fnbutton,fnflexinit1,fnflexadd1,fncmdkey,fnchk,fnaddpayee,fnagl$,fnqgl,fnrgl$,fnupdatebankbal,fnbankbal,fngethandle,fnaddreceipt,fnchain
...
13100   open #20: "Name=Q:\CLmstr\Company.h"&str$(cno)&",Shr",internal,input,relative ioerr CHAIN_SELCNO: read #20,using 'Form Pos 152,N 2',rec=1,release: wbc : close #20: 
13200   gosub OPEN_TRANSACTION_FILES
13300   open #h_tralloc=fngethandle: "Name=Q:\CLmstr\TrAlloc.h"&str$(cno)&",KFName=Q:\CLmstr\TrAlloc-Idx.h"&str$(cno)&",Shr",internal,outin,keyed 
13400 ! /r
13500 SCREEN1: ! r:
...
14200   let fncombof('BankAll',lc,mypos,0,"Q:\CLmstr\BankMstr.h"&str$(cno),1,2,3,30,"Q:\CLmstr\BankIdx1.h"&str$(cno),add_all)
14300   if wbc=0 then let resp$(1)='[All]' else let resp$(1)=str$(wbc)
14400   let fnlbl(lc+=1,1,"Working Transaction Type:",mylen,right)
14500   let fncombof('TransactionTypeall',lc,mypos,0,"Q:\CLmstr\TransactionType.dat",1,1,2,25,"Q:\CLmstr\TransactionType.idx",add_all)
14600   if wtt=0 then let resp$(2)='[All]' else let resp$(2)=str$(wtt)
14700   let fnlbl(lc+=1,1,"Payee:",mylen,right)
14800   let fncombof('Payeeall',lc,mypos,0,"Q:\CLmstr\PayMstr.h"&str$(cno),1,8,9,30,"Q:\CLmstr\PayIdx1.h"&str$(cno),add_all)
...
16200   let fncombof('PostCodeall',lc,mypos,0,"R:\acsCL\PostingCode.dat",1,1,2,25,"R:\acsCL\PostingCode.idx",add_all)
16300   let resp$(7)='[All]'
16400   let fnlbl(lc+=1,1,"Source:",mylen,right)
16500   let fncombof('SourceAll',lc,mypos,0,"Q:\CLmstr\SourceCode.dat",1,1,2,25,"R:\acsCL\SourceCode.idx",add_all)
...
20100   open #h_trmstr(1)=fngethandle: "Name=Q:\CLmstr\TrMstr.h"&str$(cno)&",KFName=Q:\CLmstr\TrIdx1.h"&str$(cno)&",Shr",internal,outin,keyed 
20200   open #h_trmstr(2)=fngethandle: "Name=Q:\CLmstr\TrMstr.h"&str$(cno)&",KFName=Q:\CLmstr\TrIdx2.h"&str$(cno)&",Shr",internal,outin,keyed 
20300   return  ! /r
20500 MENU1: ! r:
...
28300   let fncmdkey('Add &Check (Disbursment)',8,0,0,"Allows you to add hand written checks to the check book files.")
...
28600   let fncmdkey('E&xit',5,0,1,"Exits the check book system.")
...
35700     open #payee:=fngethandle: "Name=Q:\CLmstr\RecMstr.h"&str$(cno)&",KFName=Q:\CLmstr\recIdx1.h"&str$(cno)&",Shr",internal,input,keyed 
35800     open #payeegl:=fngethandle: "Name=Q:\CLmstr\ReceiptGLBreakdown.h"&str$(cno)&",KFName=Q:\CLmstr\ReceiptGLBkdIdx.h"&str$(cno)&",Shr",internal,outin,keyed 
35900   else 
36000     open #payee:=fngethandle: "Name=Q:\CLmstr\PayMstr.h"&str$(cno)&",KFName=Q:\CLmstr\PayIdx1.h"&str$(cno)&",Shr",internal,input,keyed 
36100     open #payeegl:=fngethandle: "Name=Q:\CLmstr\PayeeGLBreakdown.h"&str$(cno)&",KFName=Q:\CLmstr\PayeeGLBkdIdx.h"&str$(cno)&",Shr",internal,outin,keyed 
36200   end if 
36300 ! 
...
68700   let fncombof('Bank',lc,mypos,0,"Q:\CLmstr\BankMstr.h"&str$(cno),1,2,3,30,"Q:\CLmstr\BankIdx1.h"&str$(cno),limit_to_list,0,'',frame)
68800   let resp$(1)=str$(bank_code)
68900   let fnlbl(lc+=1,1,'Transaction Type:',mylen,right,0,frame)
69000 ! let fncombof('TransactionType',lc,mypos,0,'Q:\CLmstr\TransactionType.dat',1,1,2,25,'Q:\CLmstr\TransactionType.idx',limit_to_list,0,'',frame)
...
70900     let fncombof('ReceiptType',lc,mypos,0,"Q:\CLmstr\RecMstr.h"&str$(cno),1,8,9,30,"Q:\CLmstr\RecIdx1.h"&str$(cno),limit_to_list,0,'',frame)
...
71600     else 
71700       let fncombof('Payee',lc,mypos,0,"Q:\CLmstr\PayMstr.h"&str$(cno),1,8,9,30,"Q:\CLmstr\PayIdx1.h"&str$(cno),limit_to_list,0,'',frame)
...
72500   let fncombof('PostCode',lc,mypos,0,"R:\acsCL\PostingCode.dat",1,1,2,25,"R:\acsCL\PostingCode.idx",limit_to_list,0,'',frame)
...
84600   execute "Copy Q:\CLmstr\TrMstr.h"&str$(cno)&" "&env$('Temp')&"\Work."&session$&" -D"
84700   execute "Free Q:\CLmstr\TrMstr.h"&str$(cno)&" -n"
84800   execute "Rename "&env$('Temp')&"\Work."&session$&" Q:\CLmstr\TrMstr.h"&str$(cno)
84900   execute "Index Q:\CLmstr\TrMstr.h"&str$(cno)&" Q:\CLmstr\TrIdx1.h"&str$(cno)&" 1 11 Replace DupKeys -n"
85000   execute "Index Q:\CLmstr\TrMstr.h"&str$(cno)&" Q:\CLmstr\TrIdx2.h"&str$(cno)&" 28/1 8/11 Replace DupKeys -n"
85100   execute "Index Q:\CLmstr\Tralloc.h"&str$(cno)&" Q:\CLmstr\Tralloc-idx.h"&str$(cno)&" 1 11 Replace DupKeys -n"
85200   gosub OPEN_TRANSACTION_FILES
85300   return  ! /r
85500 ! <updateable region: ertn>
85600 ERTN: let fnerror(cap$,err,line,act$,"xit")
...
86300 XIT: let fnxit
86500 CHAIN_SELCNO: let fnchain("R:\Core\programs\selcno")
