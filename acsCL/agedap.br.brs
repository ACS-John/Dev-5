...
library 'S:\Core\Library': fntop,fnxit, fnopenprn,fncloseprn,fncno,fnerror,fnxit,fntop,fnTos,fnLbl,fnTxt,fnCmdSet,fnAcs, fndate_mmddyy_to_ccyymmdd
on error goto Ertn

dim cnam$*40,vnam$*30,de$*50,fd$*30,ft(3)
dim io1$(7)*30,bk(3,2),t1(5),t2(5),cap$*128
dim mo(13)

fncno(cno,cnam$)
fntop(program$,cap$="Accounts Payable Listing (Aged)")
cancel=99
...
HDR: f1=1
	pr #255,using 'Form POS 1,C 8,Cc 86': date$,cnam$
	pr #255,using 'Form POS 1,C 8,Cc 86': time$,cap$
...
AGE2: ! 
	...
	das=max(0,int(fnjd(d1)-fnjd(ivd)))
	for j=1 to 3
