...
00120   cancel=99 : right=1
00130   fncno(cno,cnam$)
00140   fndat(dat$,1)
00150   ty$(1)=" Check Number:" !:
        ty$(2)=" Deposit Number:" !:
...
00310   if resp$(3)(1:1)="T" then pvt$="Y": else pvt$="N" !:
          ! post previously entries
00320   dv$=resp$(4)
00330   gosub GETPRC
00340 ! ______________________________________________________________________
00350   fnwait
00360   open #20: "Name=[Q]\CLmstr\Company.h[cno],Shr",internal,outIn,relative  !:
        read #20,using 'Form POS 618,10*N 1': mat dedcode !:
        close #20: 
00370   d2$=cnvrt$("PIC(######)",d2)
00380   open #20: "Name=[Q]\GLmstr\glBucket.H[cno],Shr",internal,input,relative ioerr L400 !:
        read #20,using 'Form POS 1,N 1',rec=1: glb noRec L390
00390 L390: close #20: 
00400 L400: if glb=2 then glwk$="[Q]\GLmstr\GL"&d2$&".H[cno]"
00410   if glb><2 then glwk$="[Q]\GLmstr\GL_Work_"&env$('acsUserId')&".h[cno]"
00420   open #trmstr=1: "Name=[Q]\CLmstr\TrMstr.H[cno],KFName=[Q]\CLmstr\TrIdx1.H[cno],Shr",internal,outIn,keyed 
00430   open #tralloc=2: "Name=[Q]\CLmstr\TrAlloc.H[cno],Version=2,KFName=[Q]\CLmstr\TrAlloc-Idx.h[cno],Shr",internal,input,keyed 
00440   open #glwk101=3: "Name=[Q]\CLmstr\GLWK101.H[cno],Size=0,RecL=104,Replace",internal,output 
00450   open #glwk201=4: "Name=[Q]\CLmstr\GLWK201.H[cno],Size=0,RecL=110,Replace",internal,output 
00460   open #bankmstr=5: "Name=[Q]\CLmstr\BankMstr.H[cno],KFName=[Q]\CLmstr\BankIdx1.H[cno],Shr",internal,input,keyed 
00470   open #paymstr=6: "Name=[Q]\CLmstr\PayMstr.H[cno],KFName=[Q]\CLmstr\PayIdx1.H[cno],Shr",internal,input,keyed 
00480   fnopenprn
00490   gosub HDR
00500 READ_TRMSTR: ! 
...
00560   tr$=lpad$(rtrm$(tr$),12)
00570   pr1=0
00580   if scd=4 then pr1=val(ven$) conv L590
...
00740   td$=de$(1:30) ! SEND NAME TO GL INSTEAD OF DESCRIPTION
00750   if tr5=0 then goto READ_TRALLOC
00760   tr6=cde
00770   if tr6>1 then tr5=-tr5
...
00850   if ivd=1 then prd(4)+=tr5
00860   if ivd>1 and ivd<5 then prd(ivd+3)=-tr5
00870   if ivd=15 then prd(8)=-tr5
00880   if ivd>4 and ivd<15 and dedcode(ivd-4)=2 then tr5=-tr5
00890   if ivd>4 and ivd<15 then prd(ivd+4)=-tr5
00900   if ivd=16 then prd(19)=-tr5
00910   if fp(ivd*.01)=.19 then prd(20)=int(ivd*.01)
...
00970   prd(1)=pr1 : prd(2)=tr4
00980   prd(3)=val(tr$) conv L990
00990 L990: prd(22)=amt
01000   write #glwk201,using 'Form POS 1,N 4,2*PD 4,19*PD 5.2,PD 3': mat prd
01010 L1010: mat prd=(0)
01020   if cde=2 then p1=68 else p1=56
01030   if cde=3 and amt>0 then t1+=amt: t2=t2-amt: goto L1080
01040   if cde=3 and amt<0 then t2+=amt: t1=t1-amt: goto L1110
01050   if cde=2 then !:
          c2+=amt: t2+=amt else c1+=amt: t1+=amt
01060   pr #255,using 'Form POS 1,C 10,PIC(ZZ/ZZ/ZZ),X 2,C 35,POS P1,N 13.2': ltrm$(tr$),tr4,de$,amt pageoflow NEWPGE
01070   goto THERE
01080 L1080: pr #255,using 'Form POS 1,C 10,PIC(ZZ/ZZ/ZZ),X 2,C 35,POS P1,N 13.2': ltrm$(tr$),tr4,de$,amt pageoflow NEWPGE: p1=68
01090   pr #255,using 'Form POS 1,C 10,PIC(ZZ/ZZ/ZZ),X 2,C 35,POS P1,N 13.2': ltrm$(tr$),tr4,de$,-amt pageoflow NEWPGE
01100   goto THERE
01110 L1110: pr #255,using 'Form POS 1,C 10,PIC(ZZ/ZZ/ZZ),X 2,C 35,POS P1,N 13.2': ltrm$(tr$),tr4,de$,-amt pageoflow NEWPGE: p1=68
...
01420   execute "Copy [Q]\CLmstr\GLWk101.h[cno] "&dv$&" -n" ioerr MSGBOX1
01430   execute "Copy [Q]\CLmstr\GLWk201.h[cno] "&dv$&" -n"
01440   execute "Copy [Q]\CLmstr\PayMstr.h[cno] "&dv$&" -n"
01450   execute "Copy [Q]\CLmstr\PayIdx1.h[cno] "&dv$&" -n"
01460 XIT: fnxit
...
01500   execute "Copy [Q]\CLmstr\GLWK101.H[cno]"&' '&"[Q]\GLmstr\GL_Work_"&env$('acsUserId')&".H[cno] -n"
01510   execute "Copy [Q]\CLmstr\GLWK201.H[cno]"&' '&"[Q]\GLmstr\GLWK2"&wsid$&".H[cno] -n"
01520   if lr4=0 then goto L1550
01530   open #1: "Name=[Q]\GLmstr\PRmstr.h[cno],KFName=[Q]\GLmstr\PRINDEX.h[cno],Shr",internal,outIn,keyed ioerr L1550
...
01700 GETPRC: ! 
01710   pri1=0
01720   open #7: "Name=[Q]\PRmstr\Company.h[cno],Shr",internal,input ioerr L1830 !:
...
01800   ml$(4)="Do you wish to combine these accounts?"
01810   fnmsgbox(mat ml$,resp$,cap$,3)
01820   if resp$='Cancel' then goto XIT else prc$=resp$(1:1)
01830 L1830: return 
...
02110   open #glwk101=3: "Name=[Q]\CLmstr\GLWK101.H[cno]",internal,input 
...
02150   goto L2130
02160 L2160: open #1: "Name=[Q]\GLmstr\PRmstr.h[cno],KFName=[Q]\GLmstr\PRINDEX.h[cno],Shr",internal,outIn,keyed ioerr XIT
