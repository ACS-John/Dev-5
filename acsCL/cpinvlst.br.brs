! Replace S:\acsCL\CpInvLst
! checkbook Accounts Payable Listing (Any Time)
!
	library 'S:\Core\Library': fntop,fnxit, fnopenprn,fncloseprn,fnerror,fncno,fnxit,fntop,fnTos,fnLbl,fnTxt,fncomboa,fnCmdSet,fnAcs,fndate_mmddyy_to_ccyymmdd,fnFree
	on error goto Ertn
!
	dim cnam$*40,vnam$*30,de$*30,cap$*128,sq1$*1,item1$(2)*15
!
	fncno(cno,cnam$)
	fntop(program$,cap$="Payables Listing (Any Time)")
	cancel=99
	fnTos
	respc=0
...
HDR: ! 
	pr #255,using 'Form POS 1,C 8,CC 78': date$,cnam$
	pr #255,using 'Form POS 1,C 8,POS 32,C 40': time$,cap$
