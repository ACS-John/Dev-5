...
00170   fli$(2)="10,40,N 2,UE,N"
00180   fli$(3)="11,25,C 30,UT,N"
00190   scr$(1)="Fund #:"
00200   scr$(2)="Dept #:"
00210   scr$(3)="Department Name:"
00220 ! 
00230   for j=1 to 22 : ins$(j)=str$(j+1)&",2,C 42,H,N" : next j
00240 L240: open #1: "Name=[Q]\CLmstr\DPTMSTR.h[cno],KFName=[Q]\CLmstr\DPTIDX1.h[cno],Shr",internal,outIn,keyed ioerr L1800
...
00500   pr f "6,18,C 41,R,N": "            COMPANY NUMBER [cno]"
...
00540   pr f "17,28,C 16,R,N": "PRESS F5 TO STOP"
00550   pr f "13,15,C 47,R,N": " Enter ERASE to continue"
...
00600 L600: execute "COPY A:DPTMSTR.H[cno],[Q]\CLmstr\*.*"
00610   goto L770
00620 L620: if ltrm$(rtrm$(pas$))><"BUILD" then goto L730
00630   close #1: ioerr ignore
00640   open #2: "Name=DPTMSTR.h[cno]/DPTMSTR,KFName=DPTIDX1.h[cno]/DPTMSTR,Shr",internal,input,keyed 
00650   open #1: "Name=[Q]\CLmstr\DPTMSTR.h[cno],SIZE=0,RecL=35,Replace",internal,output 
...
00750   open #1: "Name=[Q]\CLmstr\DPTMSTR.h[cno],SIZE=0,RecL=35,Replace",internal,output 
00760   close #1: 
00770 L770: execute "Index [Q]\CLmstr\DPTMSTR.h[cno]"&' '&"[Q]\CLmstr\DPTIDX1.h[cno] 1 5 Replace DupKeys"
00780   goto L240
00790 L790: new1=1
...
00840   if ce>0 then fli$(ce)(ce1:ce2)="U": ce=0
00850   if cmdkey>0 or curfld=2 then goto L920 else ce=curfld
00860 L860: ce=ce+1: if ce>udim(fli$) then ce=1
00870 L870: fli$(ce)=rtrm$(uprc$(fli$(ce))) : ce1=pos(fli$(ce),"U",1) : if ce1=0 then goto L860
00880   ce2=ce1+1 : fli$(ce)(ce1:ce1)="UC" : goto L830
00890 CONV0: if ce>0 then fli$(ce)(ce1:ce2)="U"
00900   ce=cnt+1
00910 ERR0: pr f "24,78,C 1": bell : goto L870
00920 L920: ! 
...
01050 L1050: if ti><3 then pr f "2,20,C 25,N": ""
01060   pr f "13,20,C 25,R,N": " F1=CONTINUE, F5=STOP"
01070   ce=3: goto L1120
01080 L1080: input fields mat fli$,attr "R": fun,dpt,d$ conv CONV1
01090   if ce>0 then fli$(ce)(ce1:ce2)="U": ce=0
01100   if cmdkey>0 or curfld=3 then goto L1170 else ce=curfld+1
01110 L1110: if ce>udim(fli$) then ce=1
01120 L1120: fli$(ce)=rtrm$(uprc$(fli$(ce))) : ce1=pos(fli$(ce),"U",1)
01130   if ce1=0 then ce=ce+1 : goto L1110 else ce2=ce1+1 : fli$(ce)(ce1:ce1)="UC" : goto L1080
01140 CONV1: if ce>0 then fli$(ce)(ce1:ce2)="U"
01150   ce=cnt+1
01160 ERR1: pr f "24,78,C 1": bell : goto L1120
...
01330   pr f "8,10,c 60,n": "YOU CHOSE TO DELETE ACCOUNT # "&str$(hfun)&" "&str$(hdpt)
01340   goto L1390
01350 L1350: read #1,using L1360,key=nkey$: fun nokey L1380
01360 L1360: form pos 1,n 3
01370   goto L800
01380 L1380: pr f "8,4,c 76,n": "YOU CHOSE TO CHANGE ACCOUNT # "&str$(hfun)&" "&str$(hdpt)&" TO "&str$(fun)&" "&str$(dpt)
01390 L1390: pr f "12,10,c 35,n": "ENTER 1 TO CONTINUE; ELSE ENTER 0"
...
01580   pr f "09,18,C 41,H,N": "            COMPANY NUMBER [cno]"
...
01780 L1780: execute "Index [Q]\CLmstr\DPTMSTR.h[cno]"&' '&"[Q]\CLmstr\DPTIDX1.h[cno] 1 5 Replace DupKeys -n"
01790   goto XIT
01800 L1800: if err=4152 then goto L740
01810 L1810: restore #1,search>="": nokey MENU1
01820 L1820: ln=eof1=0
01830   pr newpage
01840   pr f "1,2,C 4,R,N": "FUND"
01850   pr f "1,7,C 4,R,N": "DEPT"
01860   pr f "1,13,C 30,R,N": "DESCRIPTION"
01870 L1870: read #1,using L1250: fun,dpt,d$ eof L1950
01880   ots$(1)=str$(ln+2)&",2,N 3,H,N"
01890   ots$(2)=str$(ln+2)&",8,N 2,H,N"
01900   ots$(3)=str$(ln+2)&",13,C 30,H,N"
01910   pr f mat ots$: fun,dpt,d$
01920   ln=ln+1
01930   if ln>21 then goto L1970
01940   goto L1870
01950 L1950: eof1=1
01960   if ln=0 then goto MENU1
01970 L1970: pr f "24,2,C 78,R,N": "HIGHLIGHT ACCOUNT # & PRESS ENTER TO SELECT; PRESS F1 TO CONTINUE; F5 TO STOP"
01980   mat ink$(ln)
