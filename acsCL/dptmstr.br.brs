...
00310   let cap$="Department"
00320 MENU1: print newpage
00330   if process=1 then let ti=4 : goto L450
00340   let cnam$=rtrm$(cnam$)
...
00370   print fields "2,9,Cc 41,R,N": cnam$
00380   print fields "3,9,Cc 41,R,N": "Company Number "&str$(cno)
00390   let sm1$(1)="1.  Initial File Preparation" !:
        let sm1$(2)="2.  Add New Records" !:
        let sm1$(3)="3.  Modify or Inquire Records" !:
        let sm1$(4)="4.  Print Proof List" !:
        let sm1$(5)="5.  Search" !:
        let sm1$(6)="6.  Reorganize"
00400   print fields "13,25,C 09,B,5": "Exit (F5)"
...
00490   print fields "5,18,Cc 41,R,N": cnam$
00500   print fields "6,18,C 41,R,N": "            COMPANY NUMBER "&str$(cno)
00510   print fields "8,13,C 52,R,N": " ******************   WARNING   ******************"
00520   print fields "10,13,C 52,H,N": "  THIS SELECTION WILL DESTROY ALL EXISTING RECORDS"
00530   print fields "11,13,C 52,H,N": "  ON THE DEPARTMENT NAME FILE."
00540   print fields "17,28,C 16,R,N": "PRESS F5 TO STOP"
00550   print fields "13,15,C 47,R,N": " Enter ERASE to continue"
...
00810   print fields mat flo$: hd$(1),mat scr$
00820   print fields "13,20,C 25,R,N": " F1=CONTINUE, F5=STOP"
00830 L830: input fields mat fli$,attr "R": hfun,hdpt conv L830
00840   if ce>0 then let fli$(ce)(ce1:ce2)="U": let ce=0
00850   if cmdkey>0 or curfld=2 then goto L920 else let ce=curfld
00860 L860: let ce=ce+1: if ce>udim(fli$) then let ce=1
00870 L870: let fli$(ce)=rtrm$(uprc$(fli$(ce))) : let ce1=pos(fli$(ce),"U",1) : if ce1=0 then goto L860
00880   let ce2=ce1+1 : let fli$(ce)(ce1:ce1)="UC" : goto L830
00890 CONV0: if ce>0 then let fli$(ce)(ce1:ce2)="U"
00900   let ce=cnt+1
00910 ERR0: print fields "24,78,C 1": bell : goto L870
00920 L920: ! 
...
01020   print fields mat flo$: hd$(1),mat scr$
01030   print fields flo$(1): hd$(2)
01040   print fields mat fli$: fun,dpt,d$
01050 L1050: if ti><3 then print fields "2,20,C 25,N": ""
01060   print fields "13,20,C 25,R,N": " F1=CONTINUE, F5=STOP"
01070   let ce=3: goto L1120
01080 L1080: input fields mat fli$,attr "R": fun,dpt,d$ conv CONV1
01090   if ce>0 then let fli$(ce)(ce1:ce2)="U": let ce=0
01100   if cmdkey>0 or curfld=3 then goto L1170 else let ce=curfld+1
01110 L1110: if ce>udim(fli$) then let ce=1
01120 L1120: let fli$(ce)=rtrm$(uprc$(fli$(ce))) : let ce1=pos(fli$(ce),"U",1)
01130   if ce1=0 then let ce=ce+1 : goto L1110 else let ce2=ce1+1 : let fli$(ce)(ce1:ce1)="UC" : goto L1080
01140 CONV1: if ce>0 then let fli$(ce)(ce1:ce2)="U"
01150   let ce=cnt+1
01160 ERR1: print fields "24,78,C 1": bell : goto L1120
01170 L1170: if cmdkey=5 then goto MENU1
01180 ! IF TI=2 AND DPT=0 THEN GOTO 470
01190   if fun<0 then let convc=1: goto ERR1
01200   if dpt<0 then let convc=2: goto ERR1
...
01310 L1310: print newpage
01320   if dpt><0 then goto L1350
01330   print fields "8,10,c 60,n": "YOU CHOSE TO DELETE ACCOUNT # "&str$(hfun)&" "&str$(hdpt)
01340   goto L1390
01350 L1350: read #1,using L1360,key=nkey$: fun nokey L1380
01360 L1360: form pos 1,n 3
01370   goto L800
01380 L1380: print fields "8,4,c 76,n": "YOU CHOSE TO CHANGE ACCOUNT # "&str$(hfun)&" "&str$(hdpt)&" TO "&str$(fun)&" "&str$(dpt)
01390 L1390: print fields "12,10,c 35,n": "ENTER 1 TO CONTINUE; ELSE ENTER 0"
...
01460   if bb=0 then let bb=cb
01470   if cb=0 then let cb=bb
...
01540 L1540: print newpage
01550   close #101: ioerr ignore
01560   open #101: "SROW=08,SCOL=18,EROW=12,ECOL=58,BORDER=DR,CAPTION=GL PROOF LIST",display,outin 
01570   print fields "08,18,Cc 41,H,N": cnam$
01580   print fields "09,18,C 41,H,N": "            COMPANY NUMBER "&str$(cno)
01590   print fields "11,18,C 41,R,N": "              IN PROCESS"
01600   print fields "13,30,C 16,R,N": "PRESS F5 TO STOP"
01610   on fkey 5 goto L1720
01620   let fnopenprn(cp,58,220,process)
01630   gosub L1690
01640 L1640: read #1,using L1250: fun,dpt,d$ eof L1720
01650   print #255,using L1660: fun,dpt,d$ pageoflow L1680
01660 L1660: form pos 1,n 3,n 7,x 2,c 30,skip 1
01670   goto L1640
01680 L1680: print #255: newpage : gosub L1690 : continue 
...
01830   print newpage
01840   print fields "1,2,C 4,R,N": "FUND"
01850   print fields "1,7,C 4,R,N": "DEPT"
01860   print fields "1,13,C 30,R,N": "DESCRIPTION"
...
01910   print fields mat ots$: fun,dpt,d$
01920   let ln=ln+1
01930   if ln>21 then goto L1970
01940   goto L1870
01950 L1950: let eof1=1
01960   if ln=0 then goto MENU1
01970 L1970: print fields "24,2,C 78,R,N": "HIGHLIGHT ACCOUNT # & PRESS ENTER TO SELECT; PRESS F1 TO CONTINUE; F5 TO STOP"
01980   mat ink$(ln)
