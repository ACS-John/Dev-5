00010 ! Replace R:\acsCL\BankBal
00020 ! Running Bank Balance
00030 ! ______________________________________________________________________
00040   library 'R:\Core\Library': fntop,fnxit, fnopenprn,fncloseprn,fncno,fndat,fnerror,fntos,fnlbl,fntxt,fncmdset,fnacs, fndate_mmddyy_to_ccyymmdd,fnmsgbox
00050   on error goto ERTN
00060 ! ______________________________________________________________________
00070   dim dat$*20,cnam$*40,vnam$*30,de$*35,bn$*30,cap$*128,ml$(3)*100
00080 ! ______________________________________________________________________
00090   let fncno(cno,cnam$)
00100   let fntop(program$, cap$="Running Bank Balance")
00110   let cancel=99
00120   let fndat(dat$,1)
00130   open #20: "Name=Q:\CLmstr\Company.h"&str$(cno)&",Shr",internal,outin,relative: read #20,using 'Form POS 152,N 2',rec=1,release: bank_code !:
        close #20: 
00140 MAIN: ! 
...
00180   let fntxt(1,33,10,0,1,"3") !:
        let resp$(respc+=1)=""
00190   let fnlbl(2,1,"Beginning Check Book Balance:",31,1)
00200   let fntxt(2,33,12,0,1,"10") !:
        let resp$(respc+=1)=""
00210   let fnlbl(3,1,"Bank Number to Print:",31,1)
00220   let fntxt(3,33,2,0,1,"30") !:
        let resp$(respc+=1)=str$(bank_code)
00230   let fncmdset(2): let fnacs(sn$,0,mat resp$,ckey)
00240   if ckey=5 then goto XIT
00250   let d1=val(resp$(1))
00260   let b1=val(resp$(2))
00270   let bank_code=val(resp$(3))
00280 ! ______________________________________________________________________
00290   open #20: "Name=Q:\CLmstr\BankMstr.H"&str$(cno)&", KFName=Q:\CLmstr\BankIdx1.H"&str$(cno)&",Shr", internal, outin, keyed  !:
        read #20,using 'Form POS 3,C 30',key=lpad$(str$(bank_code),2),release: bn$ nokey MAIN
00300   close #20: 
00310 ! ______________________________________________________________________
00320   close #trmstr: ioerr L330
00330 L330: execute "Index Q:\CLmstr\TrMstr.H"&str$(cno)&" Q:\CLmstr\Tridx3.H"&str$(cno)&" 16/12/4 2/4/8 Replace DupKeys -n" ! index in year,monthday,reference
00340 ! ______________________________________________________________________
00350   open #trmstr=5: "Name=Q:\CLmstr\TrMstr.H"&str$(cno)&", KFName=Q:\CLmstr\Tridx3.H"&str$(cno)&",Shr", internal, outin, keyed ioerr L360
...
00430   goto READ_2
00440 L440: mat ml$(2) !:
        let ml$(1)='It appears you have no transactions for' !:
        let ml$(2)="the date entered.  Enter a correct date." !:
        let fnmsgbox(mat ml$,ok$,cap$,0) !:
        goto MAIN
00450 READ_2: ! 
...
00560 HDR: ! 
00570   print #255,using 'Form POS 1,C 8,Cc 76': date$,cnam$
...
00640   if p1=0 then !:
          let d1$=cnvrt$("pic(####/##/##)",d1) !:
          let d3=val(d1$(6:7))*10000+val(d1$(9:10))*100+val(d1$(3:4)) !:
          print #255,using 'Form POS 1,C 10,pic(zz/zz/zz),X 2,C 35,POS 80,N 12.2': "",d3,"Beginning Balance",b1
00650   return 
...
00750 ERTN: let fnerror(cap$,err,line,act$,"xit")
