00010 ! Replace R:\acsCL\PostGL2.br
00020 ! GL Distribution Report     AND/OR     Post to General Ledger
10000   def library fnpostgl2(glt)
10020     library 'R:\Core\Library': fnopenprn,fncloseprn,fncno,fnerror,fnputcno,fndate_mmddyy_to_ccyymmdd,fnprocess,fnchain,fntos,fnlbl,fntxt,fncomboa,fnchk,fncmdset,fnacs,fnmsgbox
...
12160     let up1$="C"
12180     open #fundmstr=9: "Name=Q:\CLmstr\FundMstr.H"&str$(cno)&",KFName=Q:\CLmstr\FundIdx1.H"&str$(cno)&",Shr",internal,input,keyed 
12200 READ_FUNDMSTR: ! 
...
14060     let d1=val(resp$(1)(5:6))*10000+val(resp$(1)(7:8))*100+val(resp$(1)(3:4)) ! beginning date
14080     let d2=val(resp$(2)(5:6))*10000+val(resp$(2)(7:8))*100+val(resp$(2)(3:4)) ! ending date  ! convert dates back to mmddyy
...
16100     let fnopenprn
16120     open #trmstr=1: "Name=Q:\CLmstr\TrMstr.H"&str$(cno)&",KFName=Q:\CLmstr\TrIdx1.H"&str$(cno)&",Shr",internal,outin,keyed 
16140     open #tralloc=3: "Name=Q:\CLmstr\TrAlloc.H"&str$(cno)&",KFName=Q:\CLmstr\tralloc-idx.h"&str$(cno)&",Shr",internal,outin,keyed 
16160     open #bankmstr=4: "Name=Q:\CLmstr\BankMstr.H"&str$(cno)&",KFName=Q:\CLmstr\BankIdx1.H"&str$(cno)&",Shr",internal,outin,keyed 
...
18160     if ivd=0 then let ivd=pd ! kJ   10/02/06   skipping receipts w/o invoice numbers
...
18240     if scd=4 or fndate_mmddyy_to_ccyymmdd(ivd)>fndate_mmddyy_to_ccyymmdd(pd) then let ivd=pd
...
18540     let ivd=pd: goto L990 ! Manual Check
18560 L850: ! 
18580     if up1$="C" then let ivd=pd
...
19020 L1000: if pcde=0 or pcde=2 then let pcde+=1
...
20620     else 
20640       let tc1=tc1+abs(amt)
20660       let gc1=gc1+abs(amt)
20680     end if 
20700 L1590: ! 
20720     if bank_code=0 then goto L1660
20740     if tcde=1 then let tbc(bank_code,1)=tbc(bank_code,1)+amt
20760     if tcde=2 then let tbc(bank_code,2)=tbc(bank_code,2)+amt
20780 ! IF TCDE=3 AND AMT>0 THEN LET TBC(Bank_Code,2)=TBC(Bank_Code,2)+AMT
20800 ! IF TCDE=3 AND AMT<0 THEN LET TBC(Bank_Code,1)=TBC(Bank_Code,1)+ABS(AMT)
...
27020     open #20: "Name=Q:\CLmstr\PostDat.H"&str$(cno)&",Replace,RecL=12",internal,outin,relative 
...
27160 ! let fnputcnam(empty$)
27180       let fnchain("R:\acsGL\ACGLMrge")
27200     end if 
27220 ! /r
29000 END1: ! r:
29010     if scd=4 and pa1+pa2<>0 then gosub COMBINEPR
29020     if up1$="C" then goto END2
29040     open #paytrans=6: "Name=Q:\CLmstr\PayTrans.H"&str$(cno)&",KFName=Q:\CLmstr\UnPdIdx1.H"&str$(cno)&",Shr",internal,outin,keyed 
29060     open #unpdaloc=7: "Name=Q:\CLmstr\UnPdAloc.H"&str$(cno)&",KFName=Q:\CLmstr\Uaidx2.H"&str$(cno)&",Shr",internal,outin,keyed 
29080     open #paymstr=8: "Name=Q:\CLmstr\PayMstr.H"&str$(cno)&",KFName=Q:\CLmstr\PayIdx1.H"&str$(cno)&",Shr",internal,input,keyed 
29100 READ_PAYTRANS: ! 
...
33120 L2830: ! 
33140     if glb=2 then 
33160       let glwk$="Q:\GLmstr\GL"&d2$&".H"&str$(gl2)
33180       open #glwk=11: "Name="&glwk$&",RecL=104,Use",internal,output 
33200     else 
33220       let glwk$="Q:\GLmstr\GLWK1"&wsid$&".H"&str$(gl2)
...
35040 REGGL2: ! 
35060     let tr4=ivd : let tr5=amt : let tr6=tcde
35080     if tr6=2 or tr6=3 then let tr5=-tr5
35100     if scd=4 then let tr6=1
35120     let tr$=ck$ : let td$=de$ : let ven$="" ! VN$
35140     if tr6=3 then 
...
35260 ! 
35280 PRGL: ! 
35300     let gw$=pgl$ : let tr4=pivd : let x=pivd : let pivd=x
35320     let tr$="PR "&cnvrt$("PIC(ZZ/ZZ/ZZ)",pivd) : let td$=pde$ : let ven$="" : let wbank_code=pbank_code
35340     if pa1<>0 then 
35360       let tr5=pa1
35380       let tr6=1
...
35480     else 
35500       let tr5=-pa2 : let tr6=1 : let ven$=""
35520       goto SOMETHING
35540     end if 
35560 ! 
35580 BANKGL: ! 
35600     let gw$=gl$ : let wbank_code=bank_code : let tr4=d2 : let x=d2 : let d2=x
35620     let tr$="BK "&cnvrt$("PIC(ZZ/ZZ/ZZ)",d2)
35640     let td$="Bank #:"&str$(j)&" Total" : let ven$=""
35660     if tbc(j,1)<>0 then 
35680       let tr5=-tbc(j,1)
35700       let tr6=1
...
35800     else 
35820       let tr5=tbc(j,2)
35840       let tr6=2
35860       goto SOMETHING
35880     end if 
35900 ! 
35920 APGL: ! 
35940     let gw$=gl$ : let wbank_code=bank_code : let tr4=d2 : let x=d2 : let d2=x
35960     let tr$="AP "&cnvrt$("PIC(ZZ/ZZ/ZZ)",d2) : let td$="AP Total"
35980     let ven$=""
36000     if apc(j,2)<>0 then 
36020       let tr5=-apc(j,2)
36040       let tr6=4
...
39380 L3360: ! 
39400   gosub REGGL2
39420   let tbc(bank_code,1)=tbc(bank_code,1)+amt
...
41040   let tr5=amt
41060   if ivd=1 then let prd(4)+=tr5
41080   if ivd>1 and ivd<5 then let prd(ivd+3)=-tr5
41100   if ivd=15 then let prd(8)=-tr5
41120   if ivd>4 and ivd<15 and dedcode(ivd-4)=2 then let tr5=-tr5
41140   if ivd>4 and ivd<15 then let prd(ivd+4)=-tr5
41160   if ivd=16 then let prd(19)=-tr5
41180   if fp(ivd*.01)=.19 then let prd(20)=int(ivd*.01)
41200 L3770: ! 
41220   return  ! /r
42000 PRDWRITE: ! r:
42020   if pr2$="N" then goto L3850
42040   let prd(1)=val(vn$) conv L3850
42060   let prd(2)=pd
42080   let prd(3)=val(ck$) conv L3830
42100 L3830: ! 
42120   let prd(22)=ca1
...
45040 ERTN: let fnerror(cap$,err,line,act$,"NO")
...
48060     open #paymstr=8: "Name=Q:\CLmstr\PayMstr.H"&str$(cno)&",KFName=Q:\CLmstr\PayIdx1.H"&str$(cno)&",Shr",internal,input,keyed 
48080     open #unpdaloc=7: "Name=Q:\CLmstr\UnPdAloc.H"&str$(cno)&",KFName=Q:\CLmstr\Uaidx2.H"&str$(cno)&",Shr",internal,outin,keyed 
48100     open #paytrans=6: "Name=Q:\CLmstr\PayTrans.H"&str$(cno)&",KFName=Q:\CLmstr\UnPdIdx1.H"&str$(cno)&",Shr",internal,outin,keyed 
48120 CB_CU_READ: ! 
...
52192         let ivd=val(ivd$) conv ignore ! ivd$ logic added 8/12/2015 to prevent merriam wood's error here from using characters in this field
52200         if up1$="C" and gde=2 then let gde=1 ! don't allow old accrual codes mess up cash basis
52220         if ivd=0 then let ivd=pd ! kJ   10/02/06   skipping receipts w/o invoice numbers
