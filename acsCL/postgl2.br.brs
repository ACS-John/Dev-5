...
L2240: ! 
		...
		fncloseprn
L2300: ! 
		if glt=glt_print_only then goto XIT
...
REVERSE_AP: ! r: Reverse AP Entries
	...
	for j=1 to 98
		if apc(j,1)=0 then goto L3300
		if apc(j,1)=ap1 then goto L3300
	next j
L3300: ! 
	apc(j,1)=ap1
	apc(j,3)+=amt
...
L4000: form pos 34,2*c 9,c 12
	if glt><1 then goto L4040
...
EO_FUNDTR: ! 
	return  ! /r
IGNORE: continue 
! <Updateable Region: ERTN-xitable$>
ERTN: fnerror(program$,err,line,act$,xitable$)
