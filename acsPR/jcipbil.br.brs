...
00200 L200: print newpage
00210   let win=101
00220   let fnopenwin(win,09,08,16,71,cap$)
00230   print #win,fields "4,2,Cr 14,N": "Job Number:"
00240   print #win,fields "5,2,Cr 14,N": "Amount:"
00250   print #win,fields "6,2,Cr 14,N": "Date (mmddyy):"
00260   print #win,fields "7,2,Cr 14,N": "Status:"
00270   let io1$(1)="4,17,C 6,UT,N"
00280   let io1$(2)="5,17,N 10.2,UT,N"
00290   let io1$(3)="6,17,Nz 6,UT,N"
00300   let io1$(4)="7,17,N 2,UT,N"
00310   if c1=2 then print #win,fields mat io1$: jn$,mat inp
00320   if shoption=1 then print fields "17,24,C 09,B,1": "Next (F1)"
00330   if shoption=1 then print fields "17,34,C 09,B,5": "Done (F5)"
00340   if shoption=1 then print fields "17,44,C 11,B,6": "Search (F6)"
00350   if shoption=2 then print fields "17,17,C 09,B,1": "Next (F1)"
00360   if shoption=2 then print fields "17,27,C 11,B,4": "Delete (F4)"
00370   if shoption=2 then print fields "17,39,C 11,B,5": "Cancel (F5)"
00380   if shoption=2 then print fields "17,51,C 11,B,6": "Search (F6)"
00390 L390: input #win,fields mat io1$: jn$,mat inp conv L390
00400   if ce>0 then let io1$(ce)(ce1:ce2)="U": let ce=0
00410   if cmdkey>0 then goto L510 else let ce=curfld
00420   if ce<>1 then goto L450
00430   let jn$=lpad$(rtrm$(jn$),6) !:
        read #1,using L440,key=jn$: n$ nokey L600 !:
        print #win,fields "4,24,C 40,H,N": n$
00440 L440: form pos 7,c 40
00450 L450: let ce=ce+1 : if ce>udim(io1$) then let ce=1
00460 L460: let io1$(ce)=rtrm$(uprc$(io1$(ce))) : let ce1=pos(io1$(ce),"U",1) !:
        if ce1=0 then goto L450
00470   let ce2=ce1+1 : let io1$(ce)(ce1:ce1)="UC" : goto L390
00480 CONV1: if ce>0 then let io1$(ce)(ce1:ce2)="U"
00490   let ce=cnt+1
00500 ERR1: print fields "24,78,C 1": bell : goto L460
00510 L510: if cmdkey=5 then goto L730
00520   if cmdkey=6 then gosub SRCH : let win=101 !:
          print #win,fields io1$(1): k$ : let c1=3: goto L390
00530   if c1=2 and cmdkey=4 then goto L1150
00540   if inp(2)<10100 or inp(2)>123199 then let ce=3: goto ERR1
00550   if inp(1)=0 then let ce=2: goto ERR1
...
00730 L730: print newpage
00740   let win=102
00750   let fnopenwin(win,07,20,15,59,cap$)
00760   print #win,fields "9,1,C 40,R,N": " Total of Amounts Entered:"
00770   print #win,fields "9,28,N 10.2,R,N": ta
00780   let wrd2$(1)="1. Print Billing Proof List"
...
00840   next j
00850   print fields "16,27,C 25,B,5": "Exit without posting (F5)"
...
00970   print #255,using L980: "Job Cost Input Billings Proof List"
00980 L980: form skip 2,pos 10,c 60,skip 1
00990   print #255: tab(10);"Date:  ";date$;"   Time:  ";time$
01000   print #255: 
01010   print #255: "Ref #   Job #      Amount     Date     Status"
...
01050     print #255,using L1060: j,jn$,mat inp
...
01160 L1160: print newpage
01170   let win=103
01180   let fnopenwin(win,10,20,14,59,cap$)
01190   print #win,fields "4,2,C 28,N": "Reference Number to correct:"
01200   print fields "15,35,C 09,B,5": "Done (F5)"
...
01340     let b3=b3+inp(1)
01350     if inp(3)><0 then let b4=inp(3)
...
01470   print #103: newpage
01480 L1480: print newpage
01490   let win=102
01500   let fnopenwin(win,06,10,10,69,wrds$(1))
01510   let prtall=0
01520   print #win,fields "4,2,C 38,N": "Beginning Search Data (blank for all):"
01530   print fields "11,34,C 11,B,5": "Cancel (F5)"
...
01590 L1590: print newpage
01600   print fields "1,02,Cc 06,R,N": "Job No"
01610   print fields "1,09,Cc 36,R,N": "Job Name"
01620   print fields "1,46,Cc 08,R,N": "Est.Date"
01630   print fields "1,55,Cc 12,R,N": "Ct.Amount"
01640   print fields "1,68,Cc 12,R,N": "Bld.Amount"
01650   let cde=0
01660   for j=1 to 20
...
01690     let cde=1
01700     print fields str$(j+1)&",2,C 6,N": jn$
01710     print fields str$(j+1)&",9,C 36,N": n$(1:36)
01720     print fields str$(j+1)&",46,PIC(ZZ/ZZ/ZZ),N": b(1)
01730     print fields str$(j+1)&",55,N 12.2,N": b(2)
01740     print fields str$(j+1)&",68,N 12.2,N": b(3)
01750     if j>1 then goto L1790
01760     let bk=bk+1
01770     if bk>20 then let bk=1
01780     let bk$(bk)=bl$(2)(1:28)
01790 L1790: next j
01800 SREND: if j>1 then let j=j-1
01810   mat in2$(j)
01820   print fields "24,09,C 09,B,1": "Next (F1)"
01830   print fields "24,19,C 09,B,2": "Back (F2)"
01840   print fields "24,29,C 09,B,5": "Stop (F5)"
01850   print fields "24,39,C 25,R,N": "or Select Account Number:"
01860   let k$=""
...
02100   print "PROGRAM PAUSE: Type GO and press [Enter] to continue." : print "" : pause : goto ERTN_EXEC_ACT
