...
00170     x2=(x-int(x*.01)*100)*10000+int(x*.01)
00180     if int(x2*.0001)<20 then x2=x2+20000000 else x2=x2+19000000
...
00230 ! 
00240 ! 
00250   open #1: "Name="&env$('Q')&"\PRmstr\Company.h"&env$('cno')&",Shr",internal,input 
...
01170   open #1: "Name="&env$('Q')&"\PRmstr\RPMSTR.h"&env$('cno')&",KFName="&env$('Q')&"\PRmstr\RPINDEX.h"&env$('cno')&",Shr",internal,input,keyed 
01180   open #2: "Name="&env$('Q')&"\PRmstr\RPTRAIL.h"&env$('cno')&",Shr",internal,input,relative 
...
02200   win=101
02210   win_height=13: win_width=75: display_cnam=1: button_option=2: gosub L2650 ! fnWin3b(WIN,CAP$,12,62,1,2)
...
02980   if button_option=7 then !:
          fkey$(1)="Save" !:
          fkey$(4)="Delete"
02990   scrline=er+1: gosub L3660 !  fnFKEY(ER+1,MAT FKEY$,MAT DISFK,EM$,ES)
03000 ! 
...
03370 L3370: ! Def Library fnwait(&CAP$,&MESSAGE$,STOPABLE)
