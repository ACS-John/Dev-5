...
00170   let io5$(1)="11,55,N 6,U,N"
00180   let io5$(2)="12,55,N 6,U,N"
00190   print newpage
...
00220   print fields "11,18,C 40": "ENTER LOWEST  DATE OR BLANK FOR ALL:"
00230   print fields "12,18,C 40": "ENTER HIGHEST DATE OR BLANK FOR ALL:"
00240   print fields "14,22,C 34,R,N": "Press F1 to continue or F5 to stop"
00250 L250: input fields mat io5$,attr "R": prd1,prd2 conv L250
00260   if ce>0 then let io5$(ce)(ce1:ce2)="U": let ce=0
00270   if cmdkey>0 then goto L340 else let ce=curfld+1
00280   if ce>udim(io5$) then let ce=1
00290 L290: let io5$(ce)=rtrm$(uprc$(io5$(ce))) : let ce1=pos(io5$(ce),"U",1)
00300   let ce2=ce1+1 : let io5$(ce)(ce1:ce1)="UC" : goto L250
00310 CONV5: if ce>0 then let io5$(ce)(ce1:ce2)="U"
00320   let ce=cnt+1
00330 ERR5: print fields "24,78,C 1": bell : goto L290
00340 L340: ! 
...
00500   let bonus=tcp(18)
00510   let reg=tcp(21)-ot-other-bonus
00520   let accum(1)=accum(1)+reg
00530   let accum(2)=accum(2)+ot
00540   let accum(3)=accum(3)+other
00550   let accum(4)=accum(4)+bonus
00560   print #255,using L710: eno,em$,tgl(1),tgl(2),tgl(3),reg pageoflow L730
00570   let gl$=cnvrt$("N 3",tgl(1))&cnvrt$("N 6",tgl(2))&cnvrt$("N 3",tgl(3))
00580   let ga0=reg : gosub TOTGL
00590   if ot=0 then goto L630
00600   print #255,using L710: eno,em$,tgl(1),tgl(2)+1,tgl(3),ot pageoflow L730
...
00640   print #255,using L710: eno,em$,tgl(1),tgl(2)+2,tgl(3),other pageoflow L730
...
00680   print #255,using L710: eno,em$,tgl(1),tgl(2)+3,tgl(3),bonus pageoflow L730
...
00720   goto L390
00730 L730: print #255: newpage
00740   gosub L760
00750   continue 
00760 L760: let p1=p1+1
00770   print #255,using L780: date$,a$,"PAGE",p1
00780 L780: form skip 1,pos 1,c 8,pos nametab,c 40,pos 77,c 5,pic(zzz),skip 1
00790   print #255,using L800: time$,"GENERAL LEDGER DISTRIBUTION FOR PAYROLL",dat1
00800 L800: form pos 1,c 8,pos 17,c 40,skip 1,pos 29,pic(zz/zz/zz),skip 2
00810   print #255: "EMPLOYEE                                               G/L                 AMOUNT"
00820   print #255: " NUMBER        NAME                                  ACCOUNT         DEBITS     CREDITS"
00830   print #255: 
00840   return 
...
00910 L910: return 
00920 END1: print #255: " "
00930   print #255: "  GL NUMBER          TOTAL"
00940   print #255: "------------   -----------------"
00950   restore #5,key>="            ": nokey XIT
00960 L960: read #5,using L860: gl$,ga1 eof XIT
00970   print #255,using L980: gl$,ga1
