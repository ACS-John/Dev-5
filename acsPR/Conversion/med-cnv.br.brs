...
00120   let fncno(cno)
00130   print newpage
00140   print fields "10,20,c 21,n": "Company Number:"
00150 L150: rinput fields "10,41,Nz 5,UE,N": cno conv L150
00155   let fnputcno(cno)
...
00180   let mcmax=9999999
00190   for j=1 to 3: let a$(j)=a$(j)(1:30): next j
...
00260   if tcd<1 or tcd>10 then let tcd=1
00270   let dedfica=0
00280   let dedret=0
00290   for j=1 to 10
00300     if dedfed(j)>=1 and dedcode(j)=1 then let dedret+=ty(j+3)
00310 ! IF DEDFED(J)=1 AND DEDCODE(J)=1 THEN LET TDEDRET+=TY(J+3)
00320     if dedfed(j)=2 and dedcode(j)=1 then let dedfica+=ty(j+3)
00330   next j
