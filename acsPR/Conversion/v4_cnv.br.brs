...
46000 CONVERSION: ! 
46020     let tdn=tdn ! department #
46040     let prd=fndate_mmddyy_to_ccyymmdd(tdt(4)) ! payroll date to be used
46060     let newtdet(1)=tdet(1) ! salary
46080     let newtdet(2)=tdet(2) ! hourly rate
46100     let newtdet(3)=tdet(3) ! ot rate
46120     for j=1 to 10
46140       let newtdet(j+3)=tdet(j+3) ! 10 standard deductions
46160     next j
46180     let newtdc(1)=tdy(1) ! regular hours for year
46200     let newtdc(2)=tdy(2) ! ot hours
46220     let newtdc(3)=tdy(3) ! sick hours
46240     let newtdc(4)=tdy(4) ! vac hrs
46260     let newtdc(5)=tdy(5) ! holiday hrs
46280     let newtdc(6)=ty(21) ! workmans comp wage  (make all wages = total pay
46300     let newtdc(7)=ty(21) ! ss wage  ! needs help What about cafiteria
...
50120 ! Let PRD=fndate_mmddyy_to_ccyymmdd(PRD): Goto PFH_POSTREAD_PRCKHIST
...
58200   for j=1 to 10
58220     let newdedcode(j)=dedcode(j)
...
62040 ! pause
62060   let prd=fndate_mmddyy_to_ccyymmdd(prd)
...
66020     if newtdc(1)>999 then let newtdc(1)=999
...
66050     let payrollcheck_write_count+=1
66080   fnend 
