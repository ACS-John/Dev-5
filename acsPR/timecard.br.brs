...
00150   let sc1$(2)=" Date": let fl1$(2)="3,15,C 6,R,N"
00160   let sc1$(3)="Time  In": let fl1$(3)="2,28,C 8,R,N"
00170   let sc1$(4)="Time Out": let fl1$(4)="2,42,C 8,R,N"
00180   let sc1$(5)="Hrs  Min": let fl1$(5)="3,28,C 8,R,N"
00190   let sc1$(6)="Hrs  Min": let fl1$(6)="3,42,C 8,R,N"
00200   let sc1$(7)=" Hours": let fl1$(7)="3,55,C 6,R,N"
...
00350   print fields "16,35,C 09,B,5": "Exit (F5)"
00360   rinput #win,select mat io2$,attr "H": mat wrd2$
00370   if cmdkey=5 then goto XIT
00380   let ti1=curfld
00390   if ti1=2 then let sc1$(5)=sc1$(6)="Hrs  Hnd"
00400 L400: print newpage
00410   print fields mat fl1$: mat sc1$
00420   print fields "24,11,C 09,B,1": "Next (F1)"
00430   print fields "24,21,C 10,B,2": "Print (F2)"
00440   print fields "24,32,C 26,B,3": "Skip to Next Employee (F3)"
00450   print fields "24,59,C 09,B,5": "Stop (F5)"
00460 L460: input fields mat io1$: eno,mat inp conv CONV1
00470   if ce>0 then let io1$(ce)(ce1:ce2)="U": let ce=0
00480   if cmdkey>0 then goto L620 else let ce=curfld
00490   if ce>1 then goto L540
00500   read #1,using L510,key=lpad$(str$(eno),8): em$ nokey ERR1
00510 L510: form pos 9,c 30
00520   print fields "1,35,C 30,R,N": em$
00530   goto L550
00540 L540: if int((ce-1)/5)=(ce-1)/5 then goto L650
00550 L550: let ce=ce+1
00560   if ce>udim(io1$) then let ce=1
00570 L570: let io1$(ce)=rtrm$(uprc$(io1$(ce))) : let ce1=pos(io1$(ce),"U",1)
00580   let ce2=ce1+1 : let io1$(ce)(ce1:ce1)="UC" : goto L460
00590 CONV1: if ce>0 then let io1$(ce)(ce1:ce2)="U"
00600   let ce=cnt+1
00610 ERR1: print fields "24,78,C 1": bell : goto L570
00620 L620: if cmdkey=2 then goto L860
00630   if cmdkey=3 then goto L1080
00640   if cmdkey=5 or eno=0 then goto END1
00650 L650: let ln=currow
00660   let cn=curcol
...
00700   if inp(ln-3,2)<1 or inp(ln-3,2)>12 then let ce=ln*5-17: goto ERR1
00710   if inp(ln-3,3)<0 or inp(ln-3,3)>59 then let ce=ln*5-16: goto ERR1
00720   if inp(ln-3,4)<1 or inp(ln-3,4)>12 then let ce=ln*5-15: goto ERR1
00730   if inp(ln-3,5)<0 or inp(ln-3,5)>59 then let ce=ln*5-14: goto ERR1
00740   if inp(ln-3,1)<10101 or inp(ln-3,1)>123199 then let ce=ln*5-18: goto ERR1
00750   if hrs(ln-3)>12 then let ce=ln*5-15: goto ERR1
00760   print fields str$(ln)&",55,N 6.2,H,N": hrs(ln-3)
00770   let ce=ln*5-12
00780   let ce1=pos(io1$(ce),"U",1)
00790   let ce2=ce1+1
00800   let io1$(ce)(ce1:ce1)="CU"
00810   if inp(ln-3,4)>2 and inp(ln-3,4)<8 then let dt=inp(ln-3,1)+100 else let dt=inp(ln-3,1)
00820   if ln<23 then print fields str$(ln+1)&",15,N 6,UT,N": dt
00830   print fields "2,55,N 6.2,R,N": sum(hrs)
00840   goto L460
...
00880   print #255,using L890: mat sc1$
00890 L890: form pos 1,4*c 14,skip 1,pos 29,3*c 14
00900   let t1=t2=0
00910   for j=1 to 20
00920     if j>1 then let eno=0
00930     if hrs(j)=0 then goto L1030
00940     print #255,using L950: eno,inp(j,1),inp(j,2),inp(j,3),inp(j,4),inp(j,5),hrs(j)
...
01190   print "PROGRAM PAUSE: Type GO and press [Enter] to continue." : print "" : pause : goto ERTN_EXEC_ACT
