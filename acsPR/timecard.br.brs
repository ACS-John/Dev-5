...
00210   io1$(1)="1,21,N 9,UET,N"
00220   for j=2 to 21
00230     io1$(j*5-8)=str$(j+2)&",15,N 6,UT,N"
00240     io1$(j*5-7)=str$(j+2)&",28,N 2,UT,N"
00250     io1$(j*5-6)=str$(j+2)&",34,N 2,UT,N"
...
00470   if ce>0 then io1$(ce)(ce1:ce2)="U": ce=0
...
00570 L570: io1$(ce)=rtrm$(uprc$(io1$(ce))) : ce1=pos(io1$(ce),"U",1)
00580   ce2=ce1+1 : io1$(ce)(ce1:ce1)="UC" : goto L460
...
00680   if ti1=1 then hrs(ln-3)=round((inp(ln-3,4)+(inp(ln-3,5)/60))-(inp(ln-3,2)+(inp(ln-3,3)/60)),2) else hrs(ln-3)=inp(ln-3,4)+(inp(ln-3,5)*.01)-inp(ln-3,2)-(inp(ln-3,3)*.01)
00690   if hrs(ln-3)<0 then hrs(ln-3)=hrs(ln-3)+12
...
00790   ce2=ce1+1
00800   io1$(ce)(ce1:ce1)="CU"
00810   if inp(ln-3,4)>2 and inp(ln-3,4)<8 then dt=inp(ln-3,1)+100 else dt=inp(ln-3,1)
