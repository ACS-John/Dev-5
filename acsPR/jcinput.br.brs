...
00620   print fields "15,35,C 09,B,5": "Exit (F5)"
...
00840   print #win,fields mat iolabel2$: mat label2$
00850   print #win,fields "5,31,C 28,N": "(1.Salary, 2.Hourly, 3.Both)"
00860   print #win,fields "14,32,C 40,N": "(0.Regular, 1-10.Deduct #, 11.Extra Pay)"
00870   if shoption=1 then print fields "21,25,C 09,B,1": "Next (F1)"
00880   if shoption=1 then print fields "21,35,C 09,B,5": "Stop (F5)"
00890   if shoption=1 then print fields "21,45,C 09,B,6": "Help (F6)"
00900   if shoption=2 then print fields "21,19,C 09,B,1": "Next (F1)"
00910   if shoption=2 then print fields "21,29,C 11,B,2": "Delete (F2)"
00920   if shoption=2 then print fields "21,41,C 09,B,5": "Stop (F5)"
00930   if shoption=2 then print fields "21,51,C 09,B,6": "Help (F6)"
00940   if c1=2 or jcsrch=1 then print #win,fields mat io2$: mat ji1,jn$,mat ji2
00950   let jcsrch=0
00960   if ji1(1)=0 then let io2$(4)="7,29,N 3,UT,N" : goto L1000
00970 L970: print #win,fields mat io2$: ji1(1),ji1(2),ji1(3)
...
01010   let ck1=cmdkey
01020   let ji1(1)=val(en$) conv L1040
01030   goto L1070
01040 L1040: let ck1=6
01050   let ce4=1 : let nam$=uprc$(rtrm$(en$))
01060   goto L2870
01070 L1070: if ce>0 then let io2$(ce)(ce1:ce2)="U": let ce=0
01080   let ce4=curfld
01090   if ck1>0 then goto L1230 else let ce=curfld
01100   if ce<>1 then goto L1130
01110 L1110: let en$=lpad$(str$(ji1(1)),8)
01120   read #1,using L1390,key=en$: empnam$,ta1 nokey ERR1 !:
        print #win,fields "4,38,C 30,N": empnam$
01130 L1130: if ce<>7 then goto L1150
01140   read #11,using L1500,key=lpad$(rtrm$(jn$),6): n$ nokey ERR1 !:
        print #win,fields "10,36,C 30,N": n$(1:30)
01150 L1150: if ce<>9 then goto L1170
01160   read #13,using L1590,key=lpad$(str$(ji2(2)),3): sub$ nokey ERR1 !:
        print #win,fields "12,32,C 30,N": sub$
01170 L1170: let ce=ce+1: if ce>udim(io2$) then let ce=1
...
01210   let ce=cnt+1
01220 ERR1: print fields "24,78,C 1": bell : goto L1180
...
01280   if ck1><1 then let ce=curfld : goto L1180
01290   let ptp=0
01300   mat hr=(0)
01310   let er=0
01320   if ji1(2)<1 or ji1(2)>3 then let ce=2: goto ERR1
01330   if ji1(3)<10100 or ji1(3)>123199 then let ce=3: goto ERR1
...
01430   if nta=0 then let ce=4: goto ERR1
01440   let ta1=nta
01450   goto L1400
...
01510   let ce=8
01520   let cn$=lpad$(rtrm$(jn$),6)&lpad$(str$(ji2(1)),5)
...
01600   let ce=0
01610 L1610: if ji2(3)=0 then let ce=10: goto ERR1
...
01790 L1790: print newpage
01800   let win=104
01810   let fnopenwin(win,10,20,14,59,cap$)
01820   print #win,fields "04,2,C 28,N": "Reference Number to correct:"
01830   print fields "15,35,C 09,B,5": "Done (F5)"
...
01950   let wrd3$(1)="1. Print Input Proof List"
...
02010   next j
02020   print fields "16,27,C 25,B,5": "Exit Without Posting (F5)"
...
02180   print #255,using L2210: cnam$
02190   print #255,using L2210: "Job Cost Input Proof List"
02200   print #255,using L2210: "Date: "&date$&"      Time: "&time$
02210 L2210: form pos 1,cc 113,skip 1
02220   print #255: "Ref #   Emp #  Method-Pay  Date   Dept  Reg-Hrs   OT-Hrs  Job #   Category  Sub-Category   Amount  Ded-Add  Units"
02230   return 
...
02300     print #255,using L2310: " ________"," ________"," ____________",t5,t6,t10 pageoflow PROOF_LIST_NWPG
02310 L2310: form pos 38,2*c 9,x 29,c 13,skip 1,pos 8,"Total",pos 38,2*n 9.2,x 29,n 13.2,skip 2
02320 L2320: let en=ji1(1)
02330     let t5=0
02340     let t6=0
02350     let t10=0
02360 L2360: print #255,using L2370: j,mat ji1,jn$,mat ji2 pageoflow PROOF_LIST_NWPG
...
02440   next j
02450   print #255,using L2310: " ________"," ________"," ____________",t5,t6,t10
02460   print #255,using L2470: " ________"," ________"," ____________",gt5,gt6,gt10
...
02710   print "PROGRAM PAUSE: Type GO and press [Enter] to continue." : print "" : pause : goto ERTN_EXEC_ACT
...
02820   let prtall=0
02830   print fields "7,9,C 45,H,N": "Employee Name (blank for all):"
02840   print fields "9,34,C 11,B,5": "Cancel (F5)"
...
03220 L3220: print newpage ! INPUT FROM DISKETTE FILE
03230   let win=102
03240   let fnopenwin(win,10,20,15,59,cap$)
03250   let io2b$(1)="8,42,CU 1,UET,N"
03260   let io2b$(2)="10,43,Nz 6,UET,N"
03270   print #win,fields "4,2,Cr 26,N": "Diskette Drive:"
03280   print #win,fields "5,2,Cr 26,N": "Transaction Date (mmddyy):"
03290   let io2b$(1)="4,29,Cu 1,UT,N"
03300   let io2b$(2)="5,29,Nz 6,UT,N"
03310   print fields "16,30,C 09,B,1": "Next (F1)"
03320   print fields "16,41,C 11,B,5": "Cancel (F5)"
03330   let dv$="A"
03340 L3340: rinput #win,fields mat io2b$: dv$,td1 conv CONV2
03350   if ce>0 then let io2b$(ce)(ce1:ce2)="U": let ce=0
03360   if cmdkey>0 then goto L3430 else let ce=curfld
03370 L3370: let ce=ce+1: if ce>udim(io2b$) then let ce=1
03380 L3380: let io2b$(ce)=rtrm$(io2b$(ce)) !:
        let ce1=pos(io2b$(ce),"U",9) !:
        if ce1=0 then goto L3370
03390   let ce2=ce1+1 : let io2b$(ce)(ce1:ce1)="UC" : goto L3340
03400 CONV2: if ce>0 then let io2b$(ce)(ce1:ce2)="U"
03410   let ce=cnt+1
03420 ERR2: print fields "24,78,C 1": bell : goto L3380
03430 L3430: if cmdkey=5 then goto SCR1
03440   if dv$="A" or dv$="B" then goto L3450 else let ce=1: goto ERR2
03450 L3450: if td1<10100 or td1>123199 then let ce=2: goto ERR2
...
03890 SRCH: let bk=0 : print newpage
...
03960   print #101: newpage
03970   print fields mat ios$: mat wrds$
03980   print fields str$(udim(ios$)+14)&",32,C 16,R,N": "PRESS F5 TO STOP"
03990   input select mat ios$: mat wrds$
04000   let seq=curfld
04010   if cmdkey=5 then goto SRCHEND
04020 L4020: print newpage
04030   close #101: ioerr L4040
04040 L4040: open #101: "SROW=6,SCOL=10,EROW=10,ECOL=69,BORDER=DR,CAPTION="&wrds$(seq),display,outin 
04050   let prtall=0
04060   print fields "7,11,C 22,N": "Beginning Search Data:"
04070   print fields "9,11,C 58,H,N": "NOTE: Enter Beginning Search Data as blank for all records"
04080   print fields "11,32,C 16,R,N": "PRESS F5 TO STOP"
...
04150 L4150: print newpage
04160   print fields "1,2,C 6,R,N": "JOB #:"
04170   print fields "1,9,C 40,R,N": "JOB NAME"
04180   print fields "1,50,C 8,R,N": "EST.Date"
04190   print fields "1,59,C 10,R,N": "EST.Date"
04200   print fields "1,59,C 10,R,N": " CT.AMOUNT"
04210   print fields "1,70,C 10,R,N": "BLD.AMOUNT"
04220   let cde=0
04230   for j=1 to 20
04240     read #14,using L4250: jn$,n$,mat a$,mat b eof SREND
04250 L4250: form pos 1,c 6,c 40,3*c 30,n 6,2*pd 7.2,n 2
04260     let cde=1
04270     print fields str$(j+1)&",2,C 6,UT,N": jn$
04280     print fields str$(j+1)&",9,C 40,UT,N": n$
04290     print fields str$(j+1)&",50,PIC(ZZ/ZZ/ZZ),UT,N": b(1)
04300     print fields str$(j+1)&",59,N 10.2,UT,N": b(2)
04310     print fields str$(j+1)&",70,N 10.2,UT,N": b(3)
04320     if j>1 then goto L4360
04330     let bk=bk+1
04340     if bk>20 then let bk=1
04350     let bk$(bk)=bl$(2)(1:28)
04360 L4360: next j
04370 SREND: if j>1 then let j=j-1
04380   mat in2$(j)
04390   print fields "24,02,C 67,R,N": "ENTER TO CONTINUE; F2 TO BACKUP; F5 TO STOP; Select ACCOUNT Number:"
04400 L4400: input fields "24,70,C 6,RE,N": k$
04410   let alp=0
04420   if cmdkey=5 then goto SRCHEND
04430   if rtrm$(k$)><"" then let bl$=k$ : goto SRCHEND
...
04520   goto L4020
04530 SRCHEND: if rtrm$(k$)="" or len(rtrm$(k$))>6 then let alp=0: goto L4580
