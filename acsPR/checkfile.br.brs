...
00130     let transtype$(4)="Quarterly Totals" 
00132     let transtype$(5)="Annual Totals"
00140 ! 
...
00250     mat hf=(1)
00270     for j=1 to 20
00280       if trim$(dednames$(j))="" then let hf(j+25)=0 ! default the (All) to only those deductions that have names
00290       let dednames$(j)=trim$(dednames$(j))
00300     next j
...
00370     let justopen=1: gosub SELECT_COLUMNS: let justopen=0
...
00440     let holdeno=holdckno=holdtdn=holdprd=eno=prd=ckno=tdn=0
...
01250 L1250: let fnlbl(lc+=1,38,dednames$(j)&" ",mylen,1) : goto L1270
01260 L1260: let fnlbl(lc+=1,38,dednames$(j)&":",mylen,1)
...
01430     let tdc(4)=val(resp$(8)) ! vac hrs
01440     let tdc(5)=val(resp$(9)) ! hol hrs
01450     let tcp(26)=val(resp$(10)) ! reg pay
01460     let tcp(27)=val(resp$(11)) ! ot pay
01470     let tcp(28)=val(resp$(12)) ! other comp
01480     let tcp(29)=val(resp$(13)) ! Meals
01490     let tcp(30)=val(resp$(14)) ! Tips
01500     let tcp(31)=val(resp$(15)) ! Total Pay
01510     let tcp(32)=val(resp$(16)) ! Net pay
01520     let tdc(6)=val(resp$(17)) ! Workman comp wages
01530     let tdc(7)=val(resp$(18)) ! ss wages
01540     let tdc(8)=val(resp$(19)) ! Medicare wages
01550     let tdc(9)=val(resp$(20)) ! Federal uc
01560     let tdc(10)=val(resp$(21)) ! state uc
01570     let tcp(1)=val(resp$(22)) ! fed wh
01580     let tcp(2)=val(resp$(23)) ! ss  wh
01590     let tcp(3)=val(resp$(24)) ! med wh
01600     let tcp(4)=val(resp$(25)) ! state wh
01610     for j=1 to 20
...
02340 ! Let FNCMDKEY('&OK',5,0,1)
02350 ! Let FNACS(SN$,0,MAT RESP$,CKEY)
02360 ! Goto SCREEN1
02370 L2370: if ckey=3 then gosub SELECT_COLUMNS: goto ASKTRANSET
02380     let justopen=1: gosub SELECT_COLUMNS: let justopen=0
02390     goto L470
...
02470     let name$(1)="Emp #" : let name$(2)="Dept" !:
          let name$(3)="Date" !:
          let name$(4)="Check #" : let name$(5)="Reg Hrs" !:
          let name$(6)="OT Hrs" : let name$(7)="Sick Hrs" !:
          let name$(8)="Vac Hrs" : let name$(9)="Hol Hrs"
02480     let name$(10)="Reg Pay" : let name$(11)="OT Pay" !:
          let name$(12)="Other Pay" : let name$(13)="Meals" !:
          let name$(14)="Tips" : let name$(15)="Total Pay" !:
          let name$(16)="Net Pay"
02490     let name$(17)="W/C Wage" : let name$(18)="SS Wage" !:
          let name$(19)="Med Wage" : let name$(20)="Fed UC Wage" !:
          let name$(21)="St U/C Wage": let name$(22)="Fed Wh" !:
          let name$(23)="SS Wh" : let name$(24)="Med Wh" !:
          let name$(25)="St Wh"
02500     for j=1 to 20
02510       let name$(j+25)=dednames$(j)(1:11)
02520     next j
02530     let name$(46)="EIC"
...
02920     next j
02930 L2930: let hfm$="FORM POS 1,c 12"
02940     let ul$=hd$="            "
02950     let hs1=0: let hs2=0
02960     for j=1 to udim(hf$)
02970       if hf(j)=0 then goto L3070
02980       let hs2=hs2+1
02990       if j=1 then let hfm$=hfm$&",NZ 8" : let hs1=hs1+8 : let hz1=hs2 !:
              let ul$=ul$&" -------": let hs3=8 ! employee #
03000       if j=2 then let hfm$=hfm$&",NZ 5" : let hs1=hs1+5 : let hz1=hs2 !:
              let ul$=ul$&" ----": let hs3=5 ! dept #
03010       if j=3 then let hfm$=hfm$&",pic(bzzzz/zz/zz)" : let hs1=hs1+11 !:
              let hz1=hs2 !:
              let ul$=ul$&" ----------": let hs3=11 ! date
03020       if j=4 then let hfm$=hfm$&",NZ 7" : let hs1=hs1+7 : let hz1=hs2 !:
              let ul$=ul$&" ------": let hs3=7 ! check number
03030       if j>4 and j<10 then let hfm$=hfm$&",G 8.2" : let hs1=hs1+8 !:
              let ul$=ul$&" -------" : let hs3=8 ! hours
03040       if j>9 and j<17 then let hfm$=hfm$&",G 10.2" : let hs1=hs1+10 !:
              let ul$=ul$&" ---------" : let hs3=10 ! wages
03050       if j>16 and j<47 then let hfm$=hfm$&",G 10.2" : let hs1=hs1+10 !:
              let ul$=ul$&" ---------" : let hs3=10 ! deductions
03060       let hd$=hd$&lpad$(trim$(name$(j)(1:hs3-1)),hs3)
03070 L3070: next j
03080     mat cp0(hs2)
03090     mat cp1(hs2)
03100     mat cp2(hs2)
...
04430     if prd=>qtr2 and qtr1printed=0 and sum(totaltdc)+sum(totaltcp)<>0 then gosub PRINT_GRID : let holdckno=0 ! last check not printed yet
04440     if prd=>qtr2 and qtr1printed=0 and sum(qtr1tdc)+sum(qtr1tcp)>0 then mat totaltdc=qtr1tdc: mat totaltcp=qtr1tcp: let qtr1printed=1: let desc$="1st Qtr" : gosub PRINT_GRID : let holdckno=0
04450     if prd=>qtr3 and qtr2printed=0 and sum(totaltdc)+sum(totaltcp)<>0 then gosub PRINT_GRID ! last check not printed yet
04460     if prd=>qtr3 and qtr2printed=0 and sum(qtr2tdc)+sum(qtr2tcp)>0 then mat totaltdc=qtr2tdc: mat totaltcp=qtr2tcp: let qtr2printed=1: let desc$="2nd Qtr" : gosub PRINT_GRID : let holdckno=0
04470     if prd=>qtr4 and qtr3printed=0 and sum(totaltdc)+sum(totaltcp)<>0 then gosub PRINT_GRID ! last check not printed yet
04480     if prd=>qtr4 and qtr3printed=0 and sum(qtr3tdc)+sum(qtr3tcp)>0 then mat totaltdc=qtr3tdc: mat totaltcp=qtr3tcp: let qtr3printed=1: let desc$="3rd Qtr": gosub PRINT_GRID: let holdckno=0
04490     if prd=>qtr5 and qtr4printed=0 and sum(totaltdc)+sum(totaltcp)<>0 then gosub PRINT_GRID ! last check not printed yet
04500     if prd>qtr5 and qtr4printed=0 and sum(qtr4tdc)+sum(qtr4tcp)>0 then mat totaltdc=qtr4tdc: mat totaltcp=qtr4tcp: let qtr4printed=1: let desc$="4th Qtr": gosub PRINT_GRID : goto CONSIDER_ANNUAL
...
04680 L4680: let item$(1)=cnvrt$("pic(zzzzzzz)",recnum) : : let item$(2)=desc$ !:
          let item$(3)=cnvrt$("pic(zzzzzzzz)",enoprint) !:
          let item$(4)=cnvrt$("pic(zzz)",tdnprint): let item$(5)=str$(prdprint) !:
          let item$(6)=cnvrt$("pic(zzzzzzz)",cknoprint)
04690 L4690: let item$(7)=str$(totaltdc(1)): let item$(8)=str$(totaltdc(2)) !:
          let item$(9)=str$(totaltdc(3)): let item$(10)=str$(totaltdc(4)) !:
          let item$(11)=str$(totaltdc(5)): let item$(12)=str$(totaltcp(26)) !:
          let item$(13)=str$(totaltcp(27)): let item$(14)=str$(totaltcp(28)) !:
          let item$(15)=str$(totaltcp(29)): let item$(16)=str$(totaltcp(30))
04700     let item$(17)=str$(totaltcp(31)): let item$(18)=str$(totaltcp(32)) !:
          let item$(19)=str$(totaltdc(6)) !:
          let item$(20)=str$(totaltdc(7)): let item$(21)=str$(totaltdc(8)) !:
          let item$(22)=str$(totaltdc(9)): let item$(23)=str$(totaltdc(10)) !:
          let item$(24)=str$(totaltcp(1)): let item$(25)=str$(totaltcp(2)) !:
          let item$(26)=str$(totaltcp(3)): let item$(27)=str$(totaltcp(4))
04710     let items=27
04720     for j=1 to 20
04730       let item$(items+=1)=cnvrt$("pic(-------.zz)",totaltcp(j+4))
04740     next j
04750     let item$(items+=1)=str$(totaltcp(25)) ! eic
04760     if printit=1 then let desc$=item$(2): gosub PRINT_DETAILS: goto L4840
04770     let x=2
04780     let printitem$(1)=item$(1): let printitem$(2)=item$(2)
04790     for j=1 to 46
04800       if hf(j)=1 then let printitem$(x+=1)=item$(j+2)
04810     next j
04820 L4820: let fnflexadd1(mat printitem$)
04830     let holdeno=eno !:
          let holdckno=ckno !:
          let holdtdn=tdn !:
          let holdprd=prd
04840 L4840: if repeatit=1 then let repeatit=0: goto L4860
04850     if trim$(desc$)="1st Qtr" or trim$(desc$)="2nd Qtr" or trim$(desc$)="3rd Qtr" or trim$(desc$)="4th Qtr" then !:
            mat item$=(""): let repeatit=1 !:
            goto L4820
04860 L4860: if trim$(printitem$(2))="Employee Total" then !:
            mat printitem$=(""): let fnflexadd1(mat printitem$)
...
04990     if sum(totaltdc)+sum(totaltcp)<>0 and checkonly=1 then let desc$="Total Ck"
05000     if (sum(qtr1tdc)>0 or sum(qtr1tcp)>0) and qtr1printed=0 then !:
            mat totaltdc=qtr1tdc: mat totaltcp=qtr1tcp !:
            let qtr1printed=1: let desc$="1st Qtr": let holdnam$="": gosub PRINT_GRID
05010     if (sum(qtr2tdc)>0 or sum(qtr2tcp)>0) and qtr2printed=0 then !:
            ...
            let desc$="2nd Qtr": let holdnam$="": gosub PRINT_GRID
05020     if (sum(qtr3tdc)>0 or sum(qtr3tcp)>0) and qtr3printed=0 then !:
            ...
            let desc$="3rd Qtr": let holdnam$="" : gosub PRINT_GRID
...
05390     mat resp$=(""): let resp$(1)=gridname$
05400     goto L2620
05410 DONE: ! 
05420 XIT: ! 
