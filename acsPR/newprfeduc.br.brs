...
00370   let fntxt(4,30,3,0,0,"30",0,"Reduce the top margin to move the print up. Increse to move down.") !:
        let resp$(respc+=1)=str$(5)
00380   let fnlbl(5,1,"Left Margin:",29,1)
00390   let fntxt(5,30,3,0,0,"30",0,"Reduce the left margin to move the print left. Increse to move right.") !:
        let resp$(respc+=1)=str$(5)
...
00480   let fnfra(12,1,2,45,"Option for printing","The system can print the actual form or just fill in the blanks on a pre-printed form.",0)
...
00530   let beg_date=val(resp$(1)) ! beginning of year
...
00630   if resp$(10)="True" then let fullform=1 ! print full form
...
00760   let checkkey$=cnvrt$("pic(zzzzzzz#)",eno)&cnvrt$("pic(zz#)",0)&cnvrt$("pd 6",0) ! index employee#,department# and payroll date
...
00900   goto L720
00910 HDR: ! 
00920   print #255,using 'Form POS 20,Cc 40,Cr 20': cap$,"Page "&str$(p2+=1) !:
        print #255,using 'Form POS 20,CC 40': "For year ending "&cnvrt$("pic(zzzz/zz/zz)",end_date) !:
        print #255: ""
00930   print #255,using L940: "     Rate",a$(1),"Fed ID",b$(1)
00940 L940: form pos 1,c 9,pos 17,c 40,pos 59,c 6,pos 69,c 40
00950   print #255,using 'Form POS 3,PIC(ZZZZ.##),POS 17,C 40': feducrat,a$(2)
00960   print #255,using 'Form POS 17,C 40': a$(3)
00970   print #255: ""
00980   print #255: tab(44);"Total Wages   Excess Wages    Taxable"
00990   print #255: " SS Number             Name";
01000   print #255,using L1010: "   For Year   Over $",feducmax,"Wages"
01010 L1010: form pos 44,c 20,pic(zzzzz.##),pos 75,c 5
01020   print #255: "___________  __________________________";tab(44);"___________   ____________    _______"
01030   return 
...
01190   print #255,using L1200: ss$,em$(1)(1:28),m2,max(m2-feducmax,0),tw
...
01220   print #255: "" pageoflow PGOF
01230   return 
01240 ! ______________________________________________________________________
01250 TOTALS: ! 
01260   print #255: tab(44);"___________    ___________  _________"
01270   print #255,using L1280: "Grand Totals",t1,t2,t3
01280 L1280: form pos 28,c 12,pos 42,pic(--,---,---.##),pos 57,pic(--,---,---.##),pos 70,pic(----,---.##)
01290   print #255: ""
01300   print #255,using L1310: "Total Futa Tax",t3*feducrat/100
...
01350 PGOF: print #255: newpage : gosub HDR : continue 
...
01410   print "PROGRAM PAUSE: Type GO and press [Enter] to continue." : print "" : pause : goto ERTN_EXEC_ACT
...
01600     goto L1620
01610 L1610: let b$(1)(j:j)=""
01620 L1620: if b$(1)(j:j)=" " then let b$(1)(j:j)=""
01630     if b$(1)(j:j)="-" then let b$(1)(j:j)=""
01640   next j
...
02200   if p1>0 then let csz$(p1:p1)=" ": let p2=p1: goto L2190
02210 ! IF P2>0 AND CSZ$(P2+1:P2+1)=" " THEN LET CSZ$(P2+1:P2+1)="" ! dump any extra spaces
02220   let p2=0
02230 L2230: let p1=pos(csz$,",",1)
02240   if p1>0 then let csz$(p1:p1)=" ": let p2=p1: goto L2230
02250 ! IF P2>0 AND CSZ$(P2+1:P2+1)=" " THEN LET CSZ$(P2+1:P2+1)="" ! dump any extra spaces
02260 L2260: let p1=pos(rtrm$(csz$),"  ",1)
02270   if p1>0 then let csz$(p1+1:p1+1)="" : goto L2260
02280   let csz$=ltrm$(rtrm$(csz$)): let p1=pos(csz$," ",-1)
...
02310   let city$=csz$(1:p2-1)(1:15): let city$=ltrm$(rtrm$(city$))
02320   return 
