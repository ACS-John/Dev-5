...
00270   let fntxt(2,23,10,0,1,"3",0,"You can print or reprint for any pay period.  Normally you would use the last payroll date.")
...
00550   let a=pos (rtrm$(em$)," ",1)
00560   let b=pos (rtrm$(em$)," ",a+1)
...
00600   let checkkey$=cnvrt$("pic(ZZZZZZZ#)",eno)&"         "
...
00970 L970: let printline1(x+=1)=rptemp(j+4) ! set variables to print line
00980     goto L1000
00990 L990: let printline2(z+=1)=rptemp(j+4) ! set line two
01000 L1000: next j
01010   mat printline1(numberded1): mat printline2(numberded2)
01020   if int(numberded1/2)=numberded1/2 then print #255,using L1040: eno,em$(1:15),rptemp(1),rptemp(2),rptemp(3),rptemp(4),rpxxxx,mat printline1,tottemp pageoflow PGOF else print #255,using L1050: eno,em$(1:15),rptemp(1),rptemp(2),rptemp(3),rptemp(4),rpxxxx,mat printline1,tottemp pageoflow PGOF
01030   print #255,using L1060: mat printline2 pageoflow PGOF
...
01130   return  ! /r
01150 FINALTOTALS: ! r:
01160   print #255,using 'form skip 2,pos 10,c 12,skip 2': "Final Totals"
...
01260     print #255,using L1270: name$(j),rptemp(j)
01270 L1270: form pos 5,c 8,pos 12,pic(---,---,---.##),skip 1
01280     goto L1370
01290 L1290: if trim$(abbrevname$(j-5))="" then goto L1310 ! skip ded if not used
01300     print #255,using L1270: rtrm$(abbrevname$(j-5)),rptemp(j-1)
01310 L1310: goto L1370
01320 L1320: print #255,using L1270: name$(j),rpxxxx
01330     goto L1370
01340 L1340: print #255,using L1270: name$(j),totaleic
01350     goto L1370
01360 L1360: print #255,using L1270: name$(j),tottemp
01370 L1370: next j
01380   goto DONE ! /r
01400 DONE: ! r:
...
01460 HDR: ! r:
01470 ! Print #255,Using 1380: DATE$,TIME$,CAP$
01480   print #255,using "form pos 1,c 25": "Page "&str$(pgno+=1)&" "&date$
01490   print #255: "\qc  {\f221 \fs22 \b "&env$('cnam')&"}"
01500   print #255: "\qc  {\f201 \fs20 \b "&env$('program_caption')&"}"
01510   print #255: "\qc  {\f181 \fs16 \b Payroll Date: "&cnvrt$("pic(zzzz/zz/zz)",ppd)&"}"
01520   print #255: "\ql   "
01530 ! 
01540   form skip 3,pos 1,c 8,pos namtab,c 40,skip 1,pos 1,c 8,pos 49,c 33,skip 1
01550   print #255,using L1560: dat$
01560 L1560: form pos dattab,c 20,skip 2
01570   print #255,using L1580: "<------------Other Pay------------>","     Other Deductions-->"
...
01600   print #255,using L1610: "Number","Name",name$(1),name$(2),name$(3),name$(4),name$(5),mat dedname$,total$
...
01680   print "PROGRAM PAUSE: Type GO and press [Enter] to continue." : print "" : pause : goto ERTN_EXEC_ACT
