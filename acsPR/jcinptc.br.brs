...
00590   if ce>0 then io1$(ce)(ce1:ce2)="U": ce=0
...
00620 L620: read #11,using L900,key=lpad$(rtrm$(jn$),6): n$ nokey L630 !:
        ...
        goto L640
00630 L630: msgline$(1)="Job Number not on file." !:
        msgline$(2)="Please select a different Job Number" !:
        fnoldmsgbox(mat response$,cap$,mat msgline$,1) !:
        ce=3 !:
        goto ERR1
00640 L640: if ce<>4 then goto L690
00650   jn$=lpad$(rtrm$(jn$),6)
...
00670   read #12,using L2700,key=cat$: jn2$,cn$,cnt$ nokey L680 !:
        ...
        goto L690
00680 L680: msgline$(1)="Invalid Category Number" !:
        msgline$(2)="Please select a different Category Number" !:
        fnoldmsgbox(mat response$,cap$,mat msgline$,1) !:
        ce=4 !:
        goto ERR1
...
00700   read #13,using L1020,key=lpad$(str$(ji2(2)),3): ds$ nokey L710 !:
        ...
        goto L720
00710 L710: msgline$(1)="Invalid Sub-Category Number" !:
        msgline$(2)="Please select a different Sub-Category Number" !:
        fnoldmsgbox(mat response$,cap$,mat msgline$,1) !:
        ce=5 !:
        goto ERR1
00720 L720: ce=ce+1: if ce>udim(io1$) then ce=1
00730 L730: io1$(ce)=rtrm$(io1$(ce)) !:
        ce1=pos(io1$(ce),"U",1) !:
        if ce1=0 then goto L640
00740   ce2=ce1+1 : io1$(ce)(ce1:ce1)="UC" : goto L580
00750 CONV1: if ce>0 then io1$(ce)(ce1:ce2)="U"
00760   ce=cnt+1
...
02090   if seq=11 then nam$=lpad$(rtrm$(nam$),skl(1))
