...
! pr User Designed Reports
	fn_setup
	fnTop(program$,cap$="Print Designed Reports")
OPEN3: !
...
def fn_print_designed_report(rn)
		...
		open #h_prreport:=fngethandle: "Name=[Q]\PRmstr\PRReport.h[cno],KFName=[Q]\PRmstr\PRRptIdx.h[cno],Shr",internal,input,keyed ioerr PDR_XIT
		...
		open #h_tmp_dr:=fngethandle: "Name=[Q]\PRmstr\Tmp_Designed_Report-"&session$&"-brs.h[cno],RecL=255",display,output
...
! pF$=RTRM$(PF$)&TY$(A(J),1)&"," : pFD$=RTRM$(PFD$)&TY$(A(J),4)&","
			pf$=rtrm$(pf$)&ty$(a(j),1) : pfd$=rtrm$(pfd$)&ty$(a(j),4)&","
			goto L1630
L1620: pf$=rtrm$(pf$)&ty$(a(j),1) : pfd$=rtrm$(pfd$)&ty$(a(j),1)
L1630: !
			af$=rtrm$(af$)&",POS "&str$(pp(j))&ty$(a(j),3)
			if tdep=1 and a(j)>29 then goto L1660
			if ti(j)><1 then goto L1770
L1660: !
			lf1=len(rtrm$(ty$(a(j),1)))-1 : lf2=len(rtrm$(ty$(a(j),4)))
			if ti(j)><1 then goto L1700
			if a(j)=2 or a(j)=3 or a(j)=4 or a(j)=5 or a(j)=6 or a(j)=26 or a(j)=27 or a(j)=62 or a(j)=63 then goto L1720
...
L1720: !
			if ti(j)><1 then goto L1770
			if a(j)=2 or a(j)=3 or a(j)=4 or a(j)=5 or a(j)=6 or a(j)=26 or a(j)=27 or a(j)=62 or a(j)=63 then goto L1760
