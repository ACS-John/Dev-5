...
00260 MAIN: ! 
00270   let fntos(sn$="autoproc") !:
        let respc=0
00280   let fnlbl(1,20,"Select From the Following Companies:",50,1)
00290   let fnlbl(2,1,"Company:",10,1)
...
00400   next j
00410   let fnfra(22,1,4,30,"Period to Print","Select the type of processing.")
00420   let fnopt(1,3,"Weekly",0,1) !:
        if wmq=1 then let resp$(respc+=1)="True" else let resp$(respc+=1)="False"
00430   let fnopt(2,3,"Monthly",0,1) !:
        if wmq=2 then let resp$(respc+=1)="True" else let resp$(respc+=1)="False"
00440   let fnopt(3,3,"Quarterly",0,1) !:
        if wmq=3 then let resp$(respc+=1)="True" else let resp$(respc+=1)="False"
00450   let fncmdkey("&Next",1,1,0,"Selects the highlited company to be included in automatic processing.") !:
...
00760   let fnprocess(1)
00770   let fnputcno(clnum(1))
00774   let fnpgnum(-1) ! resets the last program processed back to 0 befor going to NEWPRauto
00800 XIT: let fnxit
