...
00110     fncno(cno)
00120     if addall=0 then fen$="CeMP.h[cno]" else !:
            fen$="CEmpALL.h[cno]"
00130     if indexfile$="" then if$="[Q]\PRmstr\rpindex.h[cno]" else !:
            if$=indexfile$
00140     fncombof(fen$,myline,mypos,43,"[Q]\PRmstr\rpmstr.h[cno]",1,8,9,30,if$,1+addall,0,"Select from the list of employees. To add an employee, go to the Employee File.",container)
00150     indexfile$=""
00160     goto XIT
