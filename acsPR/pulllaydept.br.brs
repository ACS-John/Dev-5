00010 ! Replace S:\acsPR\PullLayDept! only use for payroll (need to send out S:\acsPR\Layouts\chechhistory.lay and department.lay for any one trying to use this
00020 ! ______________________________________________________________________
00030   library 'S:\Core\Library': fnerror,fnsetmonth,fncno,fnxit,fnremove,fnDedNames
...
00390 DATALN: j3=1
00400   p1=11
00410   p2=pos(srep$(ln$,'^','~'),'~',p1+1) ! pos(ln$,"^",p1+1)
00420   p3=pos(srep$(ln$,'^','~'),'~',p2+1) ! pos(ln$,"^",p2+1)
00430   p4=pos(srep$(ln$,'^','~'),'~',p3+1) ! pos(ln$,"^",p3+1)
00440   p5=len(rtrm$(ln$))
...
00520   p1=pos(a$(j3,3)," ",1)+1
00530   p2=pos(a$(j3,3),".",1)+1
00540   p3=len(rtrm$(a$(j3,3))) ! was standard
00550 ! p3=POS(A$(J3,3),"^",1)-1 ! for acsea and acscl only  (way John does layouts)
00560   p4=pos(a$(j3,3),"*",1)
00570   if p4=0 then m1=1 else m1=val(a$(j3,3)(1:p4-1))
00580   l=int(val(a$(j3,3)(p1:p3))) ! FIELD STORAGE LENGTH
00590   if p2>1 then dp=val(a$(j3,3)(p2:p3)) else dp=0 !:
          ! DECIMAL POSITIONS
...
00720   a(j3,6)=b
00730   a=b
00740   rl=rl+int(val(a$(j3,3)(p1:p3)))*m1
...
00920 XIT: fnxit ! pr f "24,1,C 7,UT,N": "Done..."
00930   stop 
