...
00250   open #20: "Name=[Q]\PRmstr\Company.h[cno],Shr",internal,input 
...
10040   open #2: "Name=[Q]\PRmstr\RPMSTR.h[cno],KFName=[Q]\PRmstr\RPINDEX.h[cno],Shr",internal,input,keyed 
10060   open #4: "Name=[Q]\PRmstr\payrollchecks.h[cno],KFName=[Q]\PRmstr\checkidx.h[cno]",internal,outIn,keyed 
10080   open #3: "Name=[Q]\PRmstr\Department.h[cno],Shr, KFName=[Q]\PRmstr\DeptIdx.h[cno],Shr",internal,outIn,keyed 
...
10160   m1=m2=h2=h3=dedytdfica=dedqtrfica=dedytdfederal=dedqtrfederal=m4=0
...
10320   if em5=1 then pedate=begdate+19: box1+=1 ! monthly pay period
10340   if em5=2 then pedate=begdate+15 : box1+=1 ! semi-monthly
10360   if em5=3 then pedate=begdate+14 : box1+=1 ! bi-weekly
10380   if em5=4 then pedate=begdate+7: box1+=1 ! weekly
10400   !   deptkey$=cnvrt$("pic(zzzzzzz#)",eno)&cnvrt$("pic(zz#)",tdn)
...
10660   !   dcq=0 ! total wage for quarter
10680   !   tcy=0 ! total wages to end of quarter
10700   for j=1 to 20
10720     if dedfed(j)=1 and dedcode(j)=1 then dedytdfederal+=ytdtotal(j+4): dedqtrfederal+=qtr(j+4) ! TOTAL DEDUCTION FROM WAGES  TO TAXABEL FEDERAL WAGE
10740     if dedfica(j)=1 and dedcode(j)=1 then dedytdfica+=ytdtotal(j+4) : dedqtrfica+=qtr(j+4) ! TOTAL DEDUCTIONS FOR FICA FOR QUARTER
10760   next j
...
11780     h2=ficamaxw-(m2-m1)
11800     goto L2010
11820 L1980: h2=0
11840     goto L2010
11860 L2000: h2=m1
11880 L2010: if em6=1 or em6=9 then goto L2050
11900     if m2<mcm then h3=m1 : goto L2050 ! MCM = MEDICARE MAXIMUM WAGE
11920     if m2-m1>mcm then h3=0 : goto L2050
11940     h3=mcm-(m2-m1)
...
13380 ERTN: fnerror(program$,err,line,act$,"xit")
...
14980 L3680: p1=pos(csz$,".",1)
15000     if p1>0 then csz$(p1:p1)=" ": p2=p1: goto L3680
15020 ! IF P2>0 AND CSZ$(P2+1:P2+1)=" " THEN cSZ$(P2+1:P2+1)="" ! dump any extra spaces
15040     p2=0
15060 L3720: p1=pos(csz$,",",1)
15080     if p1>0 then csz$(p1:p1)=" ": p2=p1: goto L3720
