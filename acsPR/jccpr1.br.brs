...
00300 L300: io1$(ce)=rtrm$(uprc$(io1$(ce))) : ce1=pos(io1$(ce),"U",1) !:
        if ce1=0 then goto L290
00310   ce2=ce1+1 : io1$(ce)(ce1:ce1)="UC" : goto L260
00320 CONV1: if ce>0 then io1$(ce)(ce1:ce2)="U"
00330   ce=cnt+1
...
00390   mat dr=(0)
00400   dr(1)=int(df*.01)
00410   dr(7)=int(dt*.01)
00420   df1=df
00430   dt1=dt
00440   df=fndate_mmddyy_to_ccyymmdd(df)
00450   dt=fndate_mmddyy_to_ccyymmdd(dt)
00460   if dt<df then goto L260
00470   for j=2 to 7
00480     if dr(9-j)-int(dr(9-j)*.01)*100-1=0 then goto L510
00490     dr(8-j)=dr(7)-j+1
00500   next j
00510 L510: if dr(1)=0 then dr(1)=int(df1*.01)
00520   for j=2 to 6
...
00750   msgline$(1)="No Transactions exist for Job Number "&ltrm$(jn$)
00760   msgline$(2)="within the specified date range."
...
00860   tr4=fndate_mmddyy_to_ccyymmdd(tr(4))
...
01130 ERTN: fnerror(program$,err,line,act$,"xit")
