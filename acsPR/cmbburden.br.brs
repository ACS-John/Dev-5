...
00130     if indexfile$="" then let if$=env$('Q')&"\PRmstr\burdenidx.h"&str$(cno) else !:
            let if$=indexfile$
00140     let fncombof(fen$,myline,mypos,43,env$('Q')&"\PRmstr\burden.h"&str$(cno),1,8,9,30,if$,1+addall,0,"Select from the list of personnel burden records. To add a personnel burden record, take the Add option.",container)
00150     let indexfile$=""
00160     goto XIT
