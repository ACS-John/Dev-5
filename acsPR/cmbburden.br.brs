...
00110     fncno(cno)
00120     if addall=0 then fen$="Cburden.h[cno]" else !:
            fen$="CburdenALL.h[cno]"
00130     if indexfile$="" then if$="[Q]\PRmstr\burdenidx.h[cno]" else !:
            if$=indexfile$
00140     fncombof(fen$,myline,mypos,43,"[Q]\PRmstr\burden.h[cno]",1,8,9,30,if$,1+addall,0,"Select from the list of personnel burden records. To add a personnel burden record, take the Add option.",container)
00150     indexfile$=""
00160     goto XIT
