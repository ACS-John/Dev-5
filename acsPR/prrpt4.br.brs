...
00100   let fntop("S:\acsPR\prRpt4",cap$="Report File - Proof List")
...
00230   print #255,using L240: date$,cnam$,time$,"Payroll Report File Proof List"
00240 L240: form skip 2,pos 1,c 8,pos nametab,c 40,skip 1,pos 1,c 8,pos 51,c 30,skip 2
00250   print #255,using L260: "Report Number",rn
00260 L260: form pos 1,c 13,pos 20,pic(zz),skip 1
00270   print #255,using L280: "Report Title",rt$
00280 L280: form pos 1,c 12,pos 20,c 78,skip 1
00290   print #255,using L300: "Column Headings",ch$(1)
00300 L300: form pos 1,c 15,skip 2,c 132,skip 1
00310   print #255,using L320: ch$(2)
00320 L320: form pos 1,c 132,skip 2
00330   print #255,using L340: "Item # for Prt Sel",ips
00340 L340: form pos 1,c 18,pos 30,pic(zz#),skip 1
00350   print #255,using L360: "Summarize Departments",sd
00360 L360: form pos 1,c 26,pos 32,pic(#),skip 1
00370   print #255,using L360: "Use Condensed Print",cp
00380   for j=1 to 100
00390     if psc(j)=0 then goto L490
00400     if j><48 then goto L450
00410     print #255: newpage
00420     print #255,using L430: "Print Selection Criteria",psc(j)
00430 L430: form skip 3,pos 1,c 24,pos 30,pic(---------.###),skip 1
00440     goto L470
00450 L450: print #255,using L460: "PRINT SELECTION CRITERIA",psc(j)
...
00510     print #255,using L520: "Item Number to Print",inp(j), "Print Position",pp(j), "Total this Item",ti(j)
00520 L520: form pos 1,c 20,pos 25,pic(zzz),pos 35,c 14,pos 49,pic(zzz),pos 65,c 15,pos 85,pic(#),skip 1
00530   next j
00540 L540: print #255: newpage
00550   goto L210
