...
00725   fnLbl(20,1,"Total deposits for quarter including overpayments:",mylen+1,1,0,0) !:
        ...
        resp$(respc+=1)=""
00730   fnCmdSet(2): fnAcs(sn$,0,mat resp$,ck)
...
01190   for j=1 to 10
01200     if dedfed(j)=1 and dedcode(j)=1 then dwy+=m(j+10): dwq+=m(j+11)
01210     if dedfica(j)=1 and dedcode(j)=1 then dfy=dfy+m(j+10): dfq=dfq+m(j+11)
01220   next j
01230   form pos 168,21*pd 5.2,pos 273,17*pd 5.2,pos 468,pd 3
01240   m2=m2+m(1)
01250   m1=m1+m(2)
01260   tqm1=tqm1+m(4)
01270   tqm14=tqm14+m(36)
...
01300 ! kEY$=CNVRT$("pic(ZZZZZZZZ)",ENO)&RPT$(CHR$(0),6) !:
        ! Restore #4,Key>=KEY$:
01310 ! Read #4,Using 1270: DEPENO,PRD Nokey 1360 Eof 1360
01320 ! Form POS 1,N 8,PD 6
01330 ! If EM5=1 Then pEDATE=BEGDATE+19 ! monthly pay period
01340 ! If EM5=2 Then pEDATE=BEGDATE+15 ! semi-monthly
01350 ! If EM5=3 Then pEDATE=BEGDATE+14 ! bi-weekly
01360 ! If EM5=4 Then pEDATE=BEGDATE+7 ! weekly
...
01840   h2=ficawage-(m2-dfy-m1-dfq)
01850   goto L1890
01860 L1860: h2=0
01870   goto L1890
01880 L1880: h2=m1-dfq
01890 L1890: ! If EM6=1 OR EM6=9 Then Goto 1880
01900   if m2-dfy<mcm then h3=m1-dfq : goto L1930
01910   if m2-dfy-m1-dfq>mcm then h3=0 : goto L1930
01920   h3=mcm-(m2-dfy-m1-dfq)
01930 L1930: ! 
...
03890 L3890: p1=pos(csz$,".",1)
03900   if p1>0 then csz$(p1:p1)=" ": p2=p1: goto L3890
03910 ! IF P2>0 AND CSZ$(P2+1:P2+1)=" " THEN cSZ$(P2+1:P2+1)="" ! dump any extra spaces
03920   p2=0
03930 L3930: p1=pos(csz$,",",1)
03940   if p1>0 then csz$(p1:p1)=" ": p2=p1: goto L3930
