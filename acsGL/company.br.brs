...
01680   let mcr=val(resp$(5))
01690   let mcm=val(resp$(6))
01700   for j=1 to 5
01710     let gl$(j)=fnagl$(resp$(j+6))
01720     let prgl(j,1)=val(gl$(j)(1:3)): let prgl(j,2)=val(gl$(j)(4:9)): let prgl(j,3)=val(gl$(j)(10:12))
01730   next j
...
02080   let resp=0
02090   for j=1 to 10
02100     let miscname$(j)=resp$(resp+=1)
02110     if resp$(resp+=1)=option2$(1) then let dedcode(j)=1 else let dedcode(j)=2
02120     if resp$(resp+=1)="True" then let dedfed(j)=1 else let dedfed(j)=0
02130     if resp$(resp+=1)="True" then let dedfica(j)=1 else let dedfica(j)=0
02140     if resp$(resp+=1)="True" then let dedst(j)=1 else let dedst(j)=0
02150     if resp$(resp+=1)="True" then let deduc(j)=1 else let deduc(j)=0
02160     let miscgl$(j)=fnagl$(resp$(resp+=1))
02170   next j
02180   gosub SAVE
02190   close #company: 
02200   goto XIT
...
02230 XIT: let fnxit ! Let FNCHAIN("S:\acsGL\ACglscr")
