...
! statement of changes in financial position with comparrison  for 8 1/2 * 11 paper

autoLibrary
on error goto Ertn
fnTop(program$,'Change Amount')
...
L300: !
	...
	dim by(13)
	dim bp(13)
	read #1,using L240: acct$,cb,mat by,mat bp eof L360
	if acct$>cogl$(3) then goto L360
	if fscode=0 then income=income-cb else goto L340
goto L350

L340: !
	if fnpriorcd=2 then income-=bp(fscode) else income-=by(fscode)
L350: !
goto L300
L360: !
	close #1:
...
L560: !
	...
	dim te$*1
	dim ac(9)
	read #1,using L600: r$,d$,te$,sp,ls,ds,ul,rs,bc,ap,mat ac,ic,fc eof L1970
