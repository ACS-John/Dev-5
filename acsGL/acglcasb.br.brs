...
! CASH FLOW STATEMENT  WITH BUDGET
	...
	dim bm(13),bp(13),by(13),sc1$(2)*20,fl1$*256,in3$(4),p$(20)*50,cap$*128
	dim cnam$*40,b$*3,a$(8)*30,oldtrans$*16,g(8),accum(9,7),udf$*256,cch$*15
	dim r$*5,d$*50,te$*1,ac(9),report$*50,secondr$*50,foot$*132,underlin$*14
 
	fnTop(program$,cap$="Cash Flow Statement With Budget")
	fncno(cno,cnam$)
	udf$=env$('temp')&'\'
	actpd$=fnactpd$ : _
	actpd=fnactpd : _
...
L510: form pos sp,c 40,skip 1
	...
	gosub L1610
	goto L420
L550: if te$="B" and ap>0 then goto L1320 ! ENDING BANK BALANCE
	if notrans=1 then goto L940
	if ir>=val(r$) and val(r$)><0 then goto L740
	close #3: ioerr ignore
	if fnps=2 then
		execute "Index [Q]\GLmstr\GLmstr.h[cno] "&udf$&"fsindex.H[cno] 78 3 Replace DupKeys -N"
	else
		execute "Index [Q]\GLmstr\GLmstr.h[cno] "&udf$&"fsindex.H[cno] 75 3 Replace DupKeys -N"
	end if
	open #3: "Name=[Q]\GLmstr\GLmstr.h[cno],KFName="&udf$&"fsindex.h[cno],Shr",internal,input,keyed
L630: ! read amounts from gl master file
L640: read #3,using L730: ir,bb,cb,mat by,mat bp,mat bm eof L940
...
L2240: fnTos(sn$="ACglchgs2") : _
	mylen=30: mypos=mylen+3 : right=1
	fnLbl(1,1,d$,mylen,right)
	fnLbl(3,1,"Monthy Budget:",mylen,right)
	fnTxt(3,mypos,12,0,right,"10",0,"Enter the monthly budget.",0 ) : _
	resp$(1)=str$(monthb)
	fnLbl(4,1,"Total for the Month:",mylen,right)
	fnTxt(4,mypos,12,0,right,"10",0,"Enter the total for the month.",0 ) : _
	resp$(2)=str$(total)
	fnLbl(5,1,"Total Year to Date:",mylen,right)
	fnTxt(5,mypos,12,0,right,"10",0,"Enter the total for the year.",0 ) : _
	resp$(3)=str$(total2)
	fnLbl(6,1,"Total Budget for the Year:",mylen,right)
	fnTxt(6,mypos,12,0,right,"10",0,"Enter the total budget for the  year.",0 ) : _
	resp$(4)=str$(annualb)
