...
00620   if fnpriorcd=1 then let cb=by(fnfscode) else let cb=bp(fnfscode)
00630   if fnpriorcd=2 then goto L660
00640   if fnfscode>1 then let bb=by(fnfscode-1) else let bb=0
00650   goto L670
00660 L660: if fnfscode>1 then let bb=bp(fnfscode-1) else let bb=0
...
00720     let annualb=annualb+bm(z)
00730   next z
...
00890   if rs=1 then print #255,using L1100: td$(1:sp2)," ",-tr(5) else print #255,using L1100: td$(1:sp2),"",tr(5)
00900   goto L860
...
01090   if total=0 and total2=0 and ls+ds+ul+ic>0 then print #255,using L1100: "",dollar$,0 pageoflow L1600
...
01220   print #255,using L1100: d$(1:sp2),dollar$,accum1 pageoflow L1600
01230   gosub L1380
01240   gosub L1610
01250   gosub L1450
01260   goto L410
...
01400     let accum(j,1)=0
01410     let accum(j,2)=0
01420 L1420: next j
01430   return 
...
01680 L1680: print #255,using L1690: underlin$
01690 L1690: form skip 1,pos 52,2*c 15,skip 1
01700 L1700: ! f REDIR=0 Then Print #255,Using 1560: " "
...
01930   goto XIT
01940 L1940: ! 
01950   print fields "2,5,C 75,N": "ENTER THE FOLLOWING INFORMATION FOR "& rtrm$(d$)
01960   print fields "6,5,C 70,N": "                    CURRENT         "
01970   print fields "7,5,C 70,N": "                     MONTH       "
