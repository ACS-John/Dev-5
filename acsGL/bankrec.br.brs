...
00450   item1$(1)="Enter Cleared Checks" !:
...
00470   if selection=0 then selection=1: !:
...
00770   fnTxt(3,48,8,0,1,"1",0,"") !:
        resp$(respc+=1)=""
00780   fnCmdSet(2): fnAcs(sn$,0,mat resp$,ck)
00790   if ck=5 or ck=cancel then goto MENU1
00800   pcd1=val(resp$(1)) ! clear old adjustments
...
01010   fnCmdSet(2): fnAcs(sn$,0,mat resp$,ck)
01020   if ck=5 then goto MENU1
01030   k$=resp$(1) ! check # to clear
01040   if ck=61 then !:
...
01950   fncloseprn
01960   t1(10)=bgbal+t1(7)+t1(4)-t1(9)-t1(2)
01970   cutoffbal=bal
...
02090   fnCmdSet(3): fnAcs(sn$,0,mat resp$,ck)
...
02350   hdw$(3)="Deposits cleared on "&cnvrt$("PIC(ZZ/ZZ/ZZ)",stmtdt)
02360   hdw$(4)="Withdrawals cleared on "&cnvrt$("PIC(ZZ/ZZ/ZZ)",stmtdt)
02370   fnTos(sn$="bankrec-12") !:
        respc=0 : mat resp$=('')
02380 ! fnFra(1,1,4,60,"Select Listings to Print","Choose any of the four listing",0)
...
02420   fnChk(4,3,hdw$(4),0) !:
        resp$(respc+=1)=""
02430   fnCmdSet(2): fnAcs(sn$,0,mat resp$,ck)
02440   if ck=5 or ck=cancel then goto MENU1
02450   if resp$(1)(1:1)="T" then w(1)=1 else w(1)=0
02460   if resp$(2)(1:1)="T" then w(2)=1 else w(2)=0
02470   if resp$(3)(1:1)="T" then w(3)=1 else w(3)=0
02480   if resp$(4)(1:1)="T" then w(4)=1 else w(4)=0
02490   fnopenprn
02500   for j=1 to 4
...
02900   fnTxt(2,32,8,0,1,"",0,"Enter the last check number in the series.") !:
        resp$(respc+=1)=""
02910   goto L2960
02920 L2920: fnLbl(1,1,"Lowest Date to Clear:",30,1)
...
02950   fnTxt(2,32,10,0,1,"1",0,"Normally you would use the last day of the month, but if the bank's cutoff date is different, you may want to use it.") !:
        resp$(respc+=1)=""
02960 L2960: fnCmdSet(2): fnAcs(sn$,0,mat resp$,ck)
02970   if ck=5 or ck=cancel then goto MENU1
02980   l1=val(resp$(1)) ! either lowest date or lowest check #
02990   h1=val(resp$(2)) ! either highest date or highest check #
03000   if ti3><2 then goto L3030
03010   l1=fncd(l1)
03020   h1=fncd(h1)
03030 L3030: if ti3=1 then k$=wbc$&str$(ti3)&lpad$(str$(l1),8)
03040   if ti3=2 then k$=wbc$&str$(ti3)&lpad$(str$(0),8)
...
03070 L3070: mat ml$(1) !:
        ...
        fnmsgbox(mat ml$,resp$,cap$,48) !:
        goto CLEAR_BY_RANGE
03080 L3080: ! pr f "1,2,C 8,R,N": " Check #"   ! do I want some kind of grid here???? kj
03090 ! pr f "1,11,C 10,R,N": "  Amount"
...
03140   tr2=fncd(val(tr$(2))) conv L3100
03150   if tr2<l1 or tr2>h1 then goto L3100
03160   goto L3190
03170 L3170: h2=val(tr$(1)) conv L3100
03180   if h2>h1 then displaycleared=1 : goto CLEAR_TRANSACTIONS_FROM_LIST ! Goto 3710
03190 L3190: rewrite #bankrec1,using 'Form POS 72,N 6': stmtdt
03200 ! pr f STR$(R1)&","&STR$(C1)&",C 8,N": TR$(1)  ! also some kind of grid here
03210 ! pr f STR$(R1)&","&STR$(C1+9)&",C 10,N": TR$(3)
03220 ! r1=R1+1
03230 ! If R1=8 Then r1=14
03240 ! If R1>23 Then r1=2: c1=C1+20 Else Goto 4230
03250 ! If C1>62 Then c1=2
03260 ! pr f "1,"&STR$(C1)&",C 8,R,N": " Check #"
03270 ! pr f "1,"&STR$(C1+9)&",C 10,R,N": "  Amount"
03280   goto L3100
...
03600   fnOpt(8,3,"Make Corrections",0,frame) !:
        if sel_code=8 then resp$(respc+=1)="True" else !:
          resp$(respc+=1)="False"
03610 ! fnCOMBOA("bankrec-3",3,30,MAT ITEM3$,"Select the funtion you would like to perform.  Normally you would Erase Previous, Enter Amounts, Clear Matches and check your totals")!!:
...
03660 L3660: fnCmdSet(2): fnAcs(sn$,0,mat resp$,ck)
03670   if ck=5 or ck=cancel then goto MENU1
03680   for j=1 to 8
03690     if resp$(j)(1:1)="T" then sel_code=ti1=j: goto L3710
03700   next j
...
03830   fnCmdSet(2): fnAcs(sn$,0,mat resp$,ck)
...
04020 L4020: t1=t1+am1
04030     if rtrm$(tr$)><"" then t2=t2+am1
04040 L4040: next j
...
04110   for j=1 to 3
04120     io7$(j)=str$(j*2+11)&",40,N 1,UT,N"
04130   next j
04140   pr newpage
...
04170   pr f "5,11,C 62": "This is designed to take care of two or more types of Credit"
04180   pr f "6,11,C 62": "Cards that are totaled together by the card company before"
04190   pr f "7,11,C 62": "they are transmitted to your bank for deposit."
04200   pr f "09,11,c 62": "credit card deposits begin with a prefix of c1,c2, or c3."
04210   pr f "10,11,c 62": "place a 1 by the types to be added together for 1 bank deposit"
04220   for j=1 to 3
04230     pr f str$(j*2+11)&",37,C 3": "C"&str$(j)
04240   next j
04250   pr f "20,30,C 09,B,1": "Next (F1)"
04260 L4260: rinput fields mat io7$: mat dpt conv CONV7
04270   if ce>0 then io7$(ce)(ce1:ce2)="U": ce=0
04280   if ck>0 then goto L4350 else ce=curfld
04290 L4290: ce=ce+1: if ce>udim(io7$) then ce=1
04300 L4300: io7$(ce)=rtrm$(uprc$(io7$(ce))) : ce1=pos(io7$(ce),"U",10) : if ce1=0 then goto L4290
04310   ce2=ce1+1 : io7$(ce)(ce1:ce1)="UC" : goto L4260
04320 CONV7: if ce>0 then io7$(ce)(ce1:ce2)="U"
04330   ce=cnt+1
...
04430   for j=1 to 5
04440     io8$(j)="14,"&str$(j*9+15)&",N 6,UT,N"
04450     io8$(j+5)="16,"&str$(j*9+15)&",N 6,UT,N"
04460     io8$(j+10)="18,"&str$(j*9+15)&",N 6,UT,N"
04470     io8$(j+15)="20,"&str$(j*9+15)&",N 6,UT,N"
04480     io8$(j+20)="22,"&str$(j*9+15)&",N 6,UT,N"
04490   next j
04500   pr f "4,11,c 62": "This is designed to take care of two or more dates on Credit"
04510   pr f "5,11,c 62": "Card batched totaled together by the credit card company"
04520   pr f "6,11,c 62": "before they are transmitted to your bank for deposit."
04530   pr f "08,11,c 62": "this usually occurs on weekends or when a batch is transmitted"
04540   pr f "09,11,c 62": "after 8pm (Eastern time) one day and before 8pm the next."
04550   pr f "11,11,C 62": "You may have 5 different entries with up to 5 Dates per entry"
04560   for j=1 to 5
04570     pr f str$(j*2+12)&",16,C 50": "Entry "&str$(j)&"        +        +        +        +"
04580   next j
04590   pr f "13,24,C 42,N": "         (use only mmddyy format)"
04600   pr f "24,35,C 09,B,1": "Next (F1)"
04610 L4610: rinput fields mat io8$: mat dpd conv ERR8
04620   if ce>0 then io8$(ce)(ce1:ce2)="U": ce=0
04630   if ck>0 then goto L4700 else ce=curfld
04640 L4640: ce=ce+1: if ce>udim(io8$) then ce=1
04650 L4650: io8$(ce)=rtrm$(uprc$(io8$(ce))) : ce1=pos(io8$(ce),"U",10) : if ce1=0 then goto L4640
04660   ce2=ce1+1 : io8$(ce)(ce1:ce1)="UC" : goto L4610
04670 CONV8: if ce>0 then io8$(ce)(ce1:ce2)="U"
04680   ce=cnt+1
04690 ERR8: pr f "24,78,C 1": bell : goto L4650
04700 L4700: dpd$=cnvrt$("PIC(######)",stmtdt)
04710   mo2=val(dpd$(1:2))
04720   da2=val(dpd$(3:4))
04730   yr2=val(dpd$(5:6))
04740   for ce=1 to 25
04750     j1=int((ce-1)/5)+1
04760     j2=ce-(j1-1)*5
04770     if dpd(j1,j2)=0 then goto L4880
04780     dpd$=cnvrt$("PIC(######)",dpd(j1,j2))
04790     mo1=val(dpd$(1:2))
04800     da1=val(dpd$(3:4))
04810     yr1=val(dpd$(5:6))
...
05000   pr newpage
05010   pr f "10,10,C 60": "Correct Amount (0 to Delete):"
...
05190     k$=lpad$(rtrm$(bankgl$),2)&str$(tcde)&tr$(1)
...
05480 L5480: k$=lpad$(rtrm$(bankgl$),12)&str$(tcde)&lpad$(rtrm$(tr$(1)),8)
...
05530 L5530: fnTos(sn$="bankrec-11") !:
        respc=0 : mat resp$=('')
05540   fnLbl(1,1,trim$(bn$(1:30))&"-"&type$,65,2)
05550   fnflexinit1('Deposit-1',2,3,15,55,mat chdr$,mat cmask$,1)
05560   restore #clearing: 
05570   x=lrec(89): if nextrec>0 and x>1 and displayattop$="True" then goto L5580 else goto L5670
...
05690 L5690: fnLbl(17,30,"Total Cleared:",16,1)
...
05770   displaycleared=total= clear_from_range=0
05780   if ck=5 or ck=cancel then goto BANK_STMT_INFO
05790   displayattop$=resp$(3) !:
...
05810   if ck=3 then displaycleared=1: goto CLEAR_TRANSACTIONS_FROM_LIST !:
...
05840 ! k$(1:12)=LPAD$(RTRM$(K$(1:12)),12)
05850   if clr=0 then newclr=stmtdt else newclr=0 ! if no previous clearing date, use new one; if it has a date, unclear it
