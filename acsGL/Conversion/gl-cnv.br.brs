00010 ! Replace R:\acsGL\Conversion\GL-CNV
00020 ! ???
00030 ! ______________________________________________________________________
00040   library 'R:\Core\Library': fntop,fnxit, fnputcno,fnerror
...
00230   open #2: "Name=ACQ:\GLmstr\GLmstr.h"&str$(cno)&",KFName=Q:\GLmstr\GLIndex.h"&str$(cno),internal,outin,keyed 
00240   open #1: "Name=Q:\GLmstr\GLmstr.h"&str$(cno)&",Replace,RecL=338",internal,output 
...
00310   open #2: "Name=Q:\GLmstr\ACGLTRAN.h"&str$(cno)&"",internal,input,relative 
00320   open #1: "Name=Q:\GLmstr\GLTrans.h"&str$(cno)&",Replace,RecL=73",internal,outin,relative 
...
00400   open #1: "Name=Q:\GLmstr\ACGLPGMN.h"&str$(cno)&",Replace,RecL=58",internal,outin,relative 
00410   for j=1 to 20
00420     write #1,using L430,rec=j: "","",0,0,0
00430 L430: form pos 1,c 20,c 35,3*n 1
00440   next j
00450   close #1: 
00460   open #2: "Name=ACQ:\GLmstr\PRmstr.h"&str$(cno)&"",internal,outin,relative ioerr L550
00470   open #1: "Name=Q:\GLmstr\PRmstr.h"&str$(cno)&",Replace,RecL=190",internal,output 
...
00560   open #1: "Name=Q:\GLmstr\Company.h"&str$(cno),internal,outin,relative 
...
00590   close #1,free: 
00600   open #1: "Name=Q:\GLmstr\Company.h"&str$(cno)&",SIZE=0,RecL=512",internal,outin,relative 
00610   open #2: "Name=Q:\GLmstr\PRGLNUMB.h"&str$(cno)&"",internal,input ioerr L650
00620   read #2,using L630: mat prgl
00630 L630: form pos 1,21*pd 4
00640   close #2,free: 
00650 L650: open #2: "Name=Q:\GLmstr\GLREC.h"&str$(cno)&"",internal,input ioerr L690
00660   read #2,using L670: reccode
00670 L670: form pos 1,n 1
00680   close #2,free: 
00690 L690: open #2: "Name=R:\acsGL\ACGLCODE.H"&str$(cno),internal,input ioerr L730
00700   read #2,using L710: c1,c2
00710 L710: form pos 1,n 1,n 2
00720   close #2,free: 
00730 L730: open #2: "Name=Q:\GLmstr\ACGLDATE.h"&str$(cno)&"",internal,input 
...
00790   close #1: 
00800   open #1: "Name=Q:\GLmstr\GLBREC.h"&str$(cno)&",NoShr",internal,input ioerr L820
00810   close #1,free: 
00820 L820: open #1: "Name=Q:\GLmstr\GLBREC.h"&str$(cno)&",size=0,RecL=68,Shru",internal,outin,relative 
00830   close #1: 
00840   gosub L920 ! CONVERT VENDOR FILES
00850   execute "INDEX Q:\GLmstr\GLmstr.h"&str$(cno)&" Q:\GLmstr\GLIndex.h"&str$(cno)&" 1 12 Replace DupKeys -n"
00860   execute "INDEX Q:\GLmstr\GL1099.h"&str$(cno)&" Q:\GLmstr\GL109IDX.h"&str$(cno)&" 1 8 Replace DupKeys -n"
00870   execute "INDEX Q:\GLmstr\GLBREC.h"&str$(cno)&" Q:\GLmstr\GLRecIdx.h"&str$(cno)&" 1 24 Replace DupKeys -n"
00880   execute "INDEX Q:\GLmstr\PRmstr.h"&str$(cno)&" Q:\GLmstr\PRIndex.h"&str$(cno)&" 1 4 Replace DupKeys -n" ioerr L890
00890 L890: execute "INDEX Q:\GLmstr\GL1099.h"&str$(cno)&" Q:\GLmstr\GL109IDX.h"&str$(cno)&" 1 8 Replace DupKeys"
00900   goto L1170
00910 ! ______________________________________________________________________
00920 L920: open #3: "Name=Q:\GLmstr\GL1099.h"&str$(cno)&",KFName=Q:\GLmstr\GL109IDX.h"&str$(cno),internal,outin,keyed 
00930   open #2: "Name=Q:\GLmstr\GLTR1099.H"&str$(cno),internal,outin,relative ioerr L950
00940   close #2,free: 
00950 L950: open #2: "Name=Q:\GLmstr\GLTR1099.H"&str$(cno)&",SIZE=0,RecL=64,NoShr",internal,outin,relative 
00960   write #2,using L1000,rec=1: "",0,0,"","",1
00970   open #1: "Name=Q:\GLmstr\GL1099N.H"&str$(cno)&",KFName=Q:\GLmstr\GL109IDX.h"&str$(cno),internal,outin,keyed ioerr L990
00980   close #1,free: 
00990 L990: open #1: "Name=Q:\GLmstr\GL1099N.H"&str$(cno)&",size=0,RecL=127,NoShr",internal,outin,relative 
...
01130 L1130: execute "COPY Q:\GLmstr\GL1099N.H"&str$(cno)&" Q:\GLmstr\GL1099.h"&str$(cno)
01140   execute "FREE Q:\GLmstr\GL1099N.H"&str$(cno)
01150   return 
01160 ! ______________________________________________________________________
01170 L1170: ! REASS.CNV
01180   open #1: "Name=Q:\GLmstr\GLmstr.h"&str$(cno)&",KFName=Q:\GLmstr\GLIndex.h"&str$(cno),internal,outin,keyed 
01190   open #2: "Name=Q:\GLmstr\GLTrans.h"&str$(cno),internal,outin,relative 
01200   print newpage
...
01360   close #1: 
01370   close #2: 
01380   open #1: "Name=Q:\GLmstr\PRmstr.h"&str$(cno)&",KFName=Q:\GLmstr\PRIndex.h"&str$(cno),internal,outin,keyed ioerr L1580
01390   open #2: "Name=Q:\GLmstr\ACPRCKS.h"&str$(cno),internal,outin,relative ioerr L1580
01400   print newpage
...
01570   close #2: 
01580 L1580: ! 
01590   open #1: "Name=Q:\GLmstr\AcTrans.h"&str$(cno),internal,input,relative ioerr L1740
...
01690   close #1,free: 
01700   close #2: 
01710   execute "RENAME X Q:\GLmstr\AcTrans.h"&str$(cno)
01720   execute "INDEX Q:\GLmstr\AcTrans.h"&str$(cno)&" Q:\GLmstr\AcTrIdx.h"&str$(cno)&" 1/71/17/13 12/2/2/4 Replace DupKeys"
01730 ! ___    __________   ___________   __________   ___________   _________
01740 L1740: ! R:\acsGL\PRmstr.CNV
01750   dim pr1$*90,pr1(18),pr2(36)
01760   open #1: "Name=Q:\GLmstr\PRmstr.h"&str$(cno)&",KFName=Q:\GLmstr\PRIndex.h"&str$(cno),internal,outin,keyed ioerr L1940
...
01870 END1: close #1: 
01880   close #2: 
01890   execute "COPY "&env$('Temp')&"\Work."&session$&", Q:\GLmstr\PRmstr.h"&str$(cno)&""
01900   execute "INDEX Q:\GLmstr\PRmstr.h"&str$(cno)&",Q:\GLmstr\PRIndex.h"&str$(cno)&",1,4,Replace,DupKeys"
01910   let fnputcno(cno)
01920   open #1: "Name=Q:\GLmstr\ACPRCKS.h"&str$(cno)&",RecL=110,Replace",internal,output 
01930   close #1: 
01940 L1940: open #1: "Name=Q:\GLmstr\Company.h"&str$(cno),internal,outin,relative ioerr L1990
...
02070   for j=1 to 6
02080     execute "COPY Q:\GLmstr\"&fil$(j)&".h"&str$(cno)&" "&env$('Temp')&"\Work."&session$&" -83" ioerr L2360
02090     execute "COPY  "&env$('Temp')&"\Work."&session$&" Q:\GLmstr\"&fil$(j)&".h"&str$(cno)&""
02100     if j=2 or j=5 then goto L2110 else goto L2360
02110 L2110: open #1: "Name=Q:\GLmstr\"&fil$(j)&".h"&str$(cno)&",KFName=Q:\GLmstr\"&idx$(j)&".h"&str$(cno),internal,outin,keyed 
