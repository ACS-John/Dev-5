00020   library 'R:\Core\library': fnxit,fntop
00030 ! 
...
00100   print newpage
00110   print fields "8,5,C 70,HRB,N": "   WARNING THIS PROGRAM CAN ONLY BE RUN ONE TIME FOR EACH COMPANY #"
00120   print fields "10,9,C 60": "ENTER THE COMPANY # TO BE CONVERTED OR 0 TO STOP:"
00130 L130: input fields "10,60,N 2,UE,N": cno conv L130
00140   if cno=0 then stop 
00150   print newpage
00160   print fields "10,5,C 60": "CONVERSION FOR COMPANY #"&str$(cno)&" IN PROCESS"
00170 ! 
00180   rewrite #1,using L190,rec=1: cno
00190 L190: form pos 1,n 2
00200   close #1: 
00210   open #2: "Name=ACQ:\GLmstr\GLmstr.h"&str$(cno)&",KFName=Q:\GLmstr\GLIndex.h"&str$(cno),internal,outin,keyed 
00220   open #1: "Name=Q:\GLmstr\GLmstr.h"&str$(cno)&",REPLACE,RecL=338",internal,output 
...
00290   open #2: "Name=Q:\GLmstr\ACGLTRAN.h"&str$(cno),internal,input,relative 
00300   open #1: "Name=Q:\GLmstr\GLTrans.h"&str$(cno)&",REPLACE,RecL=73",internal,outin,relative 
...
00380   open #1: "Name=Q:\GLmstr\ACGLPGMN.h"&str$(cno)&",REPLACE,RecL=58",internal,outin,relative 
00390   for j=1 to 20
00400     write #1,using L410,rec=j: "","",0,0,0
00410 L410: form pos 1,c 20,c 35,3*n 1
00420   next j
00430   close #1: 
00440   open #2: "Name=ACQ:\GLmstr\PRmstr.h"&str$(cno)&"",internal,outin,relative ioerr L530
00450   open #1: "Name=Q:\GLmstr\PRmstr.h"&str$(cno)&",REPLACE,RecL=190",internal,output 
...
00540   open #1: "Name=Q:\GLmstr\Company.h"&str$(cno),internal,outin,relative 
...
00570   close #1,free: 
00580   open #1: "Name=Q:\GLmstr\Company.h"&str$(cno)&",SIZE=0,RecL=512",internal,outin,relative 
00590   open #2: "Name=Q:\GLmstr\PRGLNUMB.h"&str$(cno),internal,input ioerr L630
00600   read #2,using L610: mat prgl
00610 L610: form pos 1,21*pd 4
00620   close #2,free: 
00630 L630: open #2: "Name=Q:\GLmstr\GLREC.h"&str$(cno)&"",internal,input ioerr L670
00640   read #2,using L650: reccode
00650 L650: form pos 1,n 1
00660   close #2,free: 
00670 L670: open #2: "Name=R:\acsGL\ACGLCODE.H"&str$(cno)&"",internal,input ioerr L710
00680   read #2,using L690: c1,c2
00690 L690: form pos 1,n 1,n 2
00700   close #2,free: 
00710 L710: open #2: "Name=Q:\GLmstr\ACGLDATE.h"&str$(cno)&"",internal,input 
...
00770   close #1: 
00780   open #1: "Name=Q:\GLmstr\GLBREC.h"&str$(cno)&",NoShr",internal,input ioerr L800
00790   close #1,free: 
00800 L800: open #1: "Name=Q:\GLmstr\GLBREC.h"&str$(cno)&",size=0,RecL=68,Shru",internal,outin,relative 
00810   close #1: 
00820   gosub L890 ! CONVERT VENDOR FILES
00830   execute "INDEX Q:\GLmstr\GLmstr.h"&str$(cno)&" Q:\GLmstr\GLIndex.h"&str$(cno)&" 1 12 REPLACE DupKeys"
00840   execute "INDEX Q:\GLmstr\GL1099.h"&str$(cno)&" Q:\GLmstr\GL109IDX.h"&str$(cno)&" 1 8 REPLACE DupKeys"
00850   execute "INDEX Q:\GLmstr\GLBREC.h"&str$(cno)&" Q:\GLmstr\GLRecIdx.h"&str$(cno)&" 1 24 REPLACE DupKeys"
00860   execute "INDEX Q:\GLmstr\PRmstr.h"&str$(cno)&" Q:\GLmstr\PRIndex.h"&str$(cno)&" 1 4 REPLACE DupKeys" ioerr L870
00870 L870: execute "INDEX Q:\GLmstr\GL1099.h"&str$(cno)&" Q:\GLmstr\GL109IDX.h"&str$(cno)&" 1 8 REPLACE DupKeys"
00880   goto L1130
00890 L890: open #3: "Name=Q:\GLmstr\GL1099.h"&str$(cno)&",KFName=Q:\GLmstr\GL109IDX.h"&str$(cno),internal,outin,keyed 
00900   open #2: "Name=Q:\GLmstr\GLTR1099.H"&str$(cno),internal,outin,relative ioerr L920
00910   close #2,free: 
00920 L920: open #2: "Name=Q:\GLmstr\GLTR1099.H"&str$(cno)&",SIZE=0,RecL=64,NoShr",internal,outin,relative 
00930   write #2,using L970,rec=1: "",0,0,"","",1
00940   open #1: "Name=Q:\GLmstr\GL1099N.H"&str$(cno)&",KFName=Q:\GLmstr\GL109IDX.h"&str$(cno),internal,outin,keyed ioerr L960
00950   close #1,free: 
00960 L960: open #1: "Name=Q:\GLmstr\GL1099N.H"&str$(cno)&",size=0,RecL=127,NoShr",internal,outin,relative 
...
01100 L1100: execute "COPY Q:\GLmstr\GL1099N.H"&str$(cno)&" Q:\GLmstr\GL1099.h"&str$(cno)
01110   execute "FREE Q:\GLmstr\GL1099N.H"&str$(cno)
01120   return 
01130 L1130: ! REASS.CNV
01140   open #1: "Name=Q:\GLmstr\GLmstr.h"&str$(cno)&",KFName=Q:\GLmstr\GLIndex.h"&str$(cno),internal,outin,keyed 
01150   open #2: "Name=Q:\GLmstr\GLTrans.h"&str$(cno),internal,outin,relative 
01160   print newpage
01170   print fields "10,15,c 60,h,n": "REASSIGN GL ADDRESSES IN PROCESS"
...
01320   close #1: 
01330   close #2: 
01340   open #1: "Name=Q:\GLmstr\PRmstr.h"&str$(cno)&",KFName=Q:\GLmstr\PRIndex.h"&str$(cno),internal,outin,keyed ioerr L1540
01350   open #2: "Name=Q:\GLmstr\ACPRCKS.h"&str$(cno),internal,outin,relative ioerr L1540
01360   print newpage
01370   print fields "10,15,c 60,h,n": "REASSIGN PR ADDRESSES IN PROCESS"
...
01530   close #2: 
01540 L1540: ! 
01550   open #1: "Name=Q:\GLmstr\AcTrans.h"&str$(cno),internal,input,relative ioerr L1690
01560   print fields "14,32,C 16,BR,N": "   IN PROCESS"
...
01610     let actpd=int(tr(4)*.0001)
01620 L1620: form pos 1,n 3,n 6,n 3,n 6,pd 6.2,2*n 2,c 12,c 30,n 2
01630     write #2,using L1620: mat tr,tr$,td$,actpd
01640 L1640: next j
01650   close #1,free: 
01660   close #2: 
01670   execute "RENAME X Q:\GLmstr\AcTrans.h"&str$(cno)
01680   execute "INDEX Q:\GLmstr\AcTrans.h"&str$(cno)&" Q:\GLmstr\AcTrIdx.h"&str$(cno)&" 1/71/17/13 12/2/2/4 REPLACE DupKeys"
01690 L1690: ! R:\acsGL\PRmstr.CNV
01700   dim pr1$*90,pr1(18),pr2(36)
01710   open #1: "Name=Q:\GLmstr\PRmstr.h"&str$(cno)&",KFName=Q:\GLmstr\PRIndex.h"&str$(cno),internal,outin,keyed ioerr L1910
...
01810 END1: close #1: 
01820   close #2: 
01830   execute "COPY "&env$('Temp')&"\Work."&session$&", Q:\GLmstr\PRmstr.h"&str$(cno)&""
01840   execute "INDEX Q:\GLmstr\PRmstr.h"&str$(cno)&",Q:\GLmstr\PRIndex.h"&str$(cno)&",1,4,REPLACE,DupKeys"
01850   open #1: "Name=CNO.H"&wsid$,internal,outin,relative 
01860   rewrite #1,using L1870,rec=1: cno
01870 L1870: form pos 1,n 2
01880   close #1: 
01890   open #1: "Name=Q:\GLmstr\ACPRCKS.h"&str$(cno)&",RecL=110,REPLACE",internal,output 
01900   close #1: 
01910 L1910: open #1: "Name=Q:\GLmstr\Company.h"&str$(cno),internal,outin,relative ioerr L1960
...
02040   for j=1 to 6
02050     execute "COPY Q:\GLmstr\"&fil$(j)&".h"&str$(cno)&" "&env$('Temp')&"\Work."&session$&" -83" ioerr L2300
02060     execute "COPY  "&env$('Temp')&"\Work."&session$&" Q:\GLmstr\"&fil$(j)&".h"&str$(cno)
02070     if j=2 or j=5 then goto L2080 else goto L2300
02080 L2080: open #1: "Name=Q:\GLmstr\"&fil$(j)&".h"&str$(cno)&",KFName=Q:\GLmstr\"&idx$(j)&".h"&str$(cno),internal,outin,keyed 
