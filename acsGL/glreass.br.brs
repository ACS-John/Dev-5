...
REORG: ! r:
	...
	do
		read #1,using Ftrans: mat ta eof EoTrans
		Ftrans: form pos 333,2*pd 3
		rewrite #1,using Ftrans: 0,0
	loop
	EoTrans: !
	lr2=lrec(2)
	...
	for j=1 to lr2
		...
		read #1,using Ftrans,key=k$: mat ta nokey L480
		if ta(1)=0 then ta(1)=j
		if ta(2)>0 then rewrite #2,using L470,rec=ta(2): j
		ta(2)=j
		rewrite #1,using Ftrans,key=k$: mat ta
