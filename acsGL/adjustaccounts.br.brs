...
10570     fnqglbig(1,mypos,0,2) : let resp$(1)=fnrglbig$(gl$)
10580     fnlbl(2,1,"Date of Balance:",mylen,right)
10590     fntxt(2,mypos,8,8,1,"1",0) : let resp$(2)=cnvrt$("pic(zzzzzz)",adjustdate)
10600     fnlbl(3,1,"Balance on Date:",mylen,right)
10610     fntxt(3,mypos,14,0,right,"10",0) : let resp$(3)=''
10620     fncmdset(11)
...
10760     fntxt(2,mypos,14,0,right,"10",0) : let resp$(1)=str$(balance)
10770     for period=1 to 13
10780       fnlbl(3+period,1,"Period "&str$(period)&":",mylen,right)
10790       fntxt(3+period,mypos,14,0,right,"10",0) : let resp$(period+1)=str$(ymbal(period))
10800     next period
10810     for period=1 to 13
10820       fnlbl(3+period,mypos+20,"Prior:",mylen,right)
10830       fntxt(3+period,mypos*2+20,14,0,right,"10",0) : let resp$(period+14)=str$(priorym(period))
10840     next period
