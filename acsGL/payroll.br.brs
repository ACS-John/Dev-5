...
00420 ! fnCOMBOF("PRmstr",1,MYPOS,27,env$('Q')&"\GLmstr\PRmstr.h"&str$(cno),1,4,5,30,'',0,PAS, "Choose from the list of employees.  Click Add Employee to add a new employee not shown on list.",0) !:
        resp$(1)=str$(eno)
...
00510   holden1=en1=val(resp$(1)(1:4))
...
00970   if ckey=7 then disable=0: goto DISPLAY_EMPLOYEE
...
01360 L1360: fnFree(env$('Q')&"\GLmstr\PRIndex.h"&str$(cno))
...
02680   mat ml$(4) !:
        ...
        ml$(2)="("&trim$(cnvrt$("pic(zzzz,zzz.##)",wh))&") does not equal" !:
...
02750   if ta(1)=0 then ta(1)=lr2
02760   ta(2)=lr2
02770 L2770: if add=0 then mat prd=prd-pr1 ! file maintenance
02780   for j=3 to 20
02790     if j=3 then m1=1
02800     if j=4 then m1=3
02810     if j=5 then m1=5
02820     if j>5 then m1=(j-5)*2+5
02830     m(m1)=m(m1)+prd(j)
02840     m(m1+1)=m(m1+1)+prd(j)
02850   next j
