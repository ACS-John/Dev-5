...
dim resp$(3)*255,ymbal(13),priorym(13)
fnTop(program$)
fn_adjustaccounts
def fn_adjustaccounts
	...
	do
		...
		if gl$<>'' then
			...
			loop
			GLTRANSDONE: !
				if fn_confirmadjustment then rewrite #h_glmstr,using GLMSTR: dept,major,sub,startbal,balance,mat ymbal,mat priorym
		end if
	loop while gl$<>''
	close #h_glmstr:
	close #h_actrans:
...
def fn_getadjustment
	...
	fnQgl(1,mypos,0,2,0,60) : resp$(1)=fnRgl$(gl$,60)
