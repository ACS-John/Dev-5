...
00280   print newpage
00290   print fields "10,20,C 30,H,N": " FUND BALANCE IN PROCESS"
00300   print fields "12,2,C 18,B,5": " Press F5 to stop"
00310 L310: read #1,using L520: n$,d$,bb,cb,mat ta eof L550
00320   let fund=val(n$(1:3)): let acct=val(n$(4:9))
00330   if acct>90 then goto L460
00340   if acct<1 then goto L310
00350   let bank$(acct)=d$(1:25)
00360   let begb(acct)=bb
...
00420   if tr(5)>0 then let bankdr(acct)=bankdr(acct)+tr(5)
00430   if tr(5)<0 then let bankcr(acct)=bankcr(acct)+tr(5)
00440   goto L390
...
00620 L620: print #255,using L630: date$('mm/dd/yy'),cnam$
00630 L630: form pos 1,c 8,cc 74
00640   print #255,using L650: time$,"Fund Balance Report"
00650 L650: form pos 1,c 8,pos 31,c 28,skip 1
00660   let p1=p1+1
00670   print #255,using L680: rtrm$(pedat$),"PAGE ",p1
00680 L680: form pos 20,cc 40,pos 70,c 5,n 4,skip 2
00690   print #255,using L700: "  BEGINNING"," "," ","     ENDING"
00700 L700: form pos 29,4*c 13,skip 1
00710   print #255,using L720: "   BALANCE","       INCOME","      EXPENSE","    BALANCE"
00720 L720: form pos 29,4*c 13,skip 0
00730   print #255,using L700: " ___________"," ____________"," ____________"," ___________"
00740   return 
00750 L750: gosub L620
00760   for j=1 to 10
00770     print #255,using L780: desc$(j),beg(j),-inc(j),disb(j),beg(j)-inc(j)-disb(j)
00780 L780: form pos 1,c 25,pos 28,4*pic(--,---,---.--),skip 1
00790   next j
00800   print #255,using L700: "------------","------------","------------","------------"
00810   print #255,using L780: "TOTAL",sum(beg),-sum(inc),sum(disb),sum(beg)-sum(inc)-sum(disb)
00820   print #255,using L700: "============","============","============","============"
00830   print #255,using L870: "ACCOUNT BALANCES"
00840   print #255,using L700: "  BEGINNING"," "," ","     ENDING"
00850   print #255,using L720: "   BALANCE","       INCOME","      EXPENSE","    BALANCE"
00860   print #255,using L700: " ___________"," ____________"," ____________"," ___________"
00870 L870: form skip 4,pos 34,c 20,skip 2
00880   for j=1 to 90
00890     if rtrm$(bank$(j))="" then goto L910
00900     print #255,using L780: bank$(j),begb(j),bankdr(j),-bankcr(j),endb(j)
00910 L910: next j
00920   print #255,using L700: "------------","------------","------------","------------"
00930   print #255,using L780: "TOTAL",sum(begb),sum(bankdr),-sum(bankcr),sum(endb)
00940   print #255,using L700: "============","============","============","============"
00950   let fncloseprn
00960   return 
00970 ! ______________________________________________________________________
00980   print #255: newpage
00990   gosub L620
01000   return 
...
01050   print fields "18,25,C 32,R,N": "Press F1 to continue; F5 to stop"
01060   print fields "5,20,c 45,n": "ENTER THE FUND BALANCE AT BEGINNING OF MONTH"
01070   for j=1 to 10
01080     print fields str$(j+6)&",25,C 20,N": desc$(j) !:
          let io1$(j)=str$(j+6)&",45,N 12.2,UT,N"
01090   next j
01100 L1100: input fields mat io1$,attr "R": mat beg conv L1100
01110   if ce>0 then let io1$(ce)(ce1:ce2)="U": let ce=0
01120   if cmdkey>0 then goto L1190 else let ce=curfld
01130 L1130: let ce=ce+1: if ce>udim(io1$) then let ce=1
01140 L1140: let io1$(ce)=rtrm$(uprc$(io1$(ce))) : let ce1=pos(io1$(ce),"U",1) !:
        if ce1=0 then goto L1130
01150   let ce2=ce1+1 : let io1$(ce)(ce1:ce1)="UC" : goto L1100
01160 CONV1: if ce>0 then let io1$(ce)(ce1:ce2)="U"
01170   let ce=cnt+1
01180 ERR1: print fields "24,78,C 1": bell : goto L1140
01190 L1190: ! 
01200   print newpage
01210   return 
...
01270   print "PROGRAM PAUSE: Type GO and press [Enter] to continue." : print "" : pause : goto ERTN_EXEC_ACT
