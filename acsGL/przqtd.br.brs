! Replace S:\acsGL\PRZQTD
! Zero Quarterly Information (After-Fact-Payroll)
 
	autoLibrary
	on error goto Ertn
 
	dim n(2),k$(3)*25,ss$*11,m(36),adr(2),cnam$*40,cap$*128,ml$(3)*80
 
	fnTop(program$,cap$="Zero QTD Payroll Information")
	fncno(cno,cnam$)
	if fnprocess=1 then goto L290
L140: fnTos(sn$="PrZqtr") : _
	...
	pas$=uprc$(resp$(1))
	if pas$="ZERO" then goto L290
MSGBOX1: !
	mat ml$(2) : _
	ml$(1)="          Incorrect password! " : _
	ml$(2)="Click OK to try again; else Cancel to stop." : _
	fnmsgbox(mat ml$,resp$,cap$,49)
	if resp$="OK" then goto L140 else goto Xit
 
L290: open #1: "Name=[Q]\GLmstr\PRmstr.h[cno],KFName=[Q]\GLmstr\PRINDEX.h[cno],Shr",internal,outIn,keyed
L300: read #1,using 'Form POS 91,36*PD 5.2': mat m eof L340
