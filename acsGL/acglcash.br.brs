...
! Cash Flow Statement
	...
	dim b$*3,a$(8)*30,oldtrans$*16,g(8),accum(9,7),cap$*128
...
L700: if ir=val(r$) then total+=(cb-bb) else goto L840
	if te$="B" then total-=(cb-bb): total-=bb: total2-=bp(nap) : goto L730
	total2+=cb
...
L860: notrans=1
L870: for j=1 to 9
		if ac(j)<>9 then accum(j,1)+=total : accum(j,2)+=total2
	next j
...
L1630: !
	pr #255,using L1640: underlin$,underlin$
	L1640: form skip redir,pos 52,2*c 15,skip redir
L1650: !
	if redir=0 then pr #255,using L1660: " "
...
L1880: !
	...
	fnTxt(1,mypos,12,0,right,'10',0,"Enter the total for the current month.",0 )
	resp$(1)=str$(total)
	fnLbl(2,1,"Total Year to Date:",mylen,right)
	fnTxt(2,mypos,12,0,right,'10',0,"Enter the total for the year.",0 )
	resp$(2)=str$(total2)
