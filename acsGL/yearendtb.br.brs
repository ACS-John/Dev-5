...
00330   let costcent=val(resp$(2)) !:
        let n$=lpad$(str$(costcent),3)&"     0  0"
00340   if resp$(3)="True" then let subt=1 else let subt=0
...
00470   if subt=1 and olddno>0 and olddno<>dno then print #255,using L480: "FUND "&str$(olddno)&" TOTALS",fundt3 else goto L500
00480 L480: form skip 1,pos 30,c 20,pos 80,pic(zz,zzz,zzz.## cr),skip 2
00490   let fundt1=fundt2=fundt3=0
00500 L500: let ano=val(n$(4:9))
00510   let sno=val(n$(10:12))
00520   let begbal=0
00530   let curbal=curbal+bp(12)
00540   let fundt1=fundt1+0
00550   let fundt3=fundt3+bp(12)
00560   gosub L840
00570   goto READ_1
00580 L580: let olddno=dno
00590 L590: print #255: 
00600   if subt=1 then !:
          print #255,using L480: "Fund "&str$(olddno)&" Totals",0 ,0,fundt3
00610   print #255: ,"Trial Balance Proof Totals";
00620   print #255,using L630: 0,0,curbal
...
00700 HDR2: ! 
00710   print #255,using L730: date$('mm/dd/yy'),cnam$
00720   print #255,using L730: time$,cap$
00730 L730: form pos 1,c 8,pos 15,cc 50
00740   print #255,using L750: fnpedat$,"Page ",p1+=1
00750 L750: form pos 15,cc 50,pos 80,c 5,n 4,skip 2
00760   print #255,using L770: "Account","Reference","Ending"
00770 L770: form pos 6,c 7,pos 70,c 9,pos 84,c 9,pos 99,c 7,pos 116,c 6,skip 1
00780   print #255,using L790: "Number","Account Name/Transaction Description","Date  Source","Number","Balance"
00790 L790: form pos 6,c 6,pos 17,c 36,pos 54,c 13,pos 71,c 6,pos 85,c 7
00800   print #255,using L810: "__________","____________________________________","____","______","___________","_________"
...
00840 L840: ! 
00850   print #255,using L860: dno,ano,sno,d$,bp(12) pageoflow L890
