...
! Quarterly UC Report (From the after-the-fact payroll files in gl)
	...
	dim k(1),k$(3)*25,l$(1)*11,xm(36)
	dim fa$(3),sa$(3)*40
	dim a$(3)*40,b$(2)*12,c$*5

	fnTop(program$,"Print State UC Report")
...
L350: form pos 1,n 4,3*c 25,c 11,36*pd 5.2,2*n 5
	if xm(2)=0 or k(1)=0 then goto L530
	deducy=deducq=0
	for j=1 to 10
		if deduc(j)=1 then deducy=deducy+xm(j*2+9)
		if deduc(j)=1 then deducq=deducq+xm(j*2+10)
	next j
	xm(1)=xm(1)-deducy
	xm(2)=xm(2)-deducq
...
L790: p3=p3+1
	if xm(1)<ucm then goto L860
	if xm(1)-xm(2)>ucm then goto L840
	h2=ucm-(xm(1)-xm(2))
	goto L870
L840: h2=0
	goto L870
L860: h2=xm(2)
L870: h3=xm(2)-h2
	pr #255,using L890: l$(1),k$(1),xm(2),h3,h2,xm(34)
