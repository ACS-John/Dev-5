! Replace S:\acsGL\ACGLIncP
! -- INCOME STATEMENT FOR 8 1/2 * 11 PAPER WITH PERCENTAGES
 
	autoLibrary
	on error goto Ertn
 
	dim fl1$*256,tp1(4),by(13),cap$*128
	dim cnam$*40,b$*3,a$(8)*30,oldtrans$*16,g(8),accum(9,2),bp(13),udf$*256
	dim r$*5,d$*50,te$*1,ac(9),report$*50,secondr$*50,foot$*132,underlin$*25
 
	fnTop(program$,cap$="Income Statement with Percents")
	fncno(cno,cnam$)
	udf$=env$('temp')&'\'
	actpd=fnactpd
	actpd$=fnactpd$
...
L190: form pos mp1,pd 3,pos 81,41*pd 6.2
	pas=1 : open #hwork:=4: "Name="&env$('temp')&"\Work."&session$&",KFName=IDX."&wsid$&",Replace,RecL=33,KPS=1,KLN=5",internal,outIn,keyed
L210: acglfnsi=1 : _
...
L330: on fkey 5 goto L1910
	...
	goto L390
L380: execute "Index [Q]\GLmstr\GLmstr.h[cno] "&udf$&"fsindex.H[cno] 72 3 Replace DupKeys -N"
L390: open #3: "Name=[Q]\GLmstr\GLmstr.h[cno],KFName="&udf$&"fsindex.h[cno],Shr",internal,input,keyed
