...
! Statement of Change in Financial Position with Comparison : _
	! for Letter size paper
 
	autoLibrary
	on error goto Ertn
 
	dim fl1$*256,actpd$*6,cogl$(3)*12,pedat$*20
	dim r$*5,d$*50,te$*1,ac(9),report$*50,secondr$*50,foot$*132,underlin$*14
	dim cnam$*40,b$*3,a$(8)*30,oldtrans$*16,g(8),accum(9,2),udf$*256
	dim acct$*12,bp(13),lastact$*12,d(2),by(13),cap$*128,message$*40
 
	fnTop(program$,cap$="Change in Financial Position")
	fncno(cno,cnam$)
	udf$=env$('temp')&'\'
...
BEGIN_PRINTING: !
	...
	goto L560
L550: execute "Index [Q]\GLmstr\GLmstr.h[cno] "&udf$&"fsindex.H[cno] 78 3 Replace DupKeys -N"
L560: open #3: "Name=[Q]\GLmstr\GLmstr.h[cno],KFName="&udf$&"fsindex.h[cno],Shr",internal,input,keyed
L570: read #1,using L610: r$,d$,te$,sp,ls,ds,ul,rs,bc,ap,mat ac,ic,fc eof DONE
...
L750: read #3,using L770: fr,bb,cb,mat by,mat bp,pbp noRec L740 eof L870
...
L810: if fr=val(r$) then goto L820 else goto L850
L820: if priorcd=2 then total+=(cb-pbp) else total+=(cb-bp(12))
	goto L740
 
L850: if fr<val(r$) then goto L740
	if fr>val(r$) then goto L880
L870: notrans=1
L880: for j=1 to 9 : accum(j,1)+=total : next j
	if rs=1 then total=-total else goto L900
L900: if ds=1 then dollar$="$" else dollar$=" "
	if total><0 then goto L930
	if ls+ul+ds+ic>0 then goto L930 else goto L570
L930: sp2=67-sp-1
...
L1310: if ls=0 then goto L1450
	if ls=99 then goto L1360
	pr #255,using L1340: " "
...
L1730: form pos 72,pic(zzzz),skip 2
	on error goto Ertn
	goto L1790
L1760: pr #255: tab(68);"Current Year"
	on error goto Ertn
	pr #255:
L1790: return
