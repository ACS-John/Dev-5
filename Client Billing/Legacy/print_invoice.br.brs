! Replace S:\Client Billing\Legacy\Print_Invoice.br
...
def library fnprint_invoice(out,align, &actnum$, mat billto$, inv_num$, inv_date, mat desc$, mat amt,pbal,ebilling;pdfline$*255)
	...
	! pr #out: "*INSERT FILE:S:\Client Billing\Legacy\acs_logo.rtf"
	! pause 
	if ebilling=0 then pr #out: "*INSERT FILE:S:\Client Billing\ACS_Logo2.rtf" else pr #out: "[pos(+0,+67)][pic(.5,.5,s:\Client Billing\Legacy\bwlogo2.jpg)]"   ! "[PIC(1,1,S:\Client Billing\ACS_Logo2.rtf)]"
	pr #out: ''
	pr #out: ''
	pr #out: ''
	pr #out: ''
	...
	if ebilling=0 then pr #out: "*INSERT FILE:S:\Client Billing\Legacy\black line - six inch.rtf.txt" else pr #out: pdfline$ : print #out: ""
	...
	if ebilling=0 then pr #out: "*INSERT FILE:S:\Client Billing\Legacy\black line - six inch.rtf.txt" else pr #out: pdfline$
	if ebilling=1 then pr #out: ""
	...
	P_TOTAL: !
	if ebilling=0 then pr #out,using "form pos 59,C 28": "{\strike             }" else pr #out: "" ! using "form pos 1,c 100" : "[PIC(1,1,S:\Client Billing\Legacy\black line - six inch.rtf.txt)]"
	...
	if ebilling=0 then pr #out,using "form pos 59,C 28": "{\ul \strike "&rpt$(" ",12)&"}" else pr #out: "" ! ,using "form pos 1,c 100" : "[PIC(1,1,S:\Client Billing\Legacy\black line - six inch.rtf.txt)]"
	if ebilling=0 then pr #out: "*INSERT FILE:S:\Client Billing\Legacy\black line - six inch.rtf.txt" else pr #out: "[pos(+0,-2)]"&pdfline$
	! if ebilling=1 then pr #out: ""
	total_amt=0
fnend 
