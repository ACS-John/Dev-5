...
def fn_customer(; &editOne$,___,editOne,ckey)
	...
	fn_customer2legacy(mat customer$,mat customerN,z$,mat e$,f$(1),mat a,mat xb,mat c,mat xd,bal,lastBillingDate,mat g,mat adr,alp$,f$(2),f$(3),bra_legacy,mat gb,df$,dr$,dc$,da$,mat extra,mat extra$)
	! read #hCustomer1,using F_CUSTOMER_1,key=x$: z$,mat e$,f$(1),mat a,mat xb,mat c,mat xd,bal,lastBillingDate,mat g,mat adr,alp$,f$(2),f$(3),bra_legacy,mat gb,df$,dr$,dc$,da$,mat extra,mat extra$ nokey ACCOUNT_X_NOKEY
	...
	EDIT_LOADED_CUSTOMER: !
		...
		oldService1DepositAmount=xb(8)
		oldService2DepositAmount=xb(9)
		oldService3DepositAmount=xb(10)
	...
	REWRITE_RECORD: ! r:
		...
		if oldService2DepositAmount<>xb(9) then
			fn_depositChangeLog(z$,oldService2DepositAmount,xb(9),date('mmddyy'),trim$(srvnam$(2))(1:15)&' Deposit Changed')
		end if
		if oldService3DepositAmount<>xb(10) then
			fn_depositChangeLog(z$,oldService3DepositAmount,xb(10),date('mmddyy'),trim$(srvnam$(3))(1:15)&' Deposit Changed')
		end if
		if oldService4DepositAmount<>xb(11) then
			fn_depositChangeLog(z$,oldService4DepositAmount,xb(11),date('mmddyy'),trim$(srvnam$(4))(1:15)&' Deposit Changed')
		end if
	...
	ACC_KC_VALID_ROUTE_TEST: ! r:
		...
		! rewrite #hCustomer1,using F_CUSTOMER_1: z$,mat e$,f$(1),mat a,mat xb,mat c,mat xd,bal,lastBillingDate,mat g,mat adr,alp$,f$(2),f$(3),bra_legacy,mat gb,df$,dr$,dc$,da$,mat extra,mat extra$
	...
	ADD_CONTINUE: ! r:
		z$=x$ : mat e$=('') : e$(4)=newe4$
		mat f$=('') : mat a=(0) : mat xb=(0) : mat c=(0) : mat xd=(0)
		...
		! 		write #hCustomer1,using F_CUSTOMER_1: z$,mat e$,f$(1),mat a,mat xb,mat c,mat xd,bal,lastBillingDate,mat g,mat adr,alp$,f$(2),f$(3),bra_legacy,mat gb,df$,dr$,dc$,da$
...
ServiceScreen: ! r:
	...
	if ckey>20 and ckey<=30 then
		...
		if service_code=1 then
			...
			fnLbl(srvLine+=1,1,'Deposit:'        ,srvCol1len,1) : fnTxt(srvLine,srvCol2pos, 8,0,1,'10') : rateInfo$(respc+=1)=str$(xb(8))  ! 6
			fnLbl(srvLine+=1,1,'Deposit Date:'   ,srvCol1len,1) : fnTxt(srvLine,srvCol2pos, 8,0,1,'1' ) : rateInfo$(respc+=1)=str$(c(1))  ! 7
			fnLbl(srvLine+=1,1,'Standard Charge:',srvCol1len,1) : fnTxt(srvLine,srvCol2pos,10,0,1,'10') : rateInfo$(respc+=1)=str$(xb(1))  ! 8
			fnLbl(srvLine+=1,1,'Current Reading:',srvCol1len,1) : fnTxt(srvLine,srvCol2pos,11,0,1,'20') : rateInfo$(respc+=1)=str$(xd(1))  ! 9
			fnLbl(srvLine+=1,1,'Prior Reading:'  ,srvCol1len,1) : fnTxt(srvLine,srvCol2pos,11,0,1,'20') : rateInfo$(respc+=1)=str$(xd(2))  ! 10
			fnLbl(srvLine+=1,1,'Usage - Current:',srvCol1len,1) : fnTxt(srvLine,srvCol2pos,11,0,1,'20') : rateInfo$(respc+=1)=str$(xd(3))  ! 11
			fnLbl(srvLine+=1,1,'Usage - YTD:'    ,srvCol1len,1) : fnTxt(srvLine,srvCol2pos,11,0,1,'20') : rateInfo$(respc+=1)=str$(xd(4))  ! 12
			fnLbl(srvLine+=1,1,'Unit Count:'     ,srvCol1len,1) : fnTxt(srvLine,srvCol2pos, 5,0,1,'20') : rateInfo$(respc+=1)=str$(xd(13)) ! 13
		else if service_code=2 then
			fnLbl(srvLine+=1,1,'Deposit:'         ,srvCol1len,1) : fnTxt(srvLine,srvCol2pos, 8,0,1,'10') : rateInfo$(respc+=1)=str$(xb(9))
			fnLbl(srvLine+=1,1,'Deposit Date:'    ,srvCol1len,1) : fnTxt(srvLine,srvCol2pos, 8,0,1,'1' ) : rateInfo$(respc+=1)=str$(c(2))
			fnLbl(srvLine+=1,1,'Standard Charge:' ,srvCol1len,1) : fnTxt(srvLine,srvCol2pos,10,0,1,'10') : rateInfo$(respc+=1)=str$(xb(2))
			...
			if srv$(service_code)='EL' or srv$(service_code)='LM' then
				...
				fnLbl(srvLine+=1,1,'Deposit:'        ,srvCol1len,1) : fnTxt(srvLine,srvCol2pos, 8,0,1,'10') : rateInfo$(respc+=1)=str$(xb(10))
				fnLbl(srvLine+=1,1,'Deposit Date:'   ,srvCol1len,1) : fnTxt(srvLine,srvCol2pos, 8,0,1,'1' ) : rateInfo$(respc+=1)=str$(c(3))
				fnLbl(srvLine+=1,1,'Standard Charge:',srvCol1len,1) : fnTxt(srvLine,srvCol2pos,10,0,1,'10') : rateInfo$(respc+=1)=str$(xb(3))
				fnLbl(srvLine+=1,1,'Current Reading:',srvCol1len,1) : fnTxt(srvLine,srvCol2pos, 9,0,1,'20') : rateInfo$(respc+=1)=str$(xd(5))
				fnLbl(srvLine+=1,1,'Prior Reading:'  ,srvCol1len,1) : fnTxt(srvLine,srvCol2pos, 9,0,1,'20') : rateInfo$(respc+=1)=str$(xd(6))
				fnLbl(srvLine+=1,1,'Usage - Current:',srvCol1len,1) : fnTxt(srvLine,srvCol2pos, 9,0,1,'20') : rateInfo$(respc+=1)=str$(xd(7))
				fnLbl(srvLine+=1,1,'Usage - YTD:'    ,srvCol1len,1) : fnTxt(srvLine,srvCol2pos, 9,0,1,'20') : rateInfo$(respc+=1)=str$(xd(8))
				if srv$(service_code)='EL' then  !      ! skip rest of information on lawn meters
					fnLbl(srvLine+=1,1,'Demand Reading:'   ,srvCol1len,1) : fnTxt(srvLine,srvCol2pos,9,0,1,'20') : rateInfo$(respc+=1)=str$(xd(15))
					fnLbl(srvLine+=1,1,'Demand Multiplier:',srvCol1len,1) : fnTxt(srvLine,srvCol2pos,9,0,1,'33') : rateInfo$(respc+=1)=str$(xd(14)*.001)
			...
			if srv$(service_code)='GA' then ! show all of gas information (unless field is used for something other than gas)
				...
				fnLbl(srvLine+=1,1,'Deposit:'        ,srvCol1len,1) : fnTxt(srvLine,srvCol2pos, 8,0,1,'10') : rateInfo$(respc+=1)=str$(xb(11))
				fnLbl(srvLine+=1,1,'Deposit Date:'   ,srvCol1len,1) : fnTxt(srvLine,srvCol2pos, 8,0,1,'1' ) : rateInfo$(respc+=1)=str$(c(4))
				fnLbl(srvLine+=1,1,'Standard Charge:',srvCol1len,1) : fnTxt(srvLine,srvCol2pos,10,0,1,'10') : rateInfo$(respc+=1)=str$(xb(4))
				fnLbl(srvLine+=1,1,'Current Reading:',srvCol1len,1) : fnTxt(srvLine,srvCol2pos,12,0,1,'20') : rateInfo$(respc+=1)=str$(xd(9))
				fnLbl(srvLine+=1,1,'Prior Reading:'  ,srvCol1len,1) : fnTxt(srvLine,srvCol2pos,12,0,1,'20') : rateInfo$(respc+=1)=str$(xd(10))
				fnLbl(srvLine+=1,1,'Usage - Current:',srvCol1len,1) : fnTxt(srvLine,srvCol2pos,12,0,1,'20') : rateInfo$(respc+=1)=str$(xd(11))
				fnLbl(srvLine+=1,1,'Usage - YTD:'    ,srvCol1len,1) : fnTxt(srvLine,srvCol2pos,12,0,1,'20') : rateInfo$(respc+=1)=str$(xd(12))
			...
			fnLbl(srvLine+=1,1,'Standard Charge:',srvCol1len,1) : fnTxt(srvLine,srvCol2pos,10,0,1,'10',0) : rateInfo$(respc+=1)=str$(xb(5))
		else if service_code=6 then
			fnLbl(srvLine+=1,1,'Standard Charge:',srvCol1len,1) : fnTxt(srvLine,srvCol2pos,10,0,1,'10') : rateInfo$(respc+=1)=str$(xb(6))
		else if service_code=7 then
				! (place holder for future developemnt)
		else if service_code=8 then
			fnLbl(srvLine+=1,1,'Standard Charge:',srvCol1len,1) : fnTxt(srvLine,srvCol2pos,10,0,1,'10') : rateInfo$(respc+=1)=str$(xb(7))
		...
		if ckey<>5 then ! r: get local values out of mat rateInfo$ and Save the record
			...
			if service_code=1 then !  r: 1ST SERVICE  -  Water
				...
				xb(1)     =val(rateInfo$(8))
				xd(1)     =val(rateInfo$(9))
				xd(2)     =val(rateInfo$(10))
				xd(3)     =val(rateInfo$(11))
				xd(4)     =val(rateInfo$(12))
				xd(13)    =val(rateInfo$(13))
				 ! /r
				...
				xb(10)    =val(rateInfo$(6))
				c(3)     =val(rateInfo$(7))
				xb(3)     =val(rateInfo$(8))
				xd(5)     =val(rateInfo$(9))
				xd(6)     =val(rateInfo$(10))
				xd(7)     =val(rateInfo$(11))
				xd(8)     =val(rateInfo$(12))
				xd(15)    =val(rateInfo$(13))
				xd(14)    =val(rateInfo$(14))*1000
				...
				xb(11)    =val(rateInfo$(6))
				c(4)     =val(rateInfo$(7))
				xb(4)     =val(rateInfo$(8))
				xd(9)     =val(rateInfo$(9))
				xd(10)    =val(rateInfo$(10))
				xd(11)    =val(rateInfo$(11))
				xd(12)    =val(rateInfo$(12))
...
def fn_legacy2customer(mat customer$,mat customerN,&z$,mat e$,&f1$,mat a,mat xb,mat c,mat xd,&bal,&lastBillingDate,mat g,mat adr,&alp$,&f2$,&f3$,&bra_legacy,mat gb,&df$,&dr$,&dc$,&da$,mat extra,mat extra$)
	...
	customerN(c_s01stdCharge      	)			=xb(1)
	customerN(c_s02stdCharge      	)			=xb(2)
	customerN(c_s03stdCharge      	)			=xb(3)
	customerN(c_s04stdCharge      	)			=xb(4)
	customerN(c_s05stdCharge      	)			=xb(5)
	customerN(c_s06stdCharge      	)			=xb(6)
	customerN(c_s07stdCharge      	)			=xb(7)
	customerN(c_s01depositAmt     	)			=xb(8)
	customerN(c_s02depositAmt     	)			=xb(9)
	customerN(c_s03depositAmt     	)			=xb(10)
	customerN(c_s04depositAmt     	)			=xb(11)
	customerN(c_s01depositDate    	)			=c(1)
	customerN(c_s02depositDate    	)			=c(2)
	customerN(c_s03depositDate    	)			=c(3)
	customerN(c_s04depositDate    	)			=c(4)
	customerN(c_s01readingCur     	)			=xd(1)
	customerN(c_s01readingPri     	)			=xd(2)
	customerN(c_s01UsageCur       	)			=xd(3)
	customerN(c_s01UsageYtd       	)			=xd(4)
	customerN(c_s03readingCur     	)			=xd(5)
	customerN(c_s03ReadingPri     	)			=xd(6)
	customerN(c_s03UsageCur       	)			=xd(7)
	customerN(c_s03UsageYtd       	)			=xd(8)
	customerN(c_s04readingCur     	)			=xd(9)
	customerN(c_s04readingPri     	)			=xd(10)
	customerN(c_s04usageCur       	)			=xd(11)
	customerN(c_s04usageYtd       	)			=xd(12)
	customerN(c_s01unitCount      	)			=xd(13)
	customerN(c_demandMultiplier  	)			=xd(14)
	customerN(c_demandReading     	)			=xd(15)
...
fnend
def fn_customer2legacy(mat customer$,mat customerN,&z$,mat e$,&f1$,mat a,mat xb,mat c,mat xd,&bal,&lastBillingDate,mat g,mat adr,&alp$,&f2$,&f3$,&bra_legacy,mat gb,&df$,&dr$,&dc$,&da$,mat extra,mat extra$)
	...
	xb(1)                 	=customerN(c_s01stdCharge      		)
	xb(2)                 	=customerN(c_s02stdCharge      		)
	xb(3)                 	=customerN(c_s03stdCharge      		)
	xb(4)                 	=customerN(c_s04stdCharge      		)
	xb(5)                 	=customerN(c_s05stdCharge      		)
	xb(6)                 	=customerN(c_s06stdCharge      		)
	xb(7)                 	=customerN(c_s07stdCharge      		)
	xb(8)                 	=customerN(c_s01depositAmt     		)
	xb(9)                 	=customerN(c_s02depositAmt     		)
	xb(10)                	=customerN(c_s03depositAmt     		)
	xb(11)                	=customerN(c_s04depositAmt     		)
	c(1)                 	=customerN(c_s01depositDate    		)
	c(2)                 	=customerN(c_s02depositDate    		)
	c(3)                 	=customerN(c_s03depositDate    		)
	c(4)                 	=customerN(c_s04depositDate    		)
	xd(1)                 	=customerN(c_s01readingCur     		)
	xd(2)                 	=customerN(c_s01readingPri     		)
	xd(3)                 	=customerN(c_s01UsageCur       		)
	xd(4)                 	=customerN(c_s01UsageYtd       		)
	xd(5)                 	=customerN(c_s03readingCur     		)
	xd(6)                 	=customerN(c_s03ReadingPri     		)
	xd(7)                 	=customerN(c_s03UsageCur       		)
	xd(8)                 	=customerN(c_s03UsageYtd       		)
	xd(9)                 	=customerN(c_s04readingCur     		)
	xd(10)                	=customerN(c_s04readingPri     		)
	xd(11)                	=customerN(c_s04usageCur       		)
	xd(12)                	=customerN(c_s04usageYtd       		)
	xd(13)                	=customerN(c_s01unitCount      		)
	xd(14)                	=customerN(c_demandMultiplier  		)
	xd(15)                	=customerN(c_demandReading     		)
