...
21020     end if 
21040   next j
21060   if pencount<1 then pencount=1
...
21270     ! if env$('acsDeveloper')<>'' and trim$(z$)='100001.00' then debug_this_account=1 else debug_this_account=0
...
21920 ERTN: fnerror(program$,err,line,act$,"xit")
21940   if uprc$(act$)<>"PAUSE" then goto ERTN_EXEC_ACT
21960   execute "List -"&str$(line) : pause : goto ERTN_EXEC_ACT
21980   pr "PROGRAM PAUSE: Type GO and press [Enter] to continue." : pr "" : pause : goto ERTN_EXEC_ACT
...
22940     if resp$(8)="True" then penaltybase$="Balance" ! base penalties on current balance
22960     if pendat=0 then 
22980       msgline$(1)="You must enter a valid Penalty Date"
23000       fnmsgbox(mat msgline$,pause$,'',48)
23020       goto SM_ASK
23040     else if bildat=0 then 
23060       msgline$(1)="You must enter a valid Last Billing Date."
...
23549     pr #255: ''
23553     pr #255: '*Math for Account: '&z$&'  Penalty Based on '&penaltybase$
23557   end if 
...
23882       if show_math then pr #255: '     have budget and have not paid last payment'
23884       if show_math then pr #255: '       Base set to '&str$(g(j))
23886       base=g(j)
23888       !       goto L1060
23900     else if penaltybase$="Bill" and usebalance=0 then ! base on current bill
23905       if show_math then pr #255: '     Base on current bill ( if penaltybase$="Bill" and usebalance=0 )'
23906       if show_math then pr #255: '       usebalance='&str$(usebalance)
23910       if show_math then pr #255: '       Base set to '&str$(g(j))
23915       base=g(j)
23920       !       goto L1060
23925     else if (penaltybase$="Balance" or usebalance=1) and subjectto(j)<>0 and gb(j)>0 then 
23930       if show_math then pr #255: '     (penaltybase$="Balance" or usebalance=1) and subjectto('&str$(j)&')<>0 and gb('&str$(j)&')'
23935       if show_math then pr #255: '       Base set to '&str$(gb(j))
23940       base=gb(j)
23945     else 
23946       if show_math then pr #255: '     base set to zero'
23950       base=0 ! base on balance
23955     end if 
23960     if (penaltybase$="Balance" or usebalance=1) and subjectto(j)<>0 and gb(j)<0 then 
23962       negatives+=gb(j)
23964     end if 
23980    ! L1060: !
...
24004         pr #255: '     accumulate all charges by the penalty they are subject to'
24006         pr #255: '       basepenalty('&str$(subjectto(j))&')=itself + base(which is '&str$(base)&') totalling '&str$(basepenalty(subjectto(j))+base)
24008       end if 
24010       basepenalty(subjectto(j))=basepenalty(subjectto(j))+base
24012     end if 
...
24024         pr #255: '     if the service is any of the penalties, add the previous penalty into the balance'
24026         pr #255: '       basepenalty('&str$(j)&')=itself + base(which is '&str$(base)&') totalling '&str$(basepenalty(j)+base)
24028       end if 
24030       basepenalty(j)=basepenalty(j)+base
24032     end if 
24040   next j
24060   if env$('client')="Kimberling" and g(2)>0 then basepenalty(10)-=g(1) ! no penalty on water if they have swewer
24062   if show_math and negatives then pr #255: '     negatives='&str$(negatives)&' doing fn_worryabout'
24080   if negatives<>0 then let fn_worryabout
24100   GOT_BASEPENALTY: ! 
24102   if show_math then 
24104     for j=1 to 10
24106       if basepenalty(j)<>0 then 
24108         pr #255: '     Base Penalty ('&str$(j)&')= '&str$(basepenalty(j))
24110       end if 
24112     next j
24114   end if 
24116   ! /r
...
24220       penaltycode$=uprc$(service$(j))
24240       if j<6 then pencode=a(j) ! rate codes in customer layout are not in order.  The first 5 a( match the services. The next three services are pulled from mat extra. 9 and 10 use a(6)&a(7)
24260       if j=6 then pencode=extra(11)
24280       if j=7 then pencode=extra(12)
24300       if j=8 then pencode=extra(13)
24320       if j=9 then pencode=a(6)
24340       if j=10 then pencode=a(7)
24360       if env$('client')="Pennington" and pencode=0 then pencode=1 ! default to one so codes don't have to be added to old customer records
24380       if env$('client')="Granby" and pencode=0 then pencode=1 ! default to one so codes don't have to be added to old customer records
...
24642         ! if env$('client')='Exeter' then basepenalty(9)=basepenalty(10) : basepenalty(10)=0  ! XXX  This is for the first month only!
24645         if show_math and round(basepenalty(j)*rt(1,3),2)<>0 then 
24650           pr #255: '     calculated Penalty Base is '&penaltybase$&' '&str$(basepenalty(j))
24655           pr #255: '     Penalty: tg('&str$(j)&')=round('&str$(basepenalty(j))&'*'&str$(rt(1,3))&',2)'
24660           tg(j)=round(basepenalty(j)*rt(1,3),2)
24665           pr #255: '     tg('&str$(j)&')='&str$(tg(j))
24670         else 
24675           tg(j)=round(basepenalty(j)*rt(1,3),2) ! penalty based on base amount that was accumulated for each penalty field * rate for that penalty code
24680         end if 
24685         if show_math then 
24690           pr #255: '     tg('&str$(j)&')=max('&str$(mc1)&','&str$(tg(j))&')'
24695         end if 
24700         tg(j)=max(mc1,tg(j))
24705         if env$('client')="Pennington" then ! r:
24720           if a(6)=0 then a(6)=1 ! default tax code to 1
...
24760           ! tg(j)=round(tg(j)*(1+rt(1,3)),2) ! Pennington adds sales tax to penalty; but now they say they don't.  Not sure what happened there.
24780         end if  ! /r
24800       end if 
24820       column+=1
24840       tg(j)=max(0,tg(j)) ! no negative penalties
24860       pencolumn(column)=tg(j)
24880       coltot(column)=coltot(column)+tg(j) ! place first penalty in first column, column totals, etc
24900     end if 
24920   next j
...
25200   tamount=sum(mat tg) ! add all penalties into total transaction amount
...
25460   ! 
25480   totb+=bal
25500   if route_number<0 or route_number>99 then route_number=99
25520   route(route_number)+=sum(pencolumn)
25540   fn_print_record
25560   XIT_PENCAL: ! 
25580 fnend 
25600 def fn_print_header
25620   ! p+=1
25640   pr #255: "\qc "&env$('cnam')
25660   pr #255: "\qc  {\f181 \fs28 \b "&env$('program_caption')&" - Listing}"
25680   pr #255: dat$
25700   pr #255: "\ql   "
25720   tmp$="{\ul Account No  } {\ul Customer Name and Address      }         {\ul "
25740   for j=1 to udim(columnhead$)
25760     tmp$=tmp$&columnhead$(j)&"} { \ul "
25780   next j
25800   ! if env$('client')="Kimberling" then
25820   !   tmp$=tmp$&"} {\ul  Interest} {\ul     Balance}"
25840   if printadr=1 then 
25860     tmp$=tmp$&"} {\ul   Balance}  {\ul  Meter Address}"
25880   else 
25900     tmp$=tmp$&"} {\ul   Balance}"
25920   end if 
25940   pr #255: tmp$
25960   pr #255: ""
25980 fnend 
...
26100   !  pr #255,using F_PRLINE_KIM: z$,e$(2),pen_tmp,int_tmp,bal pageoflow PGOF
...
26200     pr #255,using F_PRLINE: z$,e$(2),mat pencolumn,bal,e$(1)(1:25) pageoflow PGOF
26220   else 
26240     pr #255,using F_PRLINE: z$,e$(2),mat pencolumn,bal pageoflow PGOF
26260   end if 
26280   if printmail=1 then 
26300     pr #255,using "Form POS 15,C 30": e$(3) pageoflow PGOF
26320     pr #255,using "Form POS 15,C 30": e$(4) pageoflow PGOF
...
26420   tmp$=rpt$(" ",52)&"{\ul"&rpt$(" ",12)&"}"
26440   column_count=udim(mat columnhead$)
26460   ! if env$('client')="Kimberling" then column_count+=1
26480   for j=1 to column_count
26500     tmp$=tmp$&" {\ul"&rpt$(" ",12)&"}"
26520   next j
26540   pr #255: tmp$
26560   ! if env$('client')="Kimberling" then
26580   !   pr #255,using "Form POS 17,C 30,x 5,3*N 12.2": "Overall Totals",pen_accum,int_accum,totb
26600   ! else 
26620   pr #255,using "Form POS 17,C 30,x 5,pencount*N 12.2,N 12.2": "Overall Totals",mat coltot,totb
26640   ! end if
26660   tmp$=rpt$(" ",52)&"{\ul \strike"&rpt$(" ",12)&"}"
26680   for j=1 to column_count
26700     tmp$=tmp$&" {\ul \strike"&rpt$(" ",12)&"}"
26720   next j
26740   pr #255: tmp$
26760   pr #255,using 'form skip 2,c 20': "Totals by Route"
26780   for j=1 to 99
26800     if route(j)<>0 then 
26820       pr #255,using "form pos 1,c 10,pic(zzz,zzz.zz)": "Route "&cnvrt$("pic(zz)",j),route(j) pageoflow PGOF
26840     end if 
26860   next j
26880   restore #1: 
26900   do 
26920     read #1,using "Form POS 292,PD 4.2": bal eof PT_EO1
26940     totalbal=totalbal+bal
26960   loop 
26980   PT_EO1: ! 
27000   pr #255: 
27020   balpos=51+(pencount*12)
27040   pr #255,using 'form pos 15,c 30,pos balpos,pic($-,---,---.##)': "Total Due from all Customers",totalbal
27060 fnend 
27080 def fn_bud1
27100   bud1=0
...
27760     negativepercent=round((sum(basepenalty)+negatives)/sum(basepenalty),2)
27780     for j=1 to 10
27800       basepenalty(j)=round(basepenalty(j)*negativepercent,2)
27820     next j
27840   end if 
27860 fnend 
...
27940   adjustment=0
27960   if round(bal*rt(1,3),2)-.01=sum(mat tg) then adjustment=.01 : goto CR_DOIT ! add one cent to a penalty breakdown amount
27980   if round(bal*rt(1,3),2)-.02=sum(mat tg) then adjustment=.02 : goto CR_DOIT ! add one cent to a penalty breakdown amount
28000   if round(bal*rt(1,3),2)+.01=sum(mat tg) then adjustment=-.01 : goto CR_DOIT ! subtract one cent from a penalty breakdown amount
28020   if round(bal*rt(1,3),2)+.02=sum(mat tg) then adjustment=-.02 : goto CR_DOIT ! subtract one cent from a penalty breakdown amount
28040   goto CR_XIT
28060   CR_DOIT: ! 
28080   penx=0
28100   for j=1 to 10 ! put rounding adjustment in first service that has a penalty
28120     if basepenalty(j)>0 then 
28140       tg(j)=tg(j)+adjustment
28160       penx+=1
28180       coltot(penx)=coltot(penx)+adjustment
28200       goto CR_XIT
28220     end if 
28240   next j
28260   CR_XIT: ! 
...
28580   mat tg=(0)
28600   tg(10)=pentot
28620   bal+=sum(mat tg)
28640   tot+=sum(mat tg)
28660   tcode=2 ! penalty trans code
28680   pencolumn(1)=tg(10)
28700   coltot(1)=coltot(1)+tg(10)
28720 fnend 
28740 def fn_flat_percent_on_lastbill(percentage)
28760   percentage=percentage/100
28780   tg(10)=round(tg(10)+g(11)*percentage,2)
28800   tg(10)=max(0,tg(10))
28820   pencolumn(1)=tg(10)
28840   coltot(1)=coltot(1)+tg(10)
28860 fnend 
28880 ! def fn_flat_percent_on_priorbal(percentage) ! not really the prior balance - but sorta is
28900 !  percentage=percentage/100
28920 !  pbal=bal-g(11)
28940 !  tg(10)=round(tg(10)+pbal*percentage,2)
28960 !  tg(10)=max(0,tg(10))
28980 !  pencolumn(1)=tg(10)
29000 !  coltot(1)=coltot(1)+tg(10)
29020 ! fnend
