...
LOOP_TOP: ! r: main loop
	...
	dim bulksort$*12
	dim a(7)
	read #hCustomer,using 'form pos 1,C 10,4*C 30,C 12,pos 217,15*PD 5,pos 373,C 12,pos 361,C 12,pos 1741,N 2,pos 1821,N 1,pos 143,7*pd 2,pos 1806,3*n 2,pos 1821,n 1,pos 1942,c 12': z$,mat e$,f1$,mat xd,f3$,f$(2),extra(1),fbc,mat a,extra(11),extra(12),extra(13),extra(17),bulksort$ eof Finis
	...
	for j=1 to udim(x_service$)
		...
		if enablePrior=1 and resp$(2)='[All]' and option$(j+1)(1:2)='WA' then
			prior=xd(3)
			x_service$(x+=1)=cnvrt$('pic(zzzzzzzzz)',xd(1))&ul$&cnvrt$('pic(zzzzzzzzz)',prior)
		end if
		if enablePrior=0 and resp$(2)='[All]' and option$(j+1)(1:2)='WA' then
			x_service$(x+=1)=cnvrt$('pic(zzzzzzzzz)',xd(1))&ul$
		end if
		...
		if enablePrior=1 and resp$(2)='[All]' and option$(j+1)(1:2)='EL' then
			prior=xd(7)
			x_service$(x+=1)=cnvrt$('pic(zzzzzzzzz)',xd(5))&ul$&cnvrt$('pic(zzzzzzzzz)',prior)
		end if
		if enablePrior=0 and resp$(2)='[All]' and option$(j+1)(1:2)='EL' then
			prior=xd(7)
			x_service$(x+=1)=cnvrt$('pic(zzzzzzzzz)',xd(5))&ul$
		end if
		...
		if enablePrior=1 and resp$(2)='[All]' and option$(j+1)(1:2)='GA' then
			prior=xd(11)
			x_service$(x+=1)=cnvrt$('pic(zzzzzzzzz)',xd(9))&ul$&cnvrt$('pic(zzzzzzzzz)',prior)
		end if
		if enablePrior=0 and resp$(2)='[All]' and option$(j+1)(1:2)='GA' then
			prior = xd(11)
			x_service$(x+=1)=cnvrt$('pic(zzzzzzzzz)',xd(9))&ul$
		end if
		...
		if enablePrior=0 and resp$(2)(1:2)='GA' and option$(j+1)(1:2)='GA' then
			prior=xd(11)
			x_service$(x+=1)=cnvrt$('pic(zzzzzzzzz)',xd(9))&ul$: mat x_service$(1)
			goto L940
		end if
	next j
	L940: !
...
def fn_campbell_meter_book ! Campbell Special Routine (printed once a year, one customer per page)
	dim z$*10,e$(4)*30,f1$*12,f2$*12,f3$*12,message$*40,g(12),xd(15)
	...
	CAMPBELL_LOOP_TOP: !
		dim g(12)
		read #hCustomer,using 'form pos 1,C 10,4*C 30,C 12,pos 217,15*PD 5,pos 373,C 12,pos 361,C 12,pos 1741,N 2,pos 1821,N 1,pos 143,7*pd 2,pos 1806,3*n 2,pos 1821,n 1,pos 1942,c 12,pos 300,12*pd 4.2': z$,mat e$,f1$,mat xd,f3$,f$(2),extra(1),fbc,mat a,extra(11),extra(12),extra(13),extra(17),bulksort$,mat g eof Finis
		watcode=a(1)
	...
	for j=1 to 39
		pr #255:''
	nex j
	pr #255,using F_CAMPBELL_OUT_W_1: xd(1),xd(3),G(1),G(2),G(5)
