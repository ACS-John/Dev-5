...
fn_setup
  if ~fnregistered_for_hh then
    ...
    fnmsgbox(mat ml$, response$, cap$,64)
    goto XIT
...
def fn_setup
  ...
  fnureg_read('Hand Held From File',preferenceHandHeldFromFile$)
...
def fn_transfer(bk$,enableMerge$,askPath$*128)
  ...
  if deviceSelected$='Aclara' then
    ...
    fn_amr(bookFile$)
	  ...
	  fn_badgerBeacon(fn_hh_input_filename$,bookFile$)
...
def fn_badgerBeacon(fileIn$*256,bookFile$*512; ___,returnN)
  if enableMerge$='True' and ~fn_okToMerge(bookFile$,'[ACS Hand Held File Generic Version 2]') then 
		...
		open #hIn:=fngethandle: "Name="&fileIn$,display,input
		open #hOut:=fngethandle: "Name="&bookFile$&",RecL=512,replace",display,output
		pr #hOut: '[ACS Hand Held File Generic Version 2]'
		pr #hOut: 'Source File='&fn_hh_input_filename$
		if dataIncludesHeaders then
			linput #hIn: line$ eof EO_BadgerBeacon ! just consume the headers
		end if
		do
			z$=''
			linput #hIn: line$ eof EO_BadgerBeacon
			fn_BadgerBeaconParseLine(line$,mat tmpDataName$,mat tmpDataValue$)
...
fnend
def fn_BadgerBeaconParseLine(line$*1024,mat tmpDataName$,mat tmpDataValue$)
...
def fn_boson(bookFile$*512)
  if ~exists(fn_hh_input_filename$) then
    ...
    fnmsgbox(mat ml$, response$, cap$,0)
  else
    fnCopy(fn_hh_input_filename$,bookFile$)
...
def fn_ezreader(bookFile$*512)
  ...
  do
    linput #2: easy$ eof EXREADER_XIT
    z$=lpad$(trim$(easy$(209:228)),10)
    reading=val(easy$(309:318))
...
def fn_sensus_in(bookFile$*512)
  ...
  do
    read #h_sensus,using "form pos 1,c 22": ln$ eof SENSUS_IN_XIT ioerr SENSUS_IN_XIT
    pr #h_readings,using "form pos 1,c 132": ln$
  loop
  SENSUS_IN_XIT: !
...
def fn_import_l_readings_txt(bookFile$*512; inFileRecordLen)
  ...
  do
    linput #hHandHeld: a$ eof ilrt_EO_L_READINGS_TXT
    if deviceSelected$="Other" and env$('client')="Brier Lake" then
      parseResponse=fn_ilrt_lineParse_BrierLake(a$,z$,reading$)
    else if deviceSelected$='READy Water' then
      ! parseResponse=fn_ilrt_lineParse_READy_Water(a$,z$,reading$)
      parseResponse=fn_ilrt_lineParseDelimited(a$,z$,1,reading$,3)
    else if deviceSelected$='Master Meter' then
      parseResponse=fn_ilrt_lineParseFixedWidth(a$,z$,1,10,reading$,14,14, readingDate$,35,8)
    else
...
fnend
def fn_ilrt_lineParse_BrierLake(a$*150,&z$,&reading$)
  ilpblReturn=0
  x=val(a$(1:3)) conv ilpbl_finis
  z$=""
  for j=1 to 8
    x=val(a$(j:j)) conv ilrt_L1060
    z$=z$&a$(j:j)
  next j
  ilrt_L1060: !
  z=val(z$)
  ...
  reading$=""
  for j1=1 to 20
    x=val(a$(j1+j:j1+j)) conv ilrt_L1120
...
fnend
def fn_ilrt_lineParse_READy_Water(a$*150,&z$,&reading$)
  ilprwReturn=0
  z$=reading$=""
  str2mat(a$,mat ilprwItem$, chr$(9))
...
fnend
def fn_ilrt_lineParseDelimited(a$*512,&key$,item_key,&reading$,item_reading; &readingDate$,item_readingDate)
  dim ilpdItem$(0)*512
  ilprwReturn=0
  z$=reading$=""
  str2mat(a$,mat ilpdItem$, chr$(9))
