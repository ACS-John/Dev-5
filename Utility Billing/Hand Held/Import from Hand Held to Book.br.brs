...
34140         let hif_return$="*ask*"
34160       else if device$="Hersey" then 
34180         let hif_return$=path$
34200       else if device$="EZReader" then 
34220         let hif_return$=path$
34240       else if device$="ACS Meter Reader" then 
34260         let hif_return$=path$
34280       else if device$="AMR" then 
34300         let hif_return$=path$
34320       else if device$="Itron FC300" then 
34340         if env$('client')='Findlay' then 
34360           let hif_return$='\\vof-pc\itronshared\FCS\Export\Output\UPLOAD.DAT'
34380 !         hif_return$= "C:\Itron\FCSShare\Export\Output\upload.dat"
34400         else 
34420           let hif_return$="C:\mvrs\xfer\upload\UPLOAD.DAT"
34440         end if 
34480       else if device$="Psion Workabout" then 
34500         let hif_return$=env$('Q')&"\UBmstr\Readings.out"
34520       else if device$="Badger" or device$="DriveBy" then 
34540         if env$('client')="Sangamon" then 
34560           let hif_return$="c:\Progra~1\connect3\connect.ot3"
34580         else 
34600           let hif_return$="c:\connect\connect.ot3"
34620         end if 
34640       else if device$="Unisys" then 
34660         let hif_return$=env$('Q')&"\UBmstr\ReadOut.dat"
34680       else if device$="Boson" then 
34700         if env$('client')="Monticello" then 
34720           let hif_return$=env$('Q')&"\UBmstr\outpalm.txt"
34740         else 
34760           let hif_return$=env$('Q')&"\UBmstr\outofpalm.txt"
34780         end if 
34800       else if device$="Green Tree" then 
34820         let hif_return$=path$
...
50420         let itron_meter_category$=line$(94:94)
50450       else if line_type$="RDG" then 
50480         let itron_reading=val(line$(34:43))
50510 !    itron_read_date$=line$(48:55)
50540         let itron_meter_chenge_out$=line$(92:92)
50570         if itron_meter_chenge_out$="Y" then let meterroll=1 else let meterroll=0
50600         if itron_meter_category$="E" then ! Electric
50630           let reading_electric=itron_reading
50660           let meterroll_electric=meterroll
50690         else if itron_meter_category$="G" then ! Gas
50720           let reading_gas=itron_reading
50750           let meterroll_gas=meterroll
...
50870           if env$('client')='Millry' then let reading_water=itron_reading*10 else let reading_water=itron_reading
50900           let meterroll_water=meterroll
50930         end if 
50960       else if line_type$="RFF" or line_type$="WRR" then 
50990         let tmpr$=line$(55:56)
51020         if val(tmpr$)=0 then let tmpr$=line$(57:58)
51050       end if 
51080     loop 
51110 EO_ITRON: ! 
...
66280     let reading$=reading$&a$(j1+j:j1+j)
66300     ilrt_L1120: ! 
66320   next j1
66340   ilpblReturn=1
66360   ilpbl_finis: !
