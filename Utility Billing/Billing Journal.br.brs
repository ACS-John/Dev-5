...
AccumulateTotalsByCode: ! r: ACCUMULATE TOTALS BY CODE
	for j=1 to 10
		...
		if x2<>0 then 
			t1(j,x2,1)=t1(j,x2,1)+1
			t1(j,x2,2)=t1(j,x2,2)+g(j)
			t1(j,x2,3)=t1(j,x2,3)+u2 : u2=0
		end if 
		L1720: ! 
	next j
return  ! /r
PrTotalsByCode: ! r: pr TOTALS BY CODE
	...
	for j1=1 to 9
		for j2=1 to 200
			...
			k$=st$(j1)(1:2)&cnvrt$("N 2",j2)
			de$=""
			read #hRate,using "Form POS 5,C 40",key=k$,release: de$ nokey PrCol1,ioerr PrCol1
	...
	next j
	L2010: ! 
	pr #255: " ": pr #255: "    Total # of Bills To Be Printed: "&str$(totalBillsToBePrinted)
return  ! /r
