...
SCREEN1: ! r:
	...
	if reportSequence=sequenceAccount then
		open #hCustomerForReport:=fngethandle: "Name=[Q]\UBmstr\Customer.h[cno],KFName=[Q]\UBmstr\ubIndex.h[cno],Shr",internal,input,keyed
	else ! if reportSequence=sequenceRoute then
		open #hCustomerForReport:=fngethandle: "Name=[Q]\UBmstr\Customer.h[cno],KFName=[Q]\UBmstr\ubIndx5.h[cno],Shr",internal,input,keyed
	end if
	if filterRoute then
