...
def library fncalk(x$,d1,f,usage_water,x2,x3,mc1,mu1,mat rt,mat a,mat b,mat c,mat d,mat g,mat w,mat x,mat extra,mat gb,h_ratemst,deposit2,btu; calc_interest_on_deposit,charge_inspection_fee,interest_credit_rate)
	...
	if fn_PassesOnlyMonthFilter(1) and serviceName$(1)<>"" and service$(1)="WA" then fn_calk_water ! always use WA as water code in rate file
	if fn_PassesOnlyMonthFilter(3) and serviceName$(3)="Lawn Meter" and service$(3)="LM" then fn_calk_lawnmeter ! must always use LM for the rate code for lawn meters
	if fn_PassesOnlyMonthFilter(3) and serviceName$(3)(1:5)="Reduc" then fn_calk_reduc
	if fn_PassesOnlyMonthFilter(2) and serviceName$(2)<>"" and service$(2)="SW" then fn_calk_sewer ! always use SW for sewer code in rate file
	if fn_PassesOnlyMonthFilter(3) and serviceName$(3)="Electric" and service$(3)="EL" then fn_calk_electric ! must always use EL for the rate code for electric
	if fn_PassesOnlyMonthFilter(3) and serviceName$(3)<>"Electric" and trim$(serviceName$(3))<>"" and trim$(service$(3))="EL" then fn_calk_electric !  allow electric go thru usage calculation if beign as another type of meter other that electric
	...
	! If serviceName$(3)<>"" AND SERVICE$(3)="AD" Then fn_calk_administrative_fee ! electric service used of administrative fee
	if fn_PassesOnlyMonthFilter(4) and trim$(serviceName$(4))="Gas" and trim$(service$(4))="GA" then fn_calk_gas
	if fn_PassesOnlyMonthFilter(4) and serviceName$(4)<>"Gas" and trim$(serviceName$(4))<>"" and trim$(service$(4))="GA" then fn_calk_gas !  allow gas go thru usage calculation if being usd as another type of meter other that gas
	...
	for j=first_non_metered_service to 10
		if fn_PassesOnlyMonthFilter(j) then
			if trim$(serviceName$(j))="Interest on Deposit" and calc_interest_on_deposit then
				g(7)=fn_interestCredit(interest_credit_rate,d1,c(4),b(11))
...
fnend
def fn_PassesOnlyMonthFilter(pomfServiceCode; ___,returnN)
	if onlyMonth(pomfServiceCode)<=0 then
		returnN=1
	else if onlyMonth(pomfServiceCode)=date(days(d1,'mmddyy'),'mm') then
		returnN=1
	end if
	fn_PassesOnlyMonthFilter=returnN
fnend
def fn_calk_water
	! if debugAccount then pr x$; 'about to go through water routine' : pause
	if x(9) then
		w(1)=x(9)
		if x(12) then usage_water=x(12)
		goto WATER_COMPLETED
	end if
	if x(12) then usage_water=x(12)
	if usage_water>=0 then goto L2720
	goto STANDARD_WATER_CHARGE
	L2720: !

	if b(1)><0 then goto STANDARD_WATER_CHARGE ! 2140 ! CALCULATION
	! WATER
	...
	! 	pr 'fn_calk_water completed for '&x$
	! 	pr '    usage_water=';usage_water
	! 	pause
	! end if
fnend
def fn_calk_sewer
	! uses x(5),w(2),b(2),a(2),extra(18),usage_water
	! reads mc1,mu1,mat rt
	! sets w(2),a(2),usage_sewer
	if debugAccount then
		pr x$&' - entering fn_calk_sewer'
	end if
	! if env$('client')='Lovington' and a(2)>0 then g(6)=5 ! storm sewer

	if x(5)><0 then w(2)=x(5) : goto SewerCompleted
	if b(2)><0 then goto SewerStandardCharge ! was disabled but re-enabled on 12/5/16 - standard charge should work on sewer.  clients like Pennington need it.

	!   if env$('client')="Ashland" and a(2)=1 and g(1)<>0 then w(2)=round(g(1)*3/4,2) : goto SewerCompleted

	read #h_ratemst,using FORM_RATEMSTR,key="SW"&lpad$(str$(a(2)),2): mc1,mu1,mat rt nokey SewerStandardCharge
	...
	SewerStandardCharge: ! r:
		w(2)=b(2)
	goto SewerCompleted ! /r
...
def fn_service_chg_from_history(service_number,history_date,scfh_account$)
	if ~scfh_setup then
		scfh_setup=1
		open #scfh_h_trans=fnH: "Name=[Q]\UBmstr\UBTransVB.h[cno],KFName=[Q]\UBmstr\UBTrIndx.h[cno],Shr",i,i,k
	end if
	scfh_return=0
...
def fn_calk_non_metered(j) ! all non-metered charges but penalty and tax
	...
	if entered_amt>0 then
		...
		! if env$('client')="Carrizo" and j=7 then gosub TrashTaxCarrizo
		if env$('client')="Pennington" and service_code$='SF' then gosub ServiceFeePennington
	end if

	NM_XIT: !
	fn_calk_non_metered=calk_non_metered_return
fnend
! TrashTaxCarrizo: ! r:  called from fn_calk_non_metered
!   if extra(12)=0 then
!     calk_non_metered_return=0
...
! return  ! /r
ServiceFeePennington: ! r: flat percentage on water and sewer
...
! r: def fn_calk_administrative_fee !  (once used on Divernon)
!   if x(10) then
!     w(3)=x(10)
!   else
...
def fn_calk_penalty ! penalty calculation
	...
	for j=1 to 10
		...
		! if env$('client')="Brier Lake" and d1<>f then basepenalty(10)=basepenalty(10)+bal-gb(10)
		if env$('client')="Brier Lake" then basepenalty(10)=basepenalty(10)+bal-gb(10)

		! if env$('client')="Granby" and d1=f  then basepenalty(10)=basepenalty(10)+bal-gb(10)
		! if env$('client')="Granby" and d1<>f then basepenalty(10)=basepenalty(10)+bal-gb(10)
		if env$('client')="Granby" then basepenalty(10)=basepenalty(10)+bal-gb(10)
	...
	!   end if
	if env$('client')="Kimberling" then fn_penalty_kimberling ! calculate interest on prev balance
	g(10)=max(0,g(10))
...
def fn_calk_sales_tax
	...
	! L5300: !
	! if debugAccount then pr x$; 'taxcode=';taxcode;'    taxservice=';taxservice : pause
	taxable=0
	...
	!   else
		...
		next j
	!   end if
	! if debugAccount then pr x$; 'taxcode=';taxcode;'    taxservice=';taxservice;' total taxable amount:';taxable : pause
...
fnend
def fn_calk_gas
	! if debugAccount then pr x$; 'about to go through gas routine' : pause
...
def fn_calk_for_final_bill
	! if debugAccount then pr x$ : pause
	...
	if extra(17)=2 then
		...
		if d1=f then
			fn_depositRefund(x$,d1) ! recalculation and deposit possibly already refunded
		else
			if b(8)<>0 then fnDepositChangeLog(x$,b(8),0,d1,trim$(serviceName$(1))(1:15)&' Deposit Refunded')
			if b(9)<>0 then fnDepositChangeLog(x$,b(9),0,d1,trim$(serviceName$(2))(1:15)&' Deposit Refunded')
			if b(10)<>0 then fnDepositChangeLog(x$,b(10),0,d1,trim$(serviceName$(3))(1:15)&' Deposit Refunded')
			if b(11)<>0 then fnDepositChangeLog(x$,b(11),0,d1,trim$(serviceName$(4))(1:15)&' Deposit Refunded')
			b(8)=b(9)=b(10)=b(11)=0
		end if
	end if
	! if debugAccount then pr x$&' has a g('&str$(serviceOther)&') of '&str$(g(serviceOther))&' at the end of fn_calk_for_final_bill' : pause
fnend  ! fn_calk_for_final_bill
def fn_depositRefund(rk$*10,d1)
	! uses a lot of local variables ie:
	! check to see if recalculation and deposit already refunded on previous calculation
	! dim da(2)
	! if debugAccount then pr rk$&' entered fn_depositRefund'
	if rk$<>"" then
		dt1=fncd(d1)
		...
		do while rkRead$=rk$
			...
			! end if
			if olddt1=dt1 and pos(dp$,' Deposit Refunded')>0 then
				! if debugAccount then pr '    olddt1=dt1 ('&str$(dt1)&')'
				! if debugAccount then pr 'removing '&str$(odp)&' from g('&str$(serviceOther)&') for '&dp$ : pause
	...
	DEPR_XIT: !
	! if debugAccount then pr 'at the end of fn_depositRefund '&rk$&' has a g('&str$(serviceOther)&') of '&str$(g(serviceOther)) : pause
fnend
def fn_calk_demand
	...
	L6360: !
	if env$('client')="Bethany" then g(6)=mc1: goto L6380
...
fnend
def fn_interestCredit(interest_credit_rate,d1,c4,b11; ___,cy1,cy2,m1,returnN)
	! requires b(11),interest_credit_rate
	returnN=round(-(interest_credit_rate*b11),2)
	if c4>=10100 then
		cy1=date(days(d1,'mmddyy'),'ccyy')
		cy2=date(days(c4,'mmddyy'),'ccyy')
		m1=(cy1-cy2)*12
		if m1>12 then goto IcFinis
		m2=int(d1*.0001)-int(c4*.0001)
		m1+=m2
		returnN=round(((returnN/12)*m1),2)
		if m1<12 then goto IcXit ! don't allow a credit if less than 12 months
	end if
	IcFinis: !
	if returnN>0 then returnN=0
	IcXit: !
	fn_interestCredit=returnN
fnend
