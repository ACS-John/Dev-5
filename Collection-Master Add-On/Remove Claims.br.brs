...
INPUT_FILENO: rinput #1,fields "3,26,C 10,[D]SAE;5,26,C 8,[D]SA": inp_dayt$,fileno$
	if cmdkey=99 then 
		goto DONE_BY_FILENO
	end if
...
if fnwarning_msg then 
	_deleted=fndelete(fileno$,r$,dayt$,1)
end if
DONE_BY_FILENO: ! r:
if _deleted then 
...
def fnsel(width; printer_prompt$*80,printfile_handle, print_cancel_option$*80,supported_printer_type_list$*80,print_destination_custom$*1024,print_pk$*32,printer_select_override) ! open printer routine
	fnsel=fnsel_lib(width,printer_prompt$,printfile_handle, print_cancel_option$,supported_printer_type_list$,print_destination_custom$,pk$,printer_select_override)
	if fkey=93 or fkey=99 then 
		prnname$=comp$=ff$=orient$="" : lpp=wid=prn_num=0
...
fnend  ! fnSel
def library fndelete_claim(delete_fileno$;date_recd$,delete_date_recd$,no_report)
	fndelete_claim=fndelete(delete_fileno$,date_recd$,delete_date_recd$,no_report)
fnend 
def fndelete(delete_fileno$,date_recd$;delete_date_recd$,no_report) 
