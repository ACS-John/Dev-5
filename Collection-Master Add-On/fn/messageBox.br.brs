...
!     mbx_button_default+=1 ! because File will be the first button, so skip it.
		...
		open #Mbx_H_Win:=fngethandle: fn_open_Parent$(Mbx_Win$,mat winData$,1,1),DISPLAY,OUTPUT
		...
		open #Mbx_H_Text:=fngethandle: 'Modal,Parent='&str$(Mbx_H_Win)&',Border=None,SRow=1,SCol=10,Rows='&str$(Mbx_H_Text_Rows)&',Cols='&str$(hTextCols),DISPLAY,OUTPUT
...
def fn_fileMenu(Mbxfm_Title$*80,fileCount, _
	...
	open #Mbxfm_H_Win:=fngethandle: fn_open_Parent$(actionWin$,mat actionWinData$,1),DISPLAY,OUTPUT
	...
	open #hTab(1):=fngethandle: fn_open_Parent$(actionTab$&',Tab=OS Path'&s$,mat Mbxfm_Tab_Data$,1),DISPLAY,OUTPUT
	open #hTab(2):=fngethandle: fn_open_Parent$(actionTab$&',Tab=Virtual Path'&s$,mat Mbxfm_Tab_Data$,1),DISPLAY,OUTPUT
	dim fileField$(0)*20
